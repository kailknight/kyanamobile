...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	if (lpObj->StatusCacheItem != -1)
	{
		for (int i = 0; i < 3; i++)
		{
			if (!gObj[aIndex].CH_InfoItem[i].IsItem()) continue;

			BYTE SlotRecv = gItemManager.InventoryInsertItem(aIndex, lpObj->CH_InfoItem[i]); // Roll back lai Item
			if (SlotRecv != 0xFF)
			{
				gItemManager.GCItemModifySend(aIndex, SlotRecv);

			}
			gObj[aIndex].CH_IndexItem[i] = -1;
			gObj[aIndex].CH_InfoItem[i].Clear();
		}
		gObj[aIndex].StatusCacheItem = -1;
		GDCharacterInfoSaveSend(aIndex);
	}
#if(CB_HUYDONGEXC)
	gBHuyDongExc.ClearCache(lpObj->Index);
#endif
	...
	gLog.Output(LOG_ITEM_CHARACTER, "[CHECKITEM] OUT Game Account:[%s]- Name[%s]", lpObj->Account, lpObj->Name);
	...
	gLog.Output(LOG_ITEM_CHARACTER, "[CHECKITEM] Thoat Game [%s][%s]", lpObj->Account, lpObj->Name);

	LogAddConnect(LOG_USER,"[%d] LogOutCharacter (%s)",lpObj->Index,lpObj->Name);
...
bool CObjectManager::CharacterUseJewelOfBles(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	//LogAdd(LOG_RED, "Bless Get Type %d Slot %d", lpItem->m_Index, lpItem->m_Slot);
	if (lpItem->m_Slot == 9 || lpItem->m_Slot == 10)
	{
		goto Next;
	}
...
bool CObjectManager::CharacterUseJewelOfSoul(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	//	return 0;
	//}
	//LogAdd(LOG_RED, "Get Type %d Slot %d", lpItem->m_Index, lpItem->m_Slot);
	if (lpItem->m_Slot == 9 || lpItem->m_Slot == 10)
	{
		goto Next;
	}

	if(lpItem->m_Index > GET_ITEM(12,6) && (lpItem->m_Index < GET_ITEM(12,36) || lpItem->m_Index > GET_ITEM(4, 7) && (lpItem->m_Index < GET_ITEM(4, 15)) || lpItem->m_Index > GET_ITEM(12,43)) && lpItem->m_Index != GET_ITEM(12,49) && lpItem->m_Index != GET_ITEM(12,50) && (lpItem->m_Index < GET_ITEM(12,262) || lpItem->m_Index > GET_ITEM(12,265)) && lpItem->m_Index != GET_ITEM(13,30) && lpItem->IsPentagramItem() == 0 && gCustomWing.CheckCustomWingByItem(lpItem->m_Index) == 0)
	{
		return 0;
	}
Next:
...
bool CObjectManager::CharacterUseCustomJewel(LPOBJ lpObj,int SourceSlot,int TargetSlot) // OK
{
	...
	{
		int ItemUpdateIndex = gCustomJewel.GetUpdateInfo(lpObj->Inventory[SourceSlot].m_Index, lpItem->m_Index);
		// LogAdd(LOG_RED, "Find Item %d %d", lpItem->m_Index, ItemUpdateIndex);

		CUSTOM_JEWEL_SUCCESS_INFO* lpInfo = gCustomJewel.GetSuccessInfo(lpObj->Inventory[SourceSlot].m_Index);

		if(ItemUpdateIndex != -1) lpItem->m_Index = ItemUpdateIndex;
...
// The *AttackSuccessRatePvPConst*/*DefenseSuccessRatePvPConst* fields below
// now default-initialize to 0 (see ServerInfo.h) instead of holding whatever
// was on the heap - but every formula that uses them divides by one
// unconditionally, so a config that is still missing/late-loaded would turn
// the old "garbage number displayed" bug into a division-by-zero crash
// instead. This keeps the fail-safe behaviour: an unset (0) constant now
// reads as 0 rather than either garbage or a crash.
static inline int SafeDivInt(int numerator, int denominator)
{
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
...
#if(CB_VIP_CHAR)
		gBCustomVIPChar.ActiveBonus(lpObj, 1);
#endif
#if(DANH_HIEU_NEW)
	...
	if(lpObj->Class == CLASS_DW)
	{
		lpObj->AttackSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_DWAttackSuccessRatePvPConstA, gServerInfo.m_DWAttackSuccessRatePvPConstB) + SafeDivInt(Dexterity*gServerInfo.m_DWAttackSuccessRatePvPConstC, gServerInfo.m_DWAttackSuccessRatePvPConstD);
	}
	else if(lpObj->Class == CLASS_DK)
	{
		lpObj->AttackSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_DKAttackSuccessRatePvPConstA, gServerInfo.m_DKAttackSuccessRatePvPConstB) + SafeDivInt(Dexterity*gServerInfo.m_DKAttackSuccessRatePvPConstC, gServerInfo.m_DKAttackSuccessRatePvPConstD);
	}
	else if(lpObj->Class == CLASS_FE)
	{
		lpObj->AttackSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_FEAttackSuccessRatePvPConstA, gServerInfo.m_FEAttackSuccessRatePvPConstB) + SafeDivInt(Dexterity*gServerInfo.m_FEAttackSuccessRatePvPConstC, gServerInfo.m_FEAttackSuccessRatePvPConstD);
	}
	else if(lpObj->Class == CLASS_MG)
	{
		lpObj->AttackSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_MGAttackSuccessRatePvPConstA, gServerInfo.m_MGAttackSuccessRatePvPConstB) + SafeDivInt(Dexterity*gServerInfo.m_MGAttackSuccessRatePvPConstC, gServerInfo.m_MGAttackSuccessRatePvPConstD);
	}
	else if(lpObj->Class == CLASS_DL)
	{
		lpObj->AttackSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_DLAttackSuccessRatePvPConstA, gServerInfo.m_DLAttackSuccessRatePvPConstB) + SafeDivInt(Dexterity*gServerInfo.m_DLAttackSuccessRatePvPConstC, gServerInfo.m_DLAttackSuccessRatePvPConstD);
	}
	else if(lpObj->Class == CLASS_SU)
	{
		lpObj->AttackSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_SUAttackSuccessRatePvPConstA, gServerInfo.m_SUAttackSuccessRatePvPConstB) + SafeDivInt(Dexterity*gServerInfo.m_SUAttackSuccessRatePvPConstC, gServerInfo.m_SUAttackSuccessRatePvPConstD);
	}
	else if(lpObj->Class == CLASS_RF)
	{
		lpObj->AttackSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_RFAttackSuccessRatePvPConstA, gServerInfo.m_RFAttackSuccessRatePvPConstB) + SafeDivInt(Dexterity*gServerInfo.m_RFAttackSuccessRatePvPConstC, gServerInfo.m_RFAttackSuccessRatePvPConstD);
	}
	...
	if(gServerInfo.m_AttackSpeedAgiStep > 0)
	{
		lpObj->PhysiSpeed = (Dexterity/gServerInfo.m_AttackSpeedAgiStep)*gServerInfo.m_AttackSpeedPerStep;
		lpObj->MagicSpeed = lpObj->PhysiSpeed;
	}
	...
	// Clamp the final attack speed to the per class maximum. Applied here so it
	// covers every source above (stats, drink, buffs, and all equipment).
	// A configured value of 0 means no cap.
	int MaxPhysiSpeed = 0;
	int MaxMagicSpeed = 0;

	if(lpObj->Class == CLASS_DW)
	{
		MaxPhysiSpeed = gServerInfo.m_DWMaxAttackPhysiSpeed;
		MaxMagicSpeed = gServerInfo.m_DWMaxAttackMagicSpeed;
	}
	else if(lpObj->Class == CLASS_DK)
	{
		MaxPhysiSpeed = gServerInfo.m_DKMaxAttackPhysiSpeed;
		MaxMagicSpeed = gServerInfo.m_DKMaxAttackMagicSpeed;
	}
	else if(lpObj->Class == CLASS_FE)
	{
		MaxPhysiSpeed = gServerInfo.m_FEMaxAttackPhysiSpeed;
		MaxMagicSpeed = gServerInfo.m_FEMaxAttackMagicSpeed;
	}
	else if(lpObj->Class == CLASS_MG)
	{
		MaxPhysiSpeed = gServerInfo.m_MGMaxAttackPhysiSpeed;
		MaxMagicSpeed = gServerInfo.m_MGMaxAttackMagicSpeed;
	}
	else if(lpObj->Class == CLASS_DL)
	{
		MaxPhysiSpeed = gServerInfo.m_DLMaxAttackPhysiSpeed;
		MaxMagicSpeed = gServerInfo.m_DLMaxAttackMagicSpeed;
	}
	else if(lpObj->Class == CLASS_SU)
	{
		MaxPhysiSpeed = gServerInfo.m_SUMaxAttackPhysiSpeed;
		MaxMagicSpeed = gServerInfo.m_SUMaxAttackMagicSpeed;
	}
	else if(lpObj->Class == CLASS_RF)
	{
		MaxPhysiSpeed = gServerInfo.m_RFMaxAttackPhysiSpeed;
		MaxMagicSpeed = gServerInfo.m_RFMaxAttackMagicSpeed;
	}

	if(MaxPhysiSpeed > 0 && lpObj->PhysiSpeed > MaxPhysiSpeed)
	{
		lpObj->PhysiSpeed = MaxPhysiSpeed;
	}

	if(MaxMagicSpeed > 0 && lpObj->MagicSpeed > MaxMagicSpeed)
	{
		lpObj->MagicSpeed = MaxMagicSpeed;
	}
	...
	if(lpObj->Class == CLASS_DW)
	{
		lpObj->DefenseSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_DWDefenseSuccessRatePvPConstA, gServerInfo.m_DWDefenseSuccessRatePvPConstB) + SafeDivInt(Dexterity, gServerInfo.m_DWDefenseSuccessRatePvPConstC);
	}
	else if(lpObj->Class == CLASS_DK)
	{
		lpObj->DefenseSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_DKDefenseSuccessRatePvPConstA, gServerInfo.m_DKDefenseSuccessRatePvPConstB) + SafeDivInt(Dexterity, gServerInfo.m_DKDefenseSuccessRatePvPConstC);
	}
	else if(lpObj->Class == CLASS_FE)
	{
		lpObj->DefenseSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_FEDefenseSuccessRatePvPConstA, gServerInfo.m_FEDefenseSuccessRatePvPConstB) + SafeDivInt(Dexterity, gServerInfo.m_FEDefenseSuccessRatePvPConstC);
	}
	else if(lpObj->Class == CLASS_MG)
	{
		lpObj->DefenseSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_MGDefenseSuccessRatePvPConstA, gServerInfo.m_MGDefenseSuccessRatePvPConstB) + SafeDivInt(Dexterity, gServerInfo.m_MGDefenseSuccessRatePvPConstC);
	}
	else if(lpObj->Class == CLASS_DL)
	{
		lpObj->DefenseSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_DLDefenseSuccessRatePvPConstA, gServerInfo.m_DLDefenseSuccessRatePvPConstB) + SafeDivInt(Dexterity, gServerInfo.m_DLDefenseSuccessRatePvPConstC);
	}
	else if(lpObj->Class == CLASS_SU)
	{
		lpObj->DefenseSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_SUDefenseSuccessRatePvPConstA, gServerInfo.m_SUDefenseSuccessRatePvPConstB) + SafeDivInt(Dexterity, gServerInfo.m_SUDefenseSuccessRatePvPConstC);
	}
	else if(lpObj->Class == CLASS_RF)
	{
		lpObj->DefenseSuccessRatePvP = SafeDivInt((lpObj->Level+lpObj->MasterLevel)*gServerInfo.m_RFDefenseSuccessRatePvPConstA, gServerInfo.m_RFDefenseSuccessRatePvPConstB) + SafeDivInt(Dexterity, gServerInfo.m_RFDefenseSuccessRatePvPConstC);
	}
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
	...
	gDanhHieu.TinhToanChiSoCapDoDanhHieu(lpObj);
#endif
#if(CB_VIP_CHAR)
	lpObj->LvVIPChar = lpMsg->LvVIPChar;
	lpObj->PointVIPChar = lpMsg->PointVIPChar;
	//LogAdd(LOG_RED, "[VIPChar] %s Lv %d Point %d", lpObj->Name, lpObj->LvVIPChar, lpObj->PointVIPChar);
#endif	
#if(MOCNAP)
...
#endif
#if(CB_NEWQUESTFF)
	lpObj->NQuest_LV = lpMsg->NQuest_LV;
	lpObj->NQuest_Status = lpMsg->NQuest_Status;
	lpObj->NQuest_C[0] = lpMsg->NQuest_C[0];
	lpObj->NQuest_C[1] = lpMsg->NQuest_C[1];
	lpObj->NQuest_C[2] = lpMsg->NQuest_C[2];
	lpObj->NQuest_C[3] = lpMsg->NQuest_C[3];
	lpObj->NQuest_C[4] = lpMsg->NQuest_C[4];
	LogAdd(LOG_RED, "[NQuest]%s Status %d Lv %d (%d, %d, %d, %d, %d)", lpObj->Name,lpObj->NQuest_Status, lpObj->NQuest_LV, lpObj->NQuest_C[0], lpObj->NQuest_C[1], lpObj->NQuest_C[2], lpObj->NQuest_C[3], lpObj->NQuest_C[4]);
#endif
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,QWORD damage,int DamageType,int flag,int type,int skill,int ShieldDamage) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			gCustomMonterPoint.MonsterDieProc(lpTarget, &gObj[SummonIndex]);
...
#if(CB_NEWQUESTFF)
			gCB_NewQuest.MonsterDieProc(lpTarget,&gObj[SummonIndex]);
#endif
		...
		// PvP damage ramp reset on death (PvPDamageRamp*, CustomConfig.ini).
		// Two halves, and both are needed:
		//
		//   - the corpse loses its own ramp outright, so it respawns fighting
		//     at base damage rather than resuming a bonus it earned before it
		//     died;
		//   - bumping its life id invalidates the ramp of whoever was hitting
		//     it, without walking every object on the server looking for them.
		//     The killer's stored copy of this id stops matching, so its next
		//     blow - on this player or on anyone who later lands in the same
		//     object slot - starts a fresh engagement.
		lpTarget->m_PvPRampLifeId++;

		for(int n=0;n < MAX_PVP_RAMP_SLOTS;n++)
		{
			lpTarget->m_PvPRamp[n].TargetIndex = -1;
			lpTarget->m_PvPRamp[n].TargetLifeId = 0;
			lpTarget->m_PvPRamp[n].StartTick = 0;
			lpTarget->m_PvPRamp[n].LastHitTick = 0;
		}
