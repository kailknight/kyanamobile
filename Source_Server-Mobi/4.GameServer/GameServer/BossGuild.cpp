...
void CBossGuild::MainProc() // OK
{
	...
	if (this->m_RemainTimeWinner > 0)
	{
		if (this->WinnerNumber >= 0)
		{
			...
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3013));
			}
			...
			if (this->m_RemainTimeWinner == 5) //Trao thuong cho Guild
			{
				...
				else
				{
					...
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3014), lpGuildInfo->Name);
					//===============================================
					...
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3015), lpGuildInfo->Name, GuildWin.Score);
					//===============================================
					...
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3016), GuildWin.WCoin, GuildWin.WCoinP, GuildWin.GobinP);
					//===============================================
					// Sét điểm Guild về 0
					for (int n = 0; n < MAX_OBJECT; n++)
					{
			...
			if (this->m_RemainTimeWinner <= 0)
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3017));
		...
		else
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3018));
...
void CBossGuild::ProcState_EMPTY() // OK
{
	...
	if (this->m_RemainTime > 0 && this->m_RemainTime <= (this->m_WarningTime * 60))
	{
		...
		if (this->MinutesLeft != minutes)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3019), (MinutesLeft + 1));
		}
	...
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3020), m_RemainTime);
	}

	if (this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3021)); //Bat Dau Su Kien
...
void CBossGuild::ProcState_START() // OK
{
	if (this->m_RemainTime > 0 && this->m_RemainTime <= 300)
	{
		...
		if (this->MinutesLeft != minutes)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3022), (MinutesLeft + 1));
		}
	}

	if (this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3023));
...
void CBossGuild::CGPacketBossGuild(BOSSGUILD_CGPACKET * aRecv, int aIndex)
{
	...
	if ((GetTickCount() - lpObj->ClickClientSend) < 2000) {
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3086)); //"Ban dang thao tac qua nhanh vui long cho doi !"
		return;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3090)); //"Chưa đến thời gian sự kiện"
		return;
	}
	...
	if (lpGuildInfo == NULL)
	{
		...
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3087)); //"[BossGuild] Không thấy thông tin Guild của bạn"
		return;
	}

	if (lpObj->GuildNumber == 0) //Verifica se o personagem faz parte de uma guild
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3088)); //"[BossGuild] Bạn chưa vào Guild nên không thể tham gia !"
		return;
	}

	if (lpGuildInfo->TotalCount < this->GuildMinPlayer) //So Thanh Vien Toi Thieu Tham Gia
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3089)); //"[BossGuild] Guild bạn không đủ người than gia !"
		return;
	}
...
bool CBossGuild::MonsterDie(int aIndexMonster, int aIndexUser)
{
	...
	if ((lpMonster->Class == 701) && lpMonster->Map == this->m_MapBossGuild)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3024), lpUser->GuildName, (lpUser->GuildNumber, lpUser->Index));
	...
	if ((lpMonster->Class == 700) && lpMonster->Map == this->m_MapBossGuild)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3025), lpUser->GuildName, (lpUser->GuildNumber, lpUser->Index));
	...
	if ((lpMonster->Class == 702) && lpMonster->Map == this->m_MapBossGuild)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3026), lpUser->GuildName, (lpUser->GuildNumber, lpUser->Index));
	...
	if ((lpMonster->Class == 705) && lpMonster->Map == this->m_MapBossGuild)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3027), lpUser->GuildName, (lpUser->GuildNumber, lpUser->Index));
	...
	if ((lpMonster->Class == 706) && lpMonster->Map == this->m_MapBossGuild)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3028), lpUser->GuildName, (lpUser->GuildNumber, lpUser->Index));
	...
	if ((lpMonster->Class == 703) && lpMonster->Map == this->m_MapBossGuild)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3029), lpUser->GuildName, (lpUser->GuildNumber, lpUser->Index));
	...
	if ((lpMonster->Class == 704) && lpMonster->Map == this->m_MapBossGuild)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3030), lpUser->GuildName, (lpUser->GuildNumber, lpUser->Index));
