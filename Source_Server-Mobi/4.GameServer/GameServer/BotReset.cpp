...
BOOL BotReset::TradeOpen(int index, int nindex)
{
	...
	if(this->Item_Req[number].ResetType == 0)
	{
		...
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(3031),lpObj->Name);
	...
	if(this->Item_Req[number].ResetType == 1)
	{
		...
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(3032),lpObj->Name);
		if(this->Item_Req[number].PCP != 0){ gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"-%d PCPoint: %d",this->Item_Req[number].PCP,
			lpObj->PCPoint); }
	...
	if(this->MarryReq[number].MarryActive == 1)
	{
		if(lpObj->MarryCharacterInfo.Married != 0 )
		{
			...
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(3033),lpObj->Name);
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(3034),lpObj->LevelUpPoint,lpObj->Money);
