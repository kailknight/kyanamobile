...
void CCTCmini::MainProc() // OK
{
	...
	if (this->m_RemainTimeWinner > 0)
	{
		if (this->WinnerNumber >= 0)
		{
			...
			{
				GCServerMsgStringSendToMap(MapCTC, 0, gMessage.GetMessage(3102)); //"[Công Thành chiến Mini] Chuẩn bị tiến hành trao thưởng!"
			}
			...
			if (this->m_RemainTimeWinner == 15) //Trao thuong cho Guild
			{
				...
				else
				{
					...
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3040), lpGuildInfo->Name);
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3041), b_GuildWin.WCoin, b_GuildWin.WCoinP, b_GuildWin.GobinP, b_GuildWin.Ruud);
			...
			if (this->m_RemainTimeWinner <= 0)
			{
				GCServerMsgStringSendToMap(MapCTC, 0, gMessage.GetMessage(3103)); //"[Công Thành chiến Mini] Đã kết thúc !"
		...
		else
		{
			...
			GCServerMsgStringSendToMap(MapCTC, 0, gMessage.GetMessage(3104)); //"[Công Thành chiến Mini] Đã kết thúc không có Guild chiến thắng !"
			this->m_RemainTimeWinner = 0;
			this->Clear();
		}
...
void CCTCmini::ProcState_EMPTY() // OK
{
	...
	if (this->m_RemainTime > 0 && this->m_RemainTime <= (this->m_WarningTime * 60))
	{
		...
		if (this->MinutesLeft != minutes)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3042), (MinutesLeft + 1));
		}
	...
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3043), m_RemainTime);
	}

	if (this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3044)); //Bat Dau Su Kien
...
void CCTCmini::ProcState_START() // OK
{
	...
	if (this->m_RemainTime > 0 && this->m_RemainTime <= 300)
	{
		...
		if (this->MinutesLeft != minutes)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3045), (MinutesLeft + 1));
		}
	}

	if (this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3046));
...
void CCTCmini::CGPacketCTCMini(CTCMINI_CGPACKET* aRecv, int aIndex)
{
	...
	if ((GetTickCount() - lpObj->ClickClientSend) < 2000) {
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3086)); //"Bạn thao tác quá nhanh vui lòng chờ đợi !"
		return;
	}
	...
	if (lpGuildInfo == NULL)
	{
		...
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3105)); //"[Công Thành chiến Mini] Không thấy thông tin Guild của bạn"
		return;
	}

	if (lpObj->GuildNumber == 0) //Verifica se o personagem faz parte de uma guild
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3106)); //"[Công Thành chiến Mini] Bạn chưa vào Guild nên không thể tham gia !"
		return;
	}

	if (lpGuildInfo->TotalCount < this->GuildMinPlayer) //So Thanh Vien Toi Thieu Tham Gia
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3107)); //"[Công Thành chiến Mini] Guild bạn không đủ người than gia !"
		return;
	}
	...
	if (TruDaChiem >= 2) //this->WinnerTime
	{
		...
		for (int n = 0; n < MAX_CTC_GUILD; n++)
		{
			if (Guild[n].Number == GuildNumber)
			{
				...
				GCServerMsgStringSendToMap(MapCTC, 0, gMessage.GetMessage(3108), this->Guild[n].GuildOwner); //"[Công Thành chiến Mini] Guild %s đã chiếm đóng thành công 2 trụ!"
			}
			else
			{
				this->Guild[n].Owner = 0;
	...
	if ((lpMonster->Class == ClassTru || lpMonster->Class == ClassCong) && lpMonster->Map == MapCTC)
	{
		for (int n = 0; n < 3; n++) //Check Kill Tru
		{
			...
			if (IndexTru[n] == lpMonster->Index)
			{
				...
				GCServerMsgStringSendToMap(MapCTC, 0, gMessage.GetMessage(3109), lpUser->GuildName, this->SoTruDaChiem(lpUser->GuildNumber, lpUser->Index)); //"[Công Thành chiến Mini] Guild %s đã chiếm %d Trụ!"
			...
			if (IndexCong[n] == lpMonster->Index)
			{
				...
				GCServerMsgStringSendToMap(MapCTC, 0, gMessage.GetMessage(3110), lpUser->GuildName, n + 1); //"[Công Thành chiến Mini] Guild %s đã phá cổng %d!"
