...
#if(REDEEMCODE)
//-----------------------------------
// Redemption code system, GameServer<->DataServer half. Grouped under the
// existing custom-feature head 0xD9 (see BCustomItemBank's GSSENDDS_COUNT_
// ITEMBANK for the established shape of this group) rather than a dedicated
// top-level head, since 0x30/0x31 are free there. PSBMSG_HEAD (BYTE size) is
// enough - every one of these structs is small and fixed-size, no need for
// the WORD-sized header GSSENDDS_COUNT_ITEMBANK needs for its variable-length
// trailing array.
//-----------------------------------

// Richer than the client-facing REDEEM_ITEM_DETAIL (Protocol.h) - adds
// Duration, which the client doesn't need for a tooltip but the actual grant
// call (GDCreateItemSend) does. Field names mirror that function's own
// parameter breakdown so a grant is a direct 1:1 mapping with no repacking.
struct REDEEM_BUNDLE_ITEM
{
	WORD ItemIndex;
	BYTE ItemLevel;
	BYTE ItemSkill;      // 0/1
	...
	BYTE Quantity;
	DWORD ItemDuration;  // seconds; 0 = permanent
	BYTE SocketOption[MAX_SOCKET_OPTION]; // 0xFF = empty slot, matches GDCreateItemSend's own default fill
	BYTE SocketOptionBonus;
	BYTE Item380;           // 0/1 - ORed into GDCreateItemSend's ItemOptionEx as bit 0x80 (C380ItemOption::Is380Item)
	BYTE HarmonyOption;     // 0-7; packed with HarmonyOptionLevel into GDCreateItemSend's JewelOfHarmonyOption byte
	BYTE HarmonyOptionLevel; // 0-15
};

// GameServer -> DataServer (0xD9 / 0x30) - non-binding preview request.
// Sent both for the player-visible "Check Code" step and, again, silently,
// right before RequestRedeem commits (see RedeemCode.cpp) - this struct
// alone never needs to know which caller it is.
struct GSSENDDS_REDEEM_CODE_PEEK
{
	PSBMSG_HEAD header;
	int aIndex;
	char AccountID[11];
...
// DataServer -> GameServer (0xD9 / 0x30) - preview result. Echoes AccountID
// back (same reason SDHP_CREATE_ITEM_RECV echoes account[] - see
// DGCreateItemRecv's gObjIsAccountValid guard) so GameServer can tell a
// reused aIndex slot (the player disconnected and someone else's session
// landed on the same slot while this round trip was in flight) from the
// player it was actually sent for.
struct SDHP_REDEEM_CODE_PEEK_RECV
{
	PSBMSG_HEAD header;
	int aIndex;
	char AccountID[11];
	BYTE Result;      // eRedeemCodeResult (Protocol.h)
	BYTE ItemCount;
	REDEEM_BUNDLE_ITEM Items[REDEEM_CODE_BUNDLE_MAX];
	// Per-code currency rewards. Appended AFTER Items[] deliberately: inserting
	// them before the array would shift every item entry's offset, which is the
	// exact failure the stride assert below exists to catch.
	DWORD RewardWCoinC;
	DWORD RewardWCoinP;
...
// GameServer -> DataServer (0xD9 / 0x31) - commit request. Only sent after
// RequestRedeem's own inventory-room check for the whole bundle has already
// passed locally - see the plan's Atomicity section for why the ordering
// matters.
struct GSSENDDS_REDEEM_CODE_COMMIT
{
	PSBMSG_HEAD header;
	int aIndex;
	char AccountID[11];
...
// DataServer -> GameServer (0xD9 / 0x31) - commit result. Only REDEEM_SUCCESS
// here means the code was actually consumed; every other result means
// WZ_CommitRedeemCode rolled back and nothing was touched.
struct SDHP_REDEEM_CODE_COMMIT_RECV
{
	PSBMSG_HEAD header;
	int aIndex;
	char AccountID[11];
...
// Hand-duplicated in DataServer's own DataServerProtocol.h. Items[] is an
// array, so an even one-byte stride difference between the two copies leaves
// Items[0] intact and silently corrupts every entry after it - assert the
// stride here so that fails the build instead of the feature. The DWORD
// ItemDuration is what pads this to 24 (vs the client-facing
// REDEEM_ITEM_DETAIL's 18, which carries no duration).
static_assert(sizeof(REDEEM_BUNDLE_ITEM) == 24, "REDEEM_BUNDLE_ITEM must stay 24 bytes and byte-identical to DataServer's copy in DataServerProtocol.h");
static_assert(sizeof(SDHP_REDEEM_CODE_PEEK_RECV) <= 255, "PSBMSG_HEAD stores the packet size in a single BYTE - this packet must never exceed 255 bytes");
#endif
...
// Redemption code's own GD*Send/DG*Recv equivalents are CRedeemCode methods
// (RedeemCode.h/.cpp), not free functions here - see that class for why
// (mirrors BCustomItemBank's self-contained style rather than DSProtocol's).
