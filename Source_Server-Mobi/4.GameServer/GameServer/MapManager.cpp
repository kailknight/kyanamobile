...
void CMapManager::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.ItemDropType = lpMemScript->GetAsNumber();
...
int CMapManager::GetMapItemDropType(int index) // OK
{
	std::map<int, MAP_MANAGER_INFO>::iterator it = this->m_MapManagerInfo.find(index);

	if (it == this->m_MapManagerInfo.end())
	{
		return -1;	// map not configured - fall back to the "*" (all drop) default
	}
	else
	{
		return it->second.ItemDropType;
	}
}

int CMapManager::CheckMapItemDropType(int index,int type) // OK
{
	int DropType = this->GetMapItemDropType(index);

	// -1 ("*") means every type is allowed, same convention as every other
	// wildcard column in this file.
	return ((DropType == -1) ? 1 : ((DropType & type) != 0));
}