...
void CCustomBuyVip::BuyVipDone(LPOBJ lpObj)
{
	...
	// LOG_BLACK, not LOG_EVENT - a VIP purchase is not a scheduled event, and
	// the LOG_EVENT colour now selects the Logs > Event Logs view.
	LogAdd(LOG_BLACK,"[%s][%s] BuyVip (Level: %d)",lpObj->Account,lpObj->Name,lpObj->BuyVip);
