...
// An item index with no Item.txt definition used to drop anyway: the server
// happily created the ground item, GetItemName() returned "No Name", and the
// client had nothing to build a model from - so it landed as a sparkle with no
// mesh and no clickable geometry, i.e. an invisible item that can never be
// picked up. That is a drop-table data error (a section/type that does not
// exist), so refuse the drop and say exactly which entry is wrong instead of
// leaving litter on the map for players to walk into.
//
// The index is reported decomposed as section/type (index = section *
// MAX_ITEM_TYPE + type) because that is how drop tables such as
// ItemBagManager's .txt files are written, so the offending line can be found
// directly.
static bool CheckDropItemDefined(int index,int mapNumber,int x,int y,const char* origin) // OK
{
	ITEM_INFO info;

	if(gItemManager.GetInfo(index,&info) != 0)
	{
		return true;
	}

	LogAdd(LOG_RED,"[ItemDrop] BLOCKED - item index %d (section %d type %d) has no entry in Item.txt - fix the drop table. map=%d pos=%d,%d src=%s",
		index,(index/MAX_ITEM_TYPE),(index%MAX_ITEM_TYPE),mapNumber,x,y,origin);
...
{
	if(CheckDropItemDefined(index,this->m_MapNumber,x,y,"MonsterItemDrop") == false)
	{
		return 0;
	}
...
{
	if(CheckDropItemDefined(index,this->m_MapNumber,x,y,"ItemDrop") == false)
	{
		return 0;
	}
	...
	for(int n=0;n < MAX_MAP_ITEM;n++)
	{
		if(this->m_Item[count].IsItem() == 0)
		{
			...
			// Ground-item lifecycle trace. Every line carries map/slot/serial so
			// a DROP can be matched to its later EXPIRE or PICKUP: a slot that
			// gets a DROP and then a second DROP with no EXPIRE/PICKUP between
			// them was reused underneath somebody's viewport, and any slot with
			// no closing line at all never got cleaned up. Toggle with
			// ItemDropLog in CustomConfig.ini.
			if(gServerInfo.m_ItemDropLog != 0)
			{
				LogAdd(LOG_BLUE,"[ItemLog] DROP map=%d slot=%d serial=%u pos=%d,%d item=%d(%s) level=%d owner=%d dur=%ums",
					this->m_MapNumber,count,this->m_Item[count].m_Serial,x,y,
					index,gItemManager.GetItemName(index),level,aIndex,duration);
			}
...
bool CMap::CheckItemGive(int aIndex,int index,bool notify) // OK
{
	...
	if(loot == 0)
	{
		// Only tell the player when they actually asked for this item.
		//
		// The reservation this rejects is normal and extremely common: every drop
		// belongs to its owner (and their party) until m_LootTime expires. The
		// auto-pickup scan in CCustomPick::ItemGet walks every nearby drop on a
		// timer and calls straight through here, so a single reserved item on the
		// ground produced a stream of "This item does not belong to you" for as
		// long as the player stood near it - with no click, on PC and mobile
		// alike. The check itself was right; announcing it was not.
		//
		// Even then, at most once every few seconds. An equipped Muun
		// collector re-requests the same reserved drop roughly once a second
		// while it keeps targeting it, down the identical pickup packet a
		// manual click uses - indistinguishable from here - so telling the
		// player every time filled their chat with the same line six or more
		// times over. Once is information; the rest is noise.
		if(notify != 0)
		{
			const DWORD now = GetTickCount();

			if((now - lpObj->m_LootRefuseNoticeTick) > LOOT_REFUSE_NOTICE_DELAY)
			{
				lpObj->m_LootRefuseNoticeTick = now;
...
void CMap::ItemGive(int aIndex,int index) // OK
{
	...
	if(gServerInfo.m_ItemDropLog != 0)
	{
		LogAdd(LOG_BLUE,"[ItemLog] PICKUP map=%d slot=%d serial=%u item=%d(%s) by=%d",
			this->m_MapNumber,index,this->m_Item[index].m_Serial,
			this->m_Item[index].m_Index,gItemManager.GetItemName(this->m_Item[index].m_Index),aIndex);
	}
		...
		switch(this->m_Item[n].m_State)
		{
			...
			default:
				if((GetTickCount() > this->m_Item[n].m_Time))
				{
					...
					{
						// Lifetime elapsed. This transition is what
						// DestroyViewportItem turns into a delete packet on the
						// next viewport tick, so if a client is still showing
						// this item afterwards, that delete never reached it.
						if(gServerInfo.m_ItemDropLog != 0)
						{
							LogAdd(LOG_BLUE,"[ItemLog] EXPIRE map=%d slot=%d serial=%u pos=%d,%d item=%d(%s)",
								this->m_MapNumber,n,this->m_Item[n].m_Serial,
								this->m_Item[n].m_X,this->m_Item[n].m_Y,
								this->m_Item[n].m_Index,gItemManager.GetItemName(this->m_Item[n].m_Index));
						}
