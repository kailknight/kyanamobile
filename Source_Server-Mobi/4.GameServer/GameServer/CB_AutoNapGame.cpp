...
void CBAutoNapGame::SendNapTienBank(BYTE * recv, int aIndex)
{
	...
	{
		gNotice.GCNoticeSend(aIndex, 255, 0, 0, 0, 0, 0, gMessage.GetMessage(3091), this->GiaTriNapThapNhat); //"Vui lòng nạp ít nhất %d"
		return;
	}
	...
	gNotice.GCNoticeSend(aIndex, 255, 0, 0, 0, 0, 0, gMessage.GetMessage(3092), pMsg.TienNap); //"Bạn vừa tạo lệnh nạp với giá trị %d VND"
