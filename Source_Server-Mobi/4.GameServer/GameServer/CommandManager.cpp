...
bool CCommandManager::ManagementCore(LPOBJ lpObj,char* message, int Npc) // OK
{
	...
	switch(code)
	{
		...
		case COMMAND_SETPASSPARTY:
			...
			break;
		case COMMAND_JOINPARTY:
			Result = CommandJoinParty(lpObj, argument);
			break;
		...
		case COMMAND_SETSTATS:
			Result = CommandSetStats(lpObj, argument);
			break;
		case COMMAND_SETML:
			Result = CommandSetMasterLevel(lpObj, argument);
			break;
		default:
			return 0;
	}
...
bool CCommandManager::CommandSetPassParty(LPOBJ lpObj, char* arg) // OK
{
	...
	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3095), lpObj->AutoPartyPassword); //"[SetPass] Thành Công, Pass : %s"
...
/*
	/join <pass> - join the party of whoever opened one with /setpass <pass>.

	Replaces having to whisper the password at the leader. The whisper route still
	works and now shares this one's wiring (CParty::JoinPartyByPassword).

	Unlike the whisper, /join names no target, so the host has to be found by the
	password alone. Only players who actually opened a party count: auto-party bit
	set AND a non-empty password. The non-empty test is load bearing - without it
	a bare "/join" would match every player whose password is blank, i.e. almost
	everyone, and drop the caller into a stranger's party.
*/
bool CCommandManager::CommandJoinParty(LPOBJ lpObj, char* arg) // OK
{
	char szPassword[sizeof(lpObj->AutoPartyPassword)] = {0};

	this->GetString(arg, szPassword, sizeof(szPassword), 0);

	if (szPassword[0] == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3124)); // usage
		return 1;
	}
	...
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		{
			continue;	// auto-party off, so this player never opened one
		}

		if (lpTarget->AutoPartyPassword[0] == 0)
		{
			continue;	// see the comment above - blank must never match
		}

		if (strcmp(lpTarget->AutoPartyPassword, szPassword) != 0)
		{
			continue;
		}

		/*
			Two hosts can pick the same password. Prefer one who already has a
			party going, so a late joiner lands in the existing group instead of
			starting a fresh two-man party with whoever happened to be scanned
			first - which would silently split the raid in half.
		*/
		if (lpHost == NULL)
		{
			lpHost = lpTarget;
		}
		else if (gParty.IsParty(lpTarget->PartyNumber) != 0 && gParty.IsParty(lpHost->PartyNumber) == 0)
		{
			lpHost = lpTarget;
		}
	}
	...
	if (lpHost == NULL)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3125)); // no such party
		return 1;
	}

	gParty.JoinPartyByPassword(lpObj, lpHost);
...
// Update Giết người mất zen tính theo mạng đạt thêm 24/12/2024
	...
	if(lpObj->PKCount >=1)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(3096),Count); //"Bạn không đủ %d Zen để rửa tội"
			return 0;
		}
		else
		{
...
bool CCommandManager::CommandReset(LPOBJ lpObj,char* arg,int Npc) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3097)); //"Bạn đã đạt GHRS hãy Relife để có thể tiếp tục Reset!"
...
{
	gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(3052),lpObj->Name,arg);
...
void CCommandManager::CommandResetAutoProc(LPOBJ lpObj) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3098)); //"Bạn đã đạt GHRS hôm nay!!"
		lpObj->AutoResetEnable = 0;
...
void CCommandManager::DGCommandRenameRecv(SDHP_COMMAND_RENAME_RECV* lpMsg) // OK
{
	...
	if (lpMsg->result == 0)
	{
		...
		LogAddCat(LOG_CAT_COMMAND,LOG_GREEN,"[CommandRename][%s][%s] - Sucess (OldName: %s)",lpObj->Account,lpObj->Name,lpMsg->name);
		return;
	}
	...
	if (lpMsg->result == 1)
	{
		...
		LogAddCat(LOG_CAT_COMMAND,LOG_BLACK,"[CommandRename][%s][%s] - Name exists (NewName: %s)",lpObj->Account,lpObj->Name,lpMsg->newname);
		return;
	}
	...
	if (lpMsg->result == 2)
	{
		...
		LogAddCat(LOG_CAT_COMMAND,LOG_RED,"[CommandRename][%s][%s] - Error (NewName: %s)",lpObj->Account,lpObj->Name,lpMsg->newname);
		return;
	}
...
bool CCommandManager::CommandInfo(LPOBJ lpObj,char* arg) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(3053),lpTarget->Account);
	}

	GJAccountLevelSend(lpObj->Index, lpTarget->Account);
	gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(3054),lpTarget->Level);
	gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(3055),lpTarget->Reset);
	gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(3056),lpTarget->MasterReset);
#endif
	return 1;
}
...
void CCommandManager::DGCommandGiftRecv(SDHP_GIFT_RECV* lpMsg)
{
	...
	if (lpMsg->result > 0)
	{
		...
		else 
		{
			...
			LogAddCat(LOG_CAT_COMMAND,LOG_GREEN,"[CustomGift][%s][%s] GiftNumber: %d",lpObj->Account,lpObj->Name, lpMsg->result);
	...
	{
		LogAddCat(LOG_CAT_COMMAND,LOG_RED,"[CustomGift][%s][%s] Gift Error",lpObj->Account,lpObj->Name);
	}
...
void CCommandManager::DGCommandTopRecv(SDHP_TOP_RECV* lpMsg)
{
	...
	if(gCustomTop.GetInfo(lpMsg->type,&CustomTopInfo) != 0)
	{
		if (lpMsg->classe == 0)
		{
			gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(3057),CustomTopInfo.Title);
		}
		else if (lpMsg->classe == 16)
		{
			gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(3058),CustomTopInfo.Title);
		}
		else if (lpMsg->classe == 32)
		{
			gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(3059),CustomTopInfo.Title);
		}
		else if (lpMsg->classe == 48)
		{
			gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(3060),CustomTopInfo.Title);
		}
		else if (lpMsg->classe == 64)
		{
			gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(3061),CustomTopInfo.Title);
		}
		else if (lpMsg->classe == 80)
		{
			gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(3062),CustomTopInfo.Title);
		}
		else if (lpMsg->classe == 96)
		{
			gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(3063),CustomTopInfo.Title);
		}
		else
		{
...
bool CCommandManager::CommandSetLevel(LPOBJ lpObj, char* arg) // OK
{
	...
	if (lpTarget == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3099)); //"Nhân vật không tồn tại hoặc không trực tuyến!"
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3100)); //"Vượt quá mức cho phép"
		return 0;
	}
...
bool CCommandManager::CommandSetReset(LPOBJ lpObj, char* arg) // OK
{
	...
	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3101), Reset, lpTarget->Reset); //"Đã cộng %d lần reset. Bạn có %d lần Reset!"
	gLog.Output(LOG_COMMAND, "[CommandSetReset] Account:[%s] Name:[%s] - (Name:%s, Reset: %d)", lpObj->Account, lpObj->Name, name, Reset);
}

// /setstats <charname> <str> <agi> <vit> <ene> <cmd>
// Sets the target's stats outright rather than adding to them, and does not
// touch LevelUpPoint - this is a GM tool, not the player-facing /addstr path.
bool CCommandManager::CommandSetStats(LPOBJ lpObj, char* arg) // OK
{
	char name[11] = { 0 };
	...
	if (name[0] == '\0')
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Usage: /setstats <name> <str> <agi> <vit> <ene> <cmd>");
		return 0;
	}
	...
	if (lpTarget == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Character does not exist or is offline!");
		return 0;
	}

	// Stats travel to the client as WORD in the character info packet, so
	// anything above 32767 would wrap and show as nonsense.
	const int MAX_STAT_VALUE = 32767;

	int stats[5] = { 0, 0, 0, 0, 0 };

	for (int n = 0; n < 5; n++)
	{
		stats[n] = this->GetNumber(arg, n + 1);

		if (stats[n] < 0 || stats[n] > MAX_STAT_VALUE)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Each stat must be between 0 and %d!", MAX_STAT_VALUE);
			return 0;
		}
	}

	lpTarget->Strength = stats[0];
	lpTarget->Dexterity = stats[1];
	lpTarget->Vitality = stats[2];
	lpTarget->Energy = stats[3];
	lpTarget->Leadership = stats[4];

	// Recalculate before syncing: damage, defence and max life/mana are all
	// derived from these, so without this the client would show new stats with
	// the old totals until something else forced a recalc.
	gObjectManager.CharacterCalcAttribute(lpTarget->Index);

	lpTarget->Life = lpTarget->MaxLife;
	lpTarget->Mana = lpTarget->MaxMana;

	GCNewCharacterInfoSend(lpTarget);
	GDCharacterInfoSaveSend(lpTarget->Index);

	gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0,
		"Your stats were set to STR:%d AGI:%d VIT:%d ENE:%d CMD:%d",
		stats[0], stats[1], stats[2], stats[3], stats[4]);

	if (lpTarget->Index != lpObj->Index)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,
			"%s stats set to STR:%d AGI:%d VIT:%d ENE:%d CMD:%d",
			lpTarget->Name, stats[0], stats[1], stats[2], stats[3], stats[4]);
	}

	gLog.Output(LOG_COMMAND, "[CommandSetStats] Account:[%s] Name:[%s] - (Name:%s, STR:%d, AGI:%d, VIT:%d, ENE:%d, CMD:%d)",
		lpObj->Account, lpObj->Name, lpTarget->Name, stats[0], stats[1], stats[2], stats[3], stats[4]);

	return 1;
}

// /setML <charname> <masterlevel>
// Sets master level and master points to the same value, the way the request
// was specified - note that the normal level-up path awards
// MasterSkillTreePoint per level, so this is not the same arithmetic.
bool CCommandManager::CommandSetMasterLevel(LPOBJ lpObj, char* arg) // OK
{
	...
	if (name[0] == '\0')
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Usage: /setML <name> <masterlevel>");
		return 0;
	}

	if (gServerInfo.m_MasterSkillTree == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Master Skill Tree is disabled on this server!");
		return 0;
	}
	...
	// The master level save packet is dropped unless the character's master
	// data has already come back from the DataServer. Without this check the
	// change would apply on screen and then vanish on relog.
	if (lpTarget->LoadMasterLevel == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "%s master data is still loading, try again in a moment!", lpTarget->Name);
		return 0;
	}

	int MasterLevel = this->GetNumber(arg, 1);

	// The client is told the maximum in the same packet, so going past the
	// configured cap would leave its master window showing an impossible
	// level. Raise MasterSkillTreeMaxLevel if a higher value is wanted.
	int MaxMasterLevel = gServerInfo.m_MasterSkillTreeMaxLevel;

	MaxMasterLevel = ((MaxMasterLevel > MAX_CHARACTER_MASTER_LEVEL) ? MAX_CHARACTER_MASTER_LEVEL : MaxMasterLevel);

	if (MasterLevel < 0 || MasterLevel > MaxMasterLevel)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Master level must be between 0 and %d!", MaxMasterLevel);
		return 0;
	}

	lpTarget->MasterLevel = MasterLevel;

	lpTarget->MasterPoint = MasterLevel;

	// Drop the experience onto the exact threshold for the new level first.
	// CalcMasterLevelNextExperience only ever raises it, so lowering someone's
	// master level without this would leave the old, larger total in place and
	// the experience bar past its own maximum.
	lpTarget->MasterExperience = gMasterSkillTree.GetMasterLevelExpTlbInfo(MasterLevel);
	...
	// Info carries the experience bar, LevelUp carries the new maximum and the
	// level up effect. The normal path sends only the second because the first
	// is implied by gradual gain; a jump needs both.
	gMasterSkillTree.GCMasterInfoSend(lpTarget);

	gMasterSkillTree.GCMasterLevelUpSend(lpTarget);

	gMasterSkillTree.GDMasterSkillTreeSaveSend(lpTarget->Index);

	gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, "Your master level was set to %d (%d master points)", MasterLevel, MasterLevel);

	if (lpTarget->Index != lpObj->Index)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "%s master level set to %d (%d master points)", lpTarget->Name, MasterLevel, MasterLevel);
	}

	gLog.Output(LOG_COMMAND, "[CommandSetMasterLevel] Account:[%s] Name:[%s] - (Name:%s, MasterLevel:%d, MasterPoint:%d)",
		lpObj->Account, lpObj->Name, lpTarget->Name, MasterLevel, MasterLevel);

	return 1;

#else

	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Master Skill Tree is not supported by this server version!");
	return 0;
