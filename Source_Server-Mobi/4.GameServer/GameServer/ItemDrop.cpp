...
int CItemDrop::GetItemDropRate(LPOBJ lpObj,LPOBJ lpTarget,int ItemIndex,int ItemLevel,int DropRate) // OK
{
	...
	int Rate = gBonusManager.GetBonusValue(lpTarget,BONUS_INDEX_CMN_ITEM_DROP_RATE,DropRate,ItemIndex,ItemLevel,lpObj->Class,lpObj->Level);

	// Permanent VIP-tier bonus, on top of any active scheduled event bonus above.
	int VIPBonus = gServerInfo.m_ItemDropRateVIPBonus[lpTarget->AccountLevel];

	if(VIPBonus > 0)
	{
		Rate += (Rate*VIPBonus)/100;
