...
CChaosBox::CChaosBox() // OK
{
	...
	this->m_TalismanMixResult = CHAOS_MIX_RESULT_TALISMAN_OVER_LIMIT;
}
...
bool CChaosBox::IsTalismanOfLuckItem(int ItemIndex) // OK
{
	return (ItemIndex == GET_ITEM(14,53)      // Talisman of Luck
		||  ItemIndex == GET_ITEM(14,189));   // Elemental Talisman of Luck
}

// Two different numbers, and conflating them is what made MaxTalismanOfLuck
// behave nothing like its name.
//
//   outCount - how many talismans are in the box. One per slot: the Talisman
//              of Luck has no ItemStack.txt entry, so it does not stack and a
//              slot never holds more than one.
//   outRate  - what they add to the success rate, which is the sum of their
//              m_Durability. That is the server's existing balance and is left
//              exactly as it was.
//
// The old code used the durability sum for BOTH. With 5 durability a piece
// that made "MaxTalismanOfLuck = 10" mean five durability-points short of
// three talismans, so the third was refused and the setting looked broken.
static void GetChaosBoxTalismanCount(LPOBJ lpObj,int ItemIndex,int* outCount,int* outRate)
{
	(*outCount) = 0;
	(*outRate) = 0;
	...
	for(int n=0;n < CHAOS_BOX_SIZE;n++)
	{
		...
		if(lpObj->ChaosBox[n].m_Index == ItemIndex)
		{
			(*outCount) += 1;
			(*outRate) += (int)lpObj->ChaosBox[n].m_Durability;
		}
	}
}

// Adds the Talisman of Luck bonus to *rate and says whether the mix may go
// ahead. Three ways it can refuse, all of which used to be one silent code:
//
//  - a talisman is in the box for a mix that is not the Regular Combination,
//  - more talismans than MaxTalismanOfLuck allows,
//  - (unchanged) nothing at all, which is not a refusal.
//
// Callers add the returned bonus to a rate that must ALREADY hold the base
// success rate. CustomItemMix used to call this first and then assign the
// base rate over the top, which threw the bonus away - the talismans were
// still taken, they just did nothing.
bool CChaosBox::GetTalismanOfLuckRate(LPOBJ lpObj,int* rate,bool bTalismanAllowed) // OK
{
	int count = 0;
	int rateBonus = 0;

	GetChaosBoxTalismanCount(lpObj,GET_ITEM(14,53),&count,&rateBonus); // Talisman of Luck
	...
	if(bTalismanAllowed == false)
	{
		this->m_TalismanMixResult = CHAOS_MIX_RESULT_TALISMAN_NOT_ALLOWED;
		return 0;
	}

	if(count > gServerInfo.m_MaxTalismanOfLuck)
	{
		this->m_TalismanMixResult = CHAOS_MIX_RESULT_TALISMAN_OVER_LIMIT;
		return 0;
	}

	// The durability sum, not the talisman count - see
	// GetChaosBoxTalismanCount. Unchanged from how this server has always
	// scored a talisman; only the LIMIT above counts talismans.
	(*rate) += rateBonus;

	return 1;
}

bool CChaosBox::GetElementalTalismanOfLuckRate(LPOBJ lpObj,int* rate,bool bTalismanAllowed) // OK
{
	int count = 0;
	int rateBonus = 0;

	GetChaosBoxTalismanCount(lpObj,GET_ITEM(14,189),&count,&rateBonus); // Elemental Talisman of Luck
...
void CChaosBox::ChaosItemMix(LPOBJ lpObj) // OK
{
	...
	{
		this->GCChaosMixSend(lpObj->Index,this->m_TalismanMixResult,0);
		return;
	}
...
void CChaosBox::PlusItemLevelMix(LPOBJ lpObj,int type) // OK
{
	...
	// SoulSuccessRate_ALx (GameServerInfo - Common.ini): flat per-account-level
	// bonus documented in CustomConfig.ini's own header comment ("item-upgrade
	// rate also gets a bonus luck % configured in Common") as applying to every
	// tier of this mix. It was being loaded (ServerInfo.cpp) and used by the
	// unrelated direct Jewel-of-Soul apply (ObjectManager.cpp, for +0..+8) but
	// never added here - so it affected neither the rate shown to the player
	// nor the roll actually used when the Chaos Machine mix executes (both
	// come from this same ChaosSuccessRate value, set once here and read again
	// down at the real roll below).
	lpObj->ChaosSuccessRate += gServerInfo.m_SoulSuccessRate[lpObj->AccountLevel];
	...
	if (lpObj->ChaosLockShowRate != 100) //CB_GETMIXRATE
	{
		// true: the item-upgrade mix takes a talisman as well as the Regular
		// Combination - this is the Talisman of Luck's original job, boosting
		// a +10/+11/+12/+13 attempt. Ordinary items take the Talisman of Luck,
		// Pentagram items the Elemental one, hence the IsPentagramItem split.
		//
		// The rate arithmetic here was already in the right order: the base
		// tier rate, SoulSuccessRate and the item's own luck bonus are all
		// added above, the talisman goes on top, and the clamp to 100 follows.
		if (lpObj->ChaosBox[ItemSlot].IsPentagramItem() == 0 && this->GetTalismanOfLuckRate(lpObj, &lpObj->ChaosSuccessRate, true) == 0)
		{
			this->GCChaosMixSend(lpObj->Index,this->m_TalismanMixResult,0);
			return;
		}

		if (lpObj->ChaosBox[ItemSlot].IsPentagramItem() != 0 && this->GetElementalTalismanOfLuckRate(lpObj, &lpObj->ChaosSuccessRate, true) == 0)
		{
			this->GCChaosMixSend(lpObj->Index,this->m_TalismanMixResult,0);
			return;
		}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, eMessageBox, 0, 0, 0, 0, 0, gMessage.GetMessage(3074), gServerInfo.m_TatQuayDo);
		// NOT m_TalismanMixResult: this is the DisableItemUpgradeAbove level
		// cap, nothing to do with talismans - the message box above already
		// tells the player why. This code only closes the mix window, which is
		// what the 240 that used to be here did back when nothing handled it.
		this->GCChaosMixSend(lpObj->Index,CHAOS_MIX_RESULT_CLOSE_SILENT,0);
		return;
	}
	...
	else
	{
		...
		char buff2[256];
		wsprintf(buff2, gMessage.GetMessage(3072), lpObj->Name, gItemManager.GetItemName(item.m_Index));
...
void CChaosBox::DinorantMix(LPOBJ lpObj) // OK
{
	...
	if (lpObj->ChaosLockShowRate != 100) //CB_GETMIXRATE
	{
		...
		if (this->GetTalismanOfLuckRate(lpObj, &lpObj->ChaosSuccessRate,true) == 0)
		{
			this->GCChaosMixSend(lpObj->Index,this->m_TalismanMixResult,0);
			return;
		}
...
void CChaosBox::BrokenHornMix(LPOBJ lpObj) // OK
{
	...
	if((GetLargeRand()%100) < lpObj->ChaosSuccessRate)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3047), lpObj->Name);
...
void CChaosBox::HornOfFenrirMix(LPOBJ lpObj) // OK
{
	...
	if((GetLargeRand()%100) < lpObj->ChaosSuccessRate)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3048), lpObj->Name);
...
void CChaosBox::HornOfFenrirUpgradeMix(LPOBJ lpObj) // OK
{
	...
	if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3049), lpObj->Name);
...
void CChaosBox::HornOfFenrirGoldMix(LPOBJ lpObj) // OK
{
	...
	if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3050), lpObj->Name);
	...
	else
	{
		...
		this->GCChaosBoxSend(lpObj, 0);
		this->GCChaosMixSend(lpObj->Index, 0, 0);
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3051), lpObj->Name);
...
void CChaosBox::JewelOfHarmonyItemSmeltMix(LPOBJ lpObj) // OK
{
	...
	if (lpObj->ChaosLockShowRate != 100) //CB_GETMIXRATE
	{
		if (ItemCount != 1)
		{
			gNotice.GCNoticeSend(lpObj->Index, eMessageBox, 0, 0, 0, 0, 0, gMessage.GetMessage(3075));
			this->GCChaosMixSend(lpObj->Index, 7, 0);
			return;
		}
	}
...
void CChaosBox::CustomItemMix(LPOBJ lpObj, int HeadCode) // OK
{
	...
	if (CustomMix != 0)
	{
		...
		// Base rate FIRST, then the talisman bonus on top. The other way round
		// - which is how this read - assigned the base rate straight over
		// whatever the talismans had just added, so every Talisman of Luck in
		// a Regular Combination was consumed for nothing.
		lpObj->ChaosSuccessRate = CustomMix->MixRate[lpObj->AccountLevel];

		// The one mix that accepts a talisman at all, hence the true.
		if (this->GetTalismanOfLuckRate(lpObj, &lpObj->ChaosSuccessRate, true) == 0)
		{
		...
		if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
		{
			...
			wsprintf(thongbaomix, gMessage.GetMessage(3073), lpObj->Name, ItemInfo.Name);
...
void CChaosBox::PentagramJewelUpgradeLevelMix(LPOBJ lpObj,BYTE info) // OK
{
	...
	if(this->GetElementalTalismanOfLuckRate(lpObj,&lpObj->ChaosSuccessRate,true) == 0)
	{
		this->GCChaosMixSend(lpObj->Index,this->m_TalismanMixResult,0);
		return;
	}
...
void CChaosBox::CGChaosMixRecv(PMSG_CHAOS_MIX_RECV* lpMsg, int aIndex) // OK
{
	...
	// OutputNoMirror, not Output: this is a per-slot forensic dump of the whole
	// chaos box on every mix press - 2 framing lines plus up to CHAOS_BOX_SIZE
	// item lines. Valuable on disk as an item-dupe trail, but it would turn the
	// Chaos Mix log view into a firehose (a handful of grinders wraps the ring
	// in minutes) and bury the one line per mix that an operator wants to read.
	gLog.OutputNoMirror(LOG_CHAOS_MIX, "[CHECK ITEM GOBIN [%s]-[%s]", lpObj->Account, lpObj->Name);
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		if (lpObj->ChaosBox[n].IsItem() != 0)
		{
			gLog.OutputNoMirror(LOG_CHAOS_MIX, "[Item] - [%s +%d] - [DB[%.0f]|Skill[%d] | Luck[%d], Op[%d]] Exl[%d] SET[%d] oEx[%d] Harmony[%d] Socket[%03d,%03d,%03d,%03d,%03d][%d] Seri[%08X]",
	...
	gLog.OutputNoMirror(LOG_CHAOS_MIX, "[==============[%s][%s]===========", lpObj->Account, lpObj->Name);

	LogAddCat(LOG_CAT_CHAOSMIX, LOG_RED, "CGChaosMixRecv Type %d %d Cs(%d)", HeadCodetype, lpMsg->info, lpObj->CustomIndexMix);
...
Jump:
	...
	// Remembered so a config reload (Reload ChaosMix/Common/All) can push a
	// fresh rate to whoever has this box open - see RefreshAllOpenMixRates()
	// below - without the client needing to re-ask, which it only does when
	// the box's item contents actually change (CMixRecipeMgr::CheckMixInventory,
	// client-side). Before this, changing a rate mid-session only reached
	// players who closed and reopened the Chaos Machine.
	lpObj->ChaosLastMixID = ChaosMixId;
	lpObj->ChaosLastMixInfo = ChaosMixInfo;

	this->RecalcAndSendMixRate(lpObj, ChaosMixId, ChaosMixInfo);
}

void CChaosBox::RecalcAndSendMixRate(LPOBJ lpObj, int ChaosMixId, int ChaosMixInfo)
{
...
void CChaosBox::RefreshAllOpenMixRates() // OK
{
	// Called after Reload ChaosMix / Reload Common / Reload All so a rate
	// change reaches players who already have the Chaos Machine open, rather
	// than only the next player who opens it (or an already-open one who
	// changes the box's contents, since that's the only other thing that
	// makes the client re-ask - CB_GetMixRate.cpp, SetInfoMixID).
	if (gServerInfo.m_UseZenRateReal == 0)
	{
		return;
	}
	...
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		...
		// 0 means the box has never held a recognized recipe this session
		// (client only sends CGSendMixInfo once GetCurMixID() is nonzero) -
		// nothing to refresh. ChaosLockShowRate != 0 means a request from
		// this same player is already being processed right now; skip
		// rather than re-enter mid-computation.
		if (lpObj->ChaosLastMixID == 0 || lpObj->ChaosLockShowRate != 0)
		{
			continue;
		}

		this->RecalcAndSendMixRate(lpObj, lpObj->ChaosLastMixID, lpObj->ChaosLastMixInfo);
	}
}
