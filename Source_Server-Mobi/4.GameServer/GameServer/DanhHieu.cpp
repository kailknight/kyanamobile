...
void DanhHieu::BUY_DANH_HIEU(LPOBJ lpObj, BUY_DANH_HIEU_REQ* aRecv)
{
	if (aRecv->Number == 1)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, eMessageBox, 0, 0, 0, 0, 0, gMessage.GetMessage(3112)); //"Bạn không thể sử dụng nó ngay bây giờ."
			return;
		}
		// Delay Khi Click
		if ((GetTickCount() - lpObj->ClickClientSend) < 500)
		{
			gNotice.GCNoticeSend(lpObj->Index, eMessageBox, 0, 0, 0, 0, 0, gMessage.GetMessage(3113)); //"Vui Lòng thao tác chậm lại"
			return;
		}
		//---------------------------------------------------------
		if (true)
		{
			...
			{
				gNotice.GCNoticeSend(lpObj->Index, eMessageBox, 0, 0, 0, 0, 0, gMessage.GetMessage(3114)); //"Bạn đã đạt cấp độ tối đa"
				return;
			}
			...
			{
				gNotice.GCNoticeSend(lpObj->Index, eMessageBox, 0, 0, 0, 0, 0, gMessage.GetMessage(3115), GetCount(lpObj), gItemManager.GetItemName(GET_ITEM(GetType(lpObj), GetIndex(lpObj)))); //"Quay Lại Khi Bạn Có Đủ [%d] [%s]!(Không Tính Cụm)"
				return;
			}

			if (lpObj->Coin1 < GetCoint(lpObj))
			{
				gNotice.GCNoticeSend(lpObj->Index, eMessageBox, 0, 0, 0, 0, 0, gMessage.GetMessage(3116), GetCoint(lpObj)); //"Bạn Không đủ %d WcoinC để nâng cấp!!!"
				return;
			}
