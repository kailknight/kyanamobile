...
void CMemoryAllocatorInfo::Alloc() // OK
{
	...
	this->m_VpPlayerItemSerial = new DWORD[MAX_VIEWPORT];
	...
	if(OBJECT_USER_RANGE(this->m_Index) != 0)
	{
		...
		#endif
		// Must stay INSIDE this user-range guard: the pool is [MAX_OBJECT]
		// (10000) but only the ~1000 user indices need a claim box. Allocating
		// outside it would waste ~19 MB on monster slots.
		this->m_SpinClaim = new CItem[SPIN_CLAIM_SIZE];
#if(HAISLOTRING) //ZEUS UP20
