...
void CustomNoticeDrop::NoticeItem(LPOBJ lpObj, CItem* lpItem)
{
	...
	// ItemPickupNotice in CustomConfig.ini - master switch, separate from the
	// Enable column in Custom\CustomNoticeItem.txt above.
	if (gServerInfo.m_ItemPickupNotice == 0)
	{
		return;
	}
