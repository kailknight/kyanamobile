...
MONSTER_ITEM_INFO* CMonsterManager::GetMonsterItemInfo(int level) // OK
{
	...
	return &this->m_MonsterItemInfo[level];
}
...
void CMonsterManager::SetMonsterData() // OK
{
	...
	// Two passes on purpose. The object pool (MAX_OBJECT_MONSTER) is smaller
	// than what MonsterSetBase asks for, and gObjAddMonster simply starts
	// failing once it is exhausted - so whatever is spawned last is what goes
	// missing. Entries sit in the table in folder-scan order, and
	// FindFirstFile enumerates subdirectories after files, which puts all of
	// MonsterSetBase\Event\ and \Invasion\ at the very end and made their NPCs
	// the first casualties. That is why the Blood Castle Archangel reappeared
	// simply by moving its file up out of Event\ - nothing about the file was
	// wrong, it was just last in line.
	//
	// The fixed-position sections (0 and 5) are where the NPCs and bosses
	// live, and there are only a few hundred of them; sections 1 and 2 are the
	// bulk field spawns. Taking the fixed ones first means a short pool
	// degrades by thinning out field monsters, which is graceful, instead of
	// dropping gate/shop/event NPCs, which is not.
	...
	for(int n=0;n < gMonsterSetBase.m_count;n++)
	{
		...
		bool fixed = (lpInfo->Type == 0 || lpInfo->Type == 5);
		...
		{
			// Almost always MAX_OBJECT_MONSTER being exhausted rather than a
			// per-map rejection, and it is otherwise completely silent - the
			// monsters simply are not there and nothing says why.
			noslot++;
			continue;
		}
	...
	LogAdd(LOG_BLUE,"[MonsterManager] Spawned %d of %d monsters (MAX_OBJECT_MONSTER %d)",(wanted-noslot),wanted,MAX_OBJECT_MONSTER);

	if(noslot != 0)
	{
		LogAdd(LOG_RED,"[MonsterManager] %d monsters had no object slot - raise MAX_OBJECT_MONSTER/MAX_OBJECT or trim MonsterSetBase",noslot);
	}
