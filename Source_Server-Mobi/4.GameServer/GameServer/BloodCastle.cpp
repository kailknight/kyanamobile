...
void CBloodCastle::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		// ZzzObject.cpp's CreateItem() picks this item's ground model via
		// (n->Level>>3): 0=Staff,1=Sword,2=Bow. Shift the tier into those
		// bits here - passing the raw 0/1/2 tier left every Saint Statue
		// reward showing the same Staff skin regardless of tier.
		GDCreateItemSend(lpTarget->Index,lpObj->Map,(BYTE)lpObj->X,(BYTE)lpObj->Y,GET_ITEM(13,19),((lpObj->Class-132)<<3),0,0,0,0,lpTarget->Index,0,0,0,0,0,0xFF,0);
	...
	if (gServerInfo.m_BCVIP == 1)
	{
		...
		{
			lpTarget->ScoreKill++;
			gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1000), lpTarget->ScoreKill, gServerInfo.m_DiemBlood);
			if (lpTarget->ScoreKill == gServerInfo.m_DiemBlood)
			{
				...
				this->NoticeSendToAll(&this->m_BloodCastleLevel[level], 0, gMessage.GetMessage(1001), lpTarget->Name, lpTarget->ScoreKill);
				return;
			}
			if (lpTarget->ScoreKill > gServerInfo.m_DiemBlood)
			{
				lpTarget->ScoreKill = 0;
				//Sleep(500);
			}
		}
	}
