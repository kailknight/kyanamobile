...
void CMemoryAllocator::BindMemoryAllocatorInfo(int index,CMemoryAllocatorInfo MemoryAllocatorInfo) // OK
{
	...
	lpObj->VpPlayerItemSerial = MemoryAllocatorInfo.m_VpPlayerItemSerial;
	...
	if(OBJECT_USER_RANGE(index) != 0)
	{
		...
		#endif
		// Only bound inside this user-range guard, so SpinClaim stays NULL for
		// monsters and every other non-user object. Anything that touches it must
		// check Type != OBJECT_USER first or it null-derefs on the first monster.
		lpObj->SpinClaim = MemoryAllocatorInfo.m_SpinClaim;
