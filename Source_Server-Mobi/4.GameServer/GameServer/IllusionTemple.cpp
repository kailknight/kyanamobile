...
void CIllusionTemple::GCIllusionTempleRewardScoreSend(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	// Stays LOG_EVENT (it is a real event outcome and one of the few classic
	// events that logs anything live on this build), but reworded - it now
	// appears in Logs > Event Logs, where "[1] winner 2, point 1 3" was
	// unreadable.
	LogAdd(LOG_EVENT,"[IllusionTemple] Finished - winner team %d (Score Y:%d B:%d)",lpLevel->WinnerTeam,lpLevel->Score[IT_TEAM_Y],lpLevel->Score[IT_TEAM_B]);
