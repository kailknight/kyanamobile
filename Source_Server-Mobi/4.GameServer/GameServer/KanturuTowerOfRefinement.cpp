...
// Seconds left before the whole Tower of Refinement phase ends and the Kanturu
// cycle drops back to standby, not just the current sub-state.
int CKanturuTowerOfRefinement::GetPhaseRemainTime() // OK
{
	int state = this->GetTowerOfRefinementState();

	if(state == KANTURU_TOWER_OF_REFINEMENT_END)
	{
		return 0;
	}

	int remain = 0;
	int n = KANTURU_TOWER_OF_REFINEMENT_REVITALIZATION;

	if(state != KANTURU_TOWER_OF_REFINEMENT_NONE)
	{
		remain = this->m_StateInfo[state].GetRemainTime();
		n = (state+1);
	}

	for(;n < KANTURU_TOWER_OF_REFINEMENT_END;n++)
	{
		if(this->m_StateInfo[n].GetCondition() == 1)
		{
			remain += (this->m_StateInfo[n].GetValue()/1000);
		}
	}
