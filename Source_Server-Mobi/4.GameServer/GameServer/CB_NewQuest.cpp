...
void CB_NewQuest::RecvProtocol(int aIndex,BYTE* Recv)
{
	...
	if (lpObj->NQuest_Status == eDaDuYeuCau)
	{
		...
		gNotice.GCNoticeSend(lpObj->Index, eMessageBox, 0, 0, 0, 0, 0, gMessage.GetMessage(3093)); //"Chúc mừng bạn đã nhận thưởng Nhiệm Vụ, hãy cố gắng nhé !"
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, eMessageBox, 0, 0, 0, 0, 0, gMessage.GetMessage(3094)); //"Vui lòng hoàn thành yêu cầu của Nhiệm Vụ !!"
	}
