...
// Feeds the "Crywolf" row of the in-game event schedule window (H).
//
// Called from every GameServer, not just the CS one: MainProc (and with it the
// whole state machine) is compiled in only for GAMESERVER_TYPE==1, but the
// schedule in Crywolf.dat is static data every sub-server loads, so each can
// work out the next start time for itself. Only "is it running right now" has
// to come off the wire, via DGCrywolfSyncRecv.
void CCrywolf::UpdateEventTimeDisplay() // OK
{
	if(gServerInfo.m_CrywolfEvent == 0 || this->m_StartTimeInfoCount == 0 || this->m_StartTimeInfo[0].m_Used == 0)
	{
		gServerDisplayer.EventCryWolf = -1;
		return;
	}

	int state = ((this->m_CrywolfState!=CRYWOLF_STATE_NONE)?this->m_CrywolfState:gCrywolfSync.GetCrywolfState());

	if(state != CRYWOLF_STATE_NONE)
	{
	...
	{
		// start time has come and gone - roll on to the next occurrence
		this->CheckSync();
	}

	// never report 0 while the event is not actually running - the client
	// renders 0 as "Online"
	gServerDisplayer.EventCryWolf = ((this->RemainTime<=0)?1:this->RemainTime);
}
