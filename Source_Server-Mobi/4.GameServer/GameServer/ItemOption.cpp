...
void CItemOption::CalcItemCommonOption(LPOBJ lpObj, bool flag) // OK
{
	...
	for (int n = 0; n < EQUIP_INVENTORY_SIZE; n++)
	{
		...
		{
			this->InsertOption(lpObj, &lpObj->EquipInventory[n], flag);
			this->InsertOptionBEX(lpObj, &lpObj->EquipInventory[n], flag);
		}
	}
#endif
}
...
void CItemOption::LoadBEX(char* FilePath) // OK
{
	...
	for (pugi::xml_node List = oItemOptionInfoBEX.child("List"); List; List = List.next_sibling())
	{
		...
		bInfo.OptionIndex = List.attribute("OptionIndex").as_int();
		bInfo.OptionValue = List.attribute("OptionValue").as_int();
...
void CItemOption::InsertOptionBEX(LPOBJ lpObj, CItem* lpItem, bool flag) // OK
{
	...
	for (std::vector<ITEM_OPTION_BEX>::iterator it = this->m_ItemOptionInfoBEX.begin(); it != this->m_ItemOptionInfoBEX.end(); it++)
	{
		...
		{
			if (lpItem->m_Level >= it->ItemLevelMin && lpItem->m_Level <= it->ItemLevelMax || it->ItemLevelMin == -1)
			{
...
void CItemOption::CBInsertOption(LPOBJ lpObj, int SpecialIndex, int SpecialValue) // OK
{
	//for (int n = 0; n < MAX_SPECIAL; n++)
	{
		...
		case ITEM_OPTION_MUL_DEFENSE:
			lpObj->Defense += (lpObj->Defense * value) / 100;
			break;
		case ITEM_OPTION_ADD_SD:
			lpObj->AddShield += value;
			break;
		case ITEM_OPTION_ADD_SD_RATE:
			lpObj->AddShield += (int)((lpObj->MaxShield * value) / 100);
			break;
