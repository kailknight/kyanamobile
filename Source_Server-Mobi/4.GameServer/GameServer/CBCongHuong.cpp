...
void CBCongHuong::CH_ClientSetItemGS(int aIndex, PMSG_MOVEITEM* lpMsg)
{
	...
	{
		gNotice.GCNoticeSend(aIndex, 0, 0, 0, 10, 0, 0, gMessage.GetMessage(3035));
		return;
	}
	if (lpObj->Inventory[lpMsg->Source].m_NewOption < 1 || (lpObj->Inventory[lpMsg->Source].m_Index / 512) >= 12)
	{
		gNotice.GCNoticeSend(aIndex, 0, 0, 0, 10, 0, 0, gMessage.GetMessage(3036));//"[CongHuong] Item khong phu hop !!");
		return;
	}

	if (this->CHAllowAnc == 0 && lpObj->Inventory[lpMsg->Source].m_SetOption > 0)
	{
		gNotice.GCNoticeSend(aIndex, 0, 0, 0, 10, 0, 0, gMessage.GetMessage(3037));//"[CongHuong] Item khong phu hop !!");
		return;
	}
	//if (lpObj->Inventory[lpMsg->Source].m_NewOption < 1 || (lpObj->Inventory[lpMsg->Source].m_Index / 512) >= 12)
	//{
...
void CBCongHuong::CH_MixItemGS(int aIndex, XULY_CGPACKET* lpMsg)
{
	...
	if (!failed)
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3038), lpObj->Name, gItemManager.GetItemName(gObj[aIndex].CH_InfoItem[0].m_Index));
	...
	else
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3039), lpObj->Name, gItemManager.GetItemName(gObj[aIndex].CH_InfoItem[0].m_Index));//"[Cộng Hưởng] %s Cộng Hưởng Thành Công !",lpObj->Name);
