...
int CItemBag::GetItemNewOption_New(ITEM_BAG_INFO* lpInfo, int Slot) // OK
{
	...
	// The loop below draws and applies one random excellent-option bit on its
	// first pass before it ever checks lpInfo->NewOption (the row's Excellent
	// column) - the count>=CheckMaxCount exit only runs AFTER that first bit
	// is already added, so a row configured for 0 excellent options still
	// got exactly one. Short-circuit here instead - 0 means none, same as
	// GetItemNewOption() above already does correctly.
...
bool CItemBag::DropItem(LPOBJ lpObj,int map,int x,int y) // OK
{
	...
	for(int n=0;n < this->m_ItemDropCount;n++)
	{
		...
		// Same 0-100 scale as m_ItemDropRate right above - every EventItemBag file
		// that has ever set this uses 0 or 100, clearly meaning "off"/"always", but
		// the %10000 check made 100 a 1% chance instead of guaranteed. Rescaled to
		// match its sibling column.
		if((GetLargeRand()%100) < this->m_SetItemDropRate)
		{
...
bool CItemBag::DropInventoryItem(LPOBJ lpObj,int map,int x,int y) // OK
{
	...
	for(int n = 0 ; n < this->m_ItemDropCount ; n++ )
	{
		...
		// Same rescale as DropItem() above - 0-100, matching m_ItemDropRate.
		if(( GetLargeRand() % 100 ) < this->m_SetItemDropRate )
		{
			gSetItemType.MakeRandomSetItem(lpObj->Index, map, px, py);
			continue;
