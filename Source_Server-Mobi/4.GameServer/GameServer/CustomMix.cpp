...
void CCustomMix::SendInfoCustomMix(int aIndex, int IndexMix)
{
	...
	// Set before the IndexMix == -1 bail below, so the window still knows the
	// cap while no recipe is matched.
	pMsg.MaxTalismanOfLuck = gServerInfo.m_MaxTalismanOfLuck;

	pMsg.Index = IndexMix;

	std::map<int, BCUSTOM_MIX>::iterator it;
	std::map<int, BCUSTOM_MIX_INFO>::iterator itInfo;
	std::vector<BCUSTOM_MIX_NGUYENLIEU>::iterator iter;

	if (IndexMix == -1) goto SendClearInfo;

	it = this->m_BData_Mix.find(IndexMix);
	...
	itInfo = this->m_BData_Mix_Info.find(IndexMix);
...
int CCustomMix::GetChaoMixItemCount(LPOBJ lpObj, int IndexMin, int IndexMax, int MinLevel, int MaxLevel, int MinOpt, int NeedLuck, int NeedSkill, int NeedExl, int Need380, int NeedAnc, int NeedHarmory, int NeedSocket) // OK
{
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		{
			// A Talisman of Luck is a success-rate modifier, never an
			// ingredient. Counting one towards a recipe row whose index range
			// happens to cover it would let a talisman stand in for a real
			// ingredient.
			if (CChaosBox::IsTalismanOfLuckItem(lpObj->ChaosBox[n].m_Index))
			{
				continue;
			}
...
int CCustomMix::CheckItemMixForIndex(LPOBJ lpObj, int IndexMix)
{
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
			...
			// THE recipe-disappears bug. Every item in the box has to match
			// some ingredient row or the loop below gives up and returns -1,
			// which SendInfoCustomMix turns into "no recipe" and the client
			// renders as an empty panel. A Talisman of Luck matches nothing,
			// so dropping one in destroyed the recipe the player had just
			// assembled.
			//
			// It looked like "the third talisman breaks it" rather than the
			// first because of the goto SetIndexMix path: a non-matching item
			// does not fail outright, it walks GetMixNumber on to the NEXT
			// row's recipe index and tries again. With few enough talismans
			// that walk happened to land back on a valid row; with one more it
			// ran off the end of the table. Which count tips it over is an
			// accident of row ordering, not a real limit.
			if (CChaosBox::IsTalismanOfLuckItem(lpObj->ChaosBox[n].m_Index))
			{
				continue;
			}
