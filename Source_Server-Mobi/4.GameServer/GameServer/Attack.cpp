...
bool CAttack::Attack(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, bool send, BYTE flag, __int64 damage, int count, bool combo) // OK
{
	...
	if(damage == 0)
	{
		...
		{
			damage -= (damage*lpTarget->DamageReduction[n])/gServerInfo.m_GioiHanGiamSatThuong;
		}
...
#endif
		if (skill == SKILL_FALLING_SLASH || skill == SKILL_LUNGE || skill == SKILL_UPPERCUT || skill == SKILL_CYCLONE || skill == SKILL_SLASH /*|| skill == SKILL_STAR_FALL || skill == SKILL_TWISTING_SLASH*/ || skill == SKILL_RAGEFUL_BLOW /*|| skill == SKILL_DEATH_STAB || skill == SKILL_CRESCENT_MOON_SLASH*/ || skill == SKILL_IMPALE || skill == SKILL_FIRE_BREATH /*|| skill == SKILL_ICE_ARROW *//*|| skill == SKILL_PENETRATION *//*|| skill == SKILL_FIRE_SLASH || skill == SKILL_SPIRAL_SLASH || skill == SKILL_FROZEN_STAB || skill == SKILL_CHARGE */ || skill == SKILL_BLOOD_STORM)
		{
		...
		if (lpObj->Class == CLASS_DK)
		{
			...
			{
				damage = (damage * lpObj->DKDamageMultiplierRate) / gServerInfo.ChinhDameXoayKiemDK;
			}

			else if (skill == SKILL_DEATH_STAB)
			{
				damage = (damage * lpObj->DKDamageMultiplierRate) / gServerInfo.ChinhDameDamGioDK;
			}

			else if (skill == SKILL_FROZEN_STAB)
			{
				damage = (damage * lpObj->DKDamageMultiplierRate) / gServerInfo.ChinhDameChemBangDK;
			}
		}
		...
		if (lpObj->Class == CLASS_MG)
		{
			...
			//}
			 if (skill == SKILL_POWER_SLASH)
			{
				damage = (damage * lpObj->MGDamageMultiplierRate) / gServerInfo.ChinhDamePhayMG;
			}
			else if (skill == SKILL_SWORD_SLASH)
			{
				damage = (damage * lpObj->MGDamageMultiplierRate) / gServerInfo.ChinhDameHoaDiemMG;
			}
			else if (skill == SKILL_FIRE_SLASH)
			{
				damage = (damage * lpObj->MGDamageMultiplierRate) / gServerInfo.ChinhDameChemLuaMG;
			}
			else if (skill == SKILL_LIGHTNING_STORM)
			{
				damage = (damage * lpObj->MGDamageMultiplierRate) / gServerInfo.ChinhDameBaoDienMG;
			}
			else if (skill == SKILL_EVIL_SPIRIT)
			{
				damage = (damage * lpObj->MGDamageMultiplierRate) / gServerInfo.ChinhDameQuayRongMG;
			}
			else if (skill == SKILL_FLAME)
			{
				damage = (damage * lpObj->MGDamageMultiplierRate) / gServerInfo.ChinhDameCotLuaMG;
			}
			else if (skill == SKILL_DECAY )
			{
				damage = (damage * lpObj->MGDamageMultiplierRate) / gServerInfo.ChinhDameMuaDocToMG;
			}
			else if (skill == SKILL_POISON)
			{
				damage = (damage * lpObj->MGDamageMultiplierRate) / gServerInfo.ChinhDameMuaDocMG;
			}
			else if (skill == SKILL_BLAST)
			{
				damage = (damage * lpObj->MGDamageMultiplierRate) / gServerInfo.ChinhDameMuaSaoBangMG;
			}
				...
				|| skill == SKILL_BLOOD_STORM || skill == SKILL_BLAST)
			{
				damage = (damage * lpObj->MGDamageMultiplierRate) / 100;
			}
		}
		...
		if (lpObj->Class == CLASS_DW)
		{
			...
			{
				damage = (damage * 200) / gServerInfo.ChinhDameQuayRongDW;
			}

			else if (skill == SKILL_ICE_STORM)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameMuaBangTuyetDW;
			}

			else if (skill == SKILL_BLAST)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameMuaSaoBangDW;
			}

			else if (skill == SKILL_FLAME)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameCotLuaDW;
			}

			else if (skill == SKILL_DECAY)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameMuaDocToDW;
			}

			else if (skill == SKILL_POISON)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameMuaDocDW;
			}
		}
		...
		{
			if (skill == SKILL_FIVE_SHOT)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameNguTienFE;
			}

			else if (skill == SKILL_ICE_ARROW)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameBanBangFE;
			}
	
			else if (skill == SKILL_PENETRATION)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameXuyenTamTienFE;
			}
		}

		if (lpObj->Class == CLASS_DL)
		{
			if ( skill == SKILL_EARTHQUAKE || skill == SKILL_ELECTRIC_SPARK /*|| skill == SKILL_FORCE || skill == SKILL_FIRE_BLAST*/)
			{
				damage = (damage * lpObj->DLDamageMultiplierRate) / 100;
			}
			if (skill == SKILL_FIRE_BURST)
			{
				damage = (damage * lpObj->DLDamageMultiplierRate) / gServerInfo.ChinhDameHoaXichLongDL;
			}

			else if (skill == SKILL_BIRDS)
			{
				damage = (damage * lpObj->DLDamageMultiplierRate) / gServerInfo.ChinhDameHonLoanDL;
			}

			else if (skill == SKILL_FIRE_SCREAM)
			{
				damage = (damage * lpObj->DLDamageMultiplierRate) / gServerInfo.ChinhDameHoaAmDL;
			}
		...
		{
			if (skill == SKILL_ELECTRIC_SURGE)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameQuaCauDienSUM;
			}

			else if (skill == SKILL_RED_STORM)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameSocDienSUM;
			}

			else if (skill == SKILL_DRAIN_LIFE)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameHutMauSUM;
			}

			else if (skill == SKILL_CHAIN_LIGHTNING)
			{
				damage = (damage * 200) / gServerInfo.ChinhDameSetLienHoanSUM;
		...
		//{
		//	damage = (damage + (((lpObj->Strength + lpObj->AddStrength) + (lpObj->Dexterity + lpObj->AddDexterity)) / gServerInfo.m_RFDamageMultiplierConstA) + ((lpObj->Vitality + lpObj->AddVitality) / gServerInfo.m_RFDamageMultiplierConstB) + ((lpObj->Energy + lpObj->AddEnergy) / gServerInfo.m_RFDamageMultiplierConstC));
		//	damage = (damage * lpObj->RFDamageMultiplierRate[2]) / gServerInfo.ChinhDameAoAnhLienHoanRF;
		//}
		else if(skill == SKILL_DARK_SIDE)
		{
	...
	else
	{
		if(skill != SKILL_EXPLOSION)
		{
			...
			if(lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
			{
				const int reflectPercent = this->GetPvPReflectPercent(lpObj,lpTarget);

				if(reflectPercent < 100)
				{
					damage = (damage*reflectPercent)/100;
				}
			}
		}
	}
	...
	if(lpObj->Type == OBJECT_USER)
	{
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			// PvP damage ramp - the longer this pair has been at it, the harder
			// the blows land. Applied here, after the flat PvP rates and before
			// the per-event rates below, so an event with its own damage rate
			// scales the ramped damage rather than the ramp overriding it.
			{
				const int rampPercent = this->GetPvPDamageRampPercent(lpObj,lpTarget);

				if(rampPercent > 0)
				{
					damage = (damage*(100+rampPercent))/100;
				}
			}
		...
		SendHidenPetIsAttack(lpObj, skill);
#endif
	}
	...
	if(lpSkill != 0 && count <= 1)
	{
		...
		gBCustomEdit.FixSkillGiatLui(lpObj->Index, lpTarget->Index, lpSkill->m_skill);
	}
	...
	// CommboEffectEnable (CustomConfig.ini): guarantee Combo's push-back,
	// but only on the hit that actually kills the target - checked here,
	// after Life/Shield were just resolved above, instead of predicted
	// beforehand (a PvP prediction against raw `damage` ignored Shield
	// absorption and could guess a kill that Shield then prevented).
	// `combo` is this function's own parameter (set true by
	// CComboSkill::CheckCombo() upstream) - lpSkill->m_skill is always the
	// real finisher skill's ID (e.g. Twisting Slash), never SKILL_COMBO,
	// so that was never a usable signal for "this hit is a combo hit".
	// Skill 59 (SKILL_COMBO) has no entry in FixTele.xml's SkillBackSpring
	// table, so this calls gObjBackSpring2 directly instead of routing
	// through FixSkillGiatLui's table lookup.
	if(gServerInfo.m_CommboEffectEnable != 0 && combo != 0 && lpSkill != 0 && count <= 1 && lpTarget->Life <= 0)
	{
		gObjBackSpring2(lpTarget,lpObj,gServerInfo.m_CustomComboDistance);
	}
	...
	if(damage > 0)
	{
		...
		if(effect != 4 && lpTarget->Type == OBJECT_USER)
		{
			...
			if(ReflectDamage > 0)
			{
				gObjAddMsgSendDelay(lpTarget,10,lpObj->Index,10,ReflectDamage);
			}
...
bool CAttack::AttackElemental(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			// Same ramp as the physical path, so elemental damage does not sit
			// outside the mechanic. Calling it twice for one blow is harmless -
			// the bonus is read off elapsed time, not accumulated per call.
			{
				const int rampPercent = this->GetPvPDamageRampPercent(lpObj,lpTarget);
...
void CAttack::WingSprite(LPOBJ lpObj, LPOBJ lpTarget, __int64* damage) // OK
{
...
void CAttack::HelperSprite(LPOBJ lpObj, LPOBJ lpTarget, __int64* damage) // OK
{
	if(lpObj != 0 && lpObj->Type == OBJECT_USER)
	{
...
void CAttack::DamageSprite(LPOBJ lpObj, __int64 damage) // OK
{
	if(lpObj->Type != OBJECT_USER)
	{
		return;
...
bool CAttack::DarkHorseSprite(LPOBJ lpObj, __int64 damage) // OK
{
	CItem* lpItem = &lpObj->Inventory[8];
...
bool CAttack::FenrirSprite(LPOBJ lpObj, __int64 damage) // OK
{
	CItem* lpItem = &lpObj->Inventory[8];
...
//luego probar aqui
// True while either PvP-pairing feature needs the clocks maintained. The two
// share one clock per pairing - "how long has this attacker been working on
// this opponent" - so neither may assume the other is switched on.
static bool IsPvPPairClockNeeded() // OK
{
	return (gServerInfo.m_PvPDamageRampPercent > 0 || gServerInfo.m_PvPReflectDecayPercent > 0);
}

// The two players have to be a real PvP pairing, and neither may be standing
// on a safe tile. Shared by everything below so one rule governs both features.
static bool IsPvPPairEligible(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(gServerInfo.InSafeZone(lpObj->Index) != false || gServerInfo.InSafeZone(lpTarget->Index) != false)
	{
		return false;
	}
...
// Finds lpObj's live clock against lpTarget, or 0. Read-only: it neither opens
// a pairing nor refreshes one, so a caller that is not itself landing a blow
// (the reflect decay) cannot keep a dead fight alive.
static PVP_RAMP_SLOT* FindPvPPairClock(LPOBJ lpObj,LPOBJ lpTarget,DWORD now,DWORD resetMs) // OK
{
	for(int n=0;n < MAX_PVP_RAMP_SLOTS;n++)
	{
		if(lpObj->m_PvPRamp[n].TargetIndex != lpTarget->Index)
		{
			continue;
		}

		// Life id moved on, so the opponent has died since this pairing began.
		if(lpObj->m_PvPRamp[n].TargetLifeId != lpTarget->m_PvPRampLifeId)
		{
			continue;
		}

		// Quiet long enough to count as over.
		if((now - lpObj->m_PvPRamp[n].LastHitTick) > resetMs)
		{
			continue;
		}

		return &lpObj->m_PvPRamp[n];
	}

	return 0;
}

// How long a pairing's clock is kept alive, in ms.
//
// The ramp and the reflect decay share one clock but each has its own
// disengage timeout, so the clock has to survive as long as the more forgiving
// of the two - otherwise the shorter timeout would restart StartTick out from
// under the other feature and silently cut its window short. Each consumer
// then applies its OWN threshold when reading, so they still behave
// independently; this only decides when the underlying clock is thrown away.
//
// A feature that is switched off does not get a vote.
static DWORD GetPvPPairClockLifeMs() // OK
{
	int seconds = 0;

	if(gServerInfo.m_PvPDamageRampPercent > 0 && gServerInfo.m_PvPDamageRampResetSeconds > seconds)
	{
		seconds = gServerInfo.m_PvPDamageRampResetSeconds;
	}

	if(gServerInfo.m_PvPReflectDecayPercent > 0 && gServerInfo.m_PvPReflectResetSeconds > seconds)
	{
		seconds = gServerInfo.m_PvPReflectResetSeconds;
	}

	return (DWORD)seconds * 1000;
}

// Whole intervals this pairing has been running. Read off the clock rather
// than counted per hit, so attack speed does not decide how fast either
// feature moves - a slow two-hander and a fast dual-wield reach the same
// point at the same moment in the fight.
static int GetPvPPairSteps(PVP_RAMP_SLOT* lpSlot,DWORD now,int IntervalSeconds) // OK
{
	if(lpSlot == 0 || IntervalSeconds <= 0)
	{
		return 0;
	}

	return (int)((now - lpSlot->StartTick) / ((DWORD)IntervalSeconds * 1000));
}

// Opens or refreshes lpObj's clock against lpTarget and returns it. Called
// only from the attacker's own blow, which is what makes the clock mean
// "how long this attacker has been on this opponent".
PVP_RAMP_SLOT* CAttack::UpdatePvPPairClock(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	if(IsPvPPairClockNeeded() == false || IsPvPPairEligible(lpObj,lpTarget) == false)
	{
		return 0;
	}

	const DWORD now = GetTickCount();
	// The shared lifetime, not either feature's own timeout - see
	// GetPvPPairClockLifeMs.
	const DWORD resetMs = GetPvPPairClockLifeMs();

	PVP_RAMP_SLOT* lpSlot = FindPvPPairClock(lpObj,lpTarget,now,resetMs);

	if(lpSlot != 0)
	{
		lpSlot->LastHitTick = now;
		return lpSlot;
	}

	// New pairing. Prefer a free slot; otherwise evict whichever has gone
	// longest without a blow. Ages are compared rather than raw ticks so the
	// choice stays correct across GetTickCount's 49-day wrap.
	for(int n=0;n < MAX_PVP_RAMP_SLOTS;n++)
	{
		if(lpObj->m_PvPRamp[n].TargetIndex == -1)
		{
			lpSlot = &lpObj->m_PvPRamp[n];
			break;
		}

		if(lpSlot == 0 || (now - lpObj->m_PvPRamp[n].LastHitTick) > (now - lpSlot->LastHitTick))
		{
			lpSlot = &lpObj->m_PvPRamp[n];
		}
	}

	lpSlot->TargetIndex = lpTarget->Index;
	lpSlot->TargetLifeId = lpTarget->m_PvPRampLifeId;
	lpSlot->StartTick = now;
	lpSlot->LastHitTick = now;

	// Deliberately not returned as a live clock: a pairing that started this
	// instant is at step 0 either way, and returning 0 keeps "just opened"
	// distinct from "running".
	return 0;
}

int CAttack::GetPvPDamageRampPercent(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	if(gServerInfo.m_PvPDamageRampPercent <= 0)
	{
		return 0;
	}

	PVP_RAMP_SLOT* lpSlot = this->UpdatePvPPairClock(lpObj,lpTarget);

	const int steps = GetPvPPairSteps(lpSlot,GetTickCount(),gServerInfo.m_PvPDamageRampSeconds);
	...
	int bonus = steps * gServerInfo.m_PvPDamageRampPercent;

	if(gServerInfo.m_PvPDamageRampMaxPercent > 0 && bonus > gServerInfo.m_PvPDamageRampMaxPercent)
	{
		bonus = gServerInfo.m_PvPDamageRampMaxPercent;
	}

	return bonus;
}

// What percentage of its reflect damage a defender still gets back, 100 down
// to m_PvPReflectMinPercent. The mirror of the damage ramp: the longer a fight
// drags on, the less the pair can hide behind reflect.
//
// Argument order is the reflect call's, NOT the attacker's: lpObj is the one
// dealing the reflect (the original defender) and lpTarget is the original
// attacker. The clock that matters is therefore lpTarget's against lpObj -
// the attacker's pairing - because that is the one being refreshed by blows.
// Reading lpObj's own clock would decay nothing for a defender who never
// swings back.
int CAttack::GetPvPReflectPercent(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	if(gServerInfo.m_PvPReflectDecayPercent <= 0)
	{
		return 100;
	}

	if(IsPvPPairEligible(lpObj,lpTarget) == false)
	{
		return 100;
	}

	const DWORD now = GetTickCount();
	// Reflect's own disengage window, independent of the ramp's.
	const DWORD resetMs = (DWORD)gServerInfo.m_PvPReflectResetSeconds * 1000;

	// Read-only on purpose: reflect is not a blow of lpObj's, so it must not
	// open a pairing or push the disengage timeout out.
	PVP_RAMP_SLOT* lpSlot = FindPvPPairClock(lpTarget,lpObj,now,resetMs);

	const int steps = GetPvPPairSteps(lpSlot,now,gServerInfo.m_PvPReflectDecaySeconds);
	...
	int percent = 100 - (steps * gServerInfo.m_PvPReflectDecayPercent);

	if(percent < gServerInfo.m_PvPReflectMinPercent)
	{
		percent = gServerInfo.m_PvPReflectMinPercent;
	}
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (lpObj->Guild != 0 && lpTarget->Guild != 0)
	{
		...
		if (lpObj->Guild->Number == lpTarget->Guild->Number && gServerInfo.m_CungGuildPK != 0 && gDuel.CheckDuel(lpObj, lpTarget) == 0
			...
			&& gBsVEvent.CheckPlayerJoined(lpObj, lpTarget) == 0 // pk map greatevent
			...
			&& lpTarget->Map != MAP_ILLUSION_TEMPLE_FINAL2)
		{
			return 0;
		}
	...
	if (gObjGetRelationShip(lpObj, lpTarget) == 1 && gServerInfo.m_PKLienMinh != 0 && gDuel.CheckDuel(lpObj, lpTarget) == 0
		&& gBsVEvent.CheckPlayerJoined(lpObj, lpTarget) == 0 // pk map greatevent
...
int CAttack::GetTargetDefense(LPOBJ lpObj,LPOBJ lpTarget,WORD* effect) // OK
{
	...
	// Berserker's defense penalty belongs to the Summoner who cast it - the
	// target. Passing the attacker made a berserking Summoner strip the
	// defense of everyone it hit instead of lowering its own.
	gSkillManager.SkillSwordPowerGetDefense(lpTarget->Index,&defense);
	...
	if((GetLargeRand()% gServerInfo.m_GioiHanLoaiBoPhongThu) < ((lpObj->IgnoreDefenseRate+lpObj->EffectOption.AddIgnoreDefenseRate)-lpTarget->ResistIgnoreDefenseRate))
	{
		(*effect) = 5;
...
int CAttack::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	__int64 DamageMin = 0;
	__int64 DamageMax = 0;
	__int64 SkillDamageMin = 0;
	__int64 SkillDamageMax = 0;
	...
	else
	{
		...
		if((GetLargeRand() % gServerInfo.m_GioiHanSatThuongChiMang) < ((lpObj->CriticalDamageRate+lpObj->EffectOption.AddCriticalDamageRate)-lpTarget->ResistCriticalDamageRate))
		{
			(*effect) = 3;
		...
		if((GetLargeRand()% gServerInfo.m_GioiHanSatThuongHoanHao) < ((lpObj->ExcellentDamageRate+lpObj->EffectOption.AddExcellentDamageRate)-lpTarget->ResistExcellentDamageRate))
		{
			(*effect) = 2;
		...
		if ((GetLargeRand() % gServerInfo.GioiHanTyLeX2SatThuong) < ((lpObj->DoubleDamageRate) - lpTarget->ResistDoubleDamageRate))
		{
			...
			(*effect) = gServerInfo.ColorDamex2;
			damage = DamageMax;
		...
		if ((GetLargeRand() % gServerInfo.GioiHanTyLeX3SatThuong) < ((lpObj->TripleDamageRate)))
		{
			...
			(*effect) = gServerInfo.ColorDamex3;
			damage = DamageMax;
			damage += lpObj->TripleDamageRate;
		}
	...
	damage = this->ApplyTargetDefense(lpObj,damage,TargetDefense);
...
/*
	...
	DamageCalcSystem 0 (default): damage - defense, the original formula. A hit
	smaller than the target's defense comes out 0 and the caller's minimum-damage
	floor ((Level+ML)/10, then the PvP rates) replaces it - which is why a
	low-damage class stayed at two digits however high its PvP rate went.

	DamageCalcSystem 1: damage * K / (K + defense), K = DefenseConstant. Defense
	takes a share of the hit instead of a flat amount, so every class keeps
	scaling with its damage and rates. Only when a player attacks (players and
	monsters as targets); monsters hitting players keep the subtraction.
*/
int CAttack::ApplyTargetDefense(LPOBJ lpObj,__int64 damage,int defense) // OK
{
	if(gServerInfo.m_DamageCalcSystem == 1 && lpObj->Type == OBJECT_USER)
	{
		__int64 k = ((gServerInfo.m_DefenseConstant < 1) ? 1 : gServerInfo.m_DefenseConstant);

		damage = (defense > 0) ? ((damage*k)/(k+defense)) : damage;
	}
	else
	{
		damage -= defense;
...
int CAttack::GetShieldDamage(LPOBJ lpObj, LPOBJ lpTarget, __int64 damage) // OK
{
...
void CAttack::GetPreviewPhysiDamage(LPOBJ lpObj,DWORD* DamageMin,DWORD* DamageMax,DWORD* MulDamage,DWORD* DivDamage) // OK
{
	...
	gSkillManager.SkillSwordPowerGetPhysiDamage(lpObj->Index, (__int64*)DamageMin, (__int64*)DamageMax);
...
void CAttack::GetPreviewMagicDamage(LPOBJ lpObj,DWORD* DamageMin,DWORD* DamageMax,DWORD* MulDamage,DWORD* DivDamage,DWORD* DamageRate) // OK
{
	...
	gSkillManager.SkillSwordPowerGetMagicDamage(lpObj->Index, (__int64*)DamageMin, (__int64*)DamageMax);
...
void CAttack::GetPreviewCurseDamage(LPOBJ lpObj,DWORD* DamageMin,DWORD* DamageMax,DWORD* MulDamage,DWORD* DivDamage,DWORD* DamageRate) // OK
{
	...
	gSkillManager.SkillSwordPowerGetCurseDamage(lpObj->Index, (__int64*)DamageMin, (__int64*)DamageMax);
