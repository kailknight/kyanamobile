...
#if(REDEEMCODE)
// Deliberately just (ItemIndex, Quantity) rather than depending on the
// redemption-code feature's own REDEEM_BUNDLE_ITEM (DSProtocol.h) - this
// keeps ItemManager itself decoupled from that feature; RedeemCode.cpp builds
// one of these per bundle row before calling CheckItemInventorySpaceForBundle.
struct REDEEM_ROOM_CHECK_ITEM
{
	int ItemIndex;
	int Quantity;
};
...
public:
...
#if(REDEEMCODE)
	bool CheckItemInventorySpaceForBundle(LPOBJ lpObj,const REDEEM_ROOM_CHECK_ITEM* items,int count);
#endif
	...
	int CItemManager::GetInventoryItemSpace(LPOBJ lpObj, int index);
