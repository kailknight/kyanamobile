...
void CTvTEvent::DivisionTeam()
{
	...
	for(int n=0;n<MAX_TVTEVENT_USER;n++)
	{
		if (this->User[n].Team == 1)
		{
			...
			gNotice.GCNoticeSend(this->User[n].Index,0,0,0,0,0,0,gMessage.GetMessage(3065),this->ScoreBlue,this->ScoreRed);
		}
		else if (this->User[n].Team == 2)
		{
			...
			gNotice.GCNoticeSend(this->User[n].Index,0,0,0,0,0,0,gMessage.GetMessage(3066),this->ScoreRed,this->ScoreBlue);
		}
	}
}
...
void CTvTEvent::UserDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	for (int i=0;i<MAX_TVTEVENT_USER;i++)
	{
		...
		{
			gNotice.GCNoticeSend(this->User[i].Index,0,0,0,0,0,0,gMessage.GetMessage(3067),this->ScoreBlue,this->ScoreRed);

		}
		else if(this->User[i].Team == TVT_REDTEAM)
		{
			gNotice.GCNoticeSend(this->User[i].Index,0,0,0,0,0,0,gMessage.GetMessage(3068),this->ScoreRed,this->ScoreBlue);
		}
	}
}
