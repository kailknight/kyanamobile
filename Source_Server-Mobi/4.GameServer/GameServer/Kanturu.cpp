...
// Seconds until Battle of Maya opens again, or 0 while a battle is running.
//
// The cycle is standby -> Maya -> (Nightmare) -> (Tower of Refinement) -> END
// -> standby. Standby is the short closed window between runs; Tower of
// Refinement is the long one, and only happens when Nightmare was beaten.
int CKanturu::GetTimeUntilNextBattle() // OK
{
	int remain;
	...
	switch(this->GetKanturuState())
	{
		...
		case KANTURU_STATE_TOWER_OF_REFINEMENT:
			remain = (this->m_TowerOfRefinement.GetPhaseRemainTime()+this->m_BattleStanby.GetPhaseRemainTime());
			break;

		default:
			// standby, or NONE/END on their way straight back into standby
			remain = this->m_BattleStanby.GetPhaseRemainTime();
			break;
	}

	// never report 0 while the battle is not actually running - the client
	// renders 0 as "Online"
	return ((remain<=0)?1:remain);
}

// Feeds the "Kanturu" row of the in-game event schedule window (H).
void CKanturu::UpdateEventTimeDisplay() // OK
{
	gServerDisplayer.EventKanturu = this->GetTimeUntilNextBattle();
}
