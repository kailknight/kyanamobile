...
// Turns the client-displayed "1.0.DDMMYY.HHMM" build string into the comparable
// YYMMDDHHMM number. 0 when the value is empty or not that shape. Defined in
// ServerInfo.cpp.
DWORD ParseClientBuildVersion(const char* szValue);
...
public:
	...
	int m_ItemDropTime;
	int m_ItemDropRate[MAX_ACCOUNT_LEVEL]; // dead - loaded but nothing reads it since ItemDropRate became the map-level-only gate; don't confuse with m_ItemDropRateVIPBonus below
	int m_ItemDropRateVIPBonus[MAX_ACCOUNT_LEVEL]; // 0-100, extra % chance added on top of a CItemDrop/CItemBag roll, by AccountLevel (VIP tier)
	int m_MoneyDropTime;
	...
	// Zero-initialized in the CServerInfo constructor (ServerInfo.cpp), not
	// here: this class is also built under an older toolset (Release_EX603CS
	// - error C2864, in-class initializers are only allowed for static const
	// integral members there) that rejects C++11 default member initializers
	// on a plain int field. See the constructor for why these specifically
	// need it - a division by an uninitialized denominator here is what
	// produced the "same garbage attack/defense rate for every class" bug.
	...
	int m_AttackSpeedAgiStep;	// >0: every Step Agility gives +PerStep speed, all classes
	int m_AttackSpeedPerStep;
	// Maximum attack speed caps (0 = no cap / disabled)
	int m_DWMaxAttackPhysiSpeed;
	int m_DKMaxAttackPhysiSpeed;
	int m_FEMaxAttackPhysiSpeed;
	int m_MGMaxAttackPhysiSpeed;
	int m_DLMaxAttackPhysiSpeed;
	int m_SUMaxAttackPhysiSpeed;
	int m_RFMaxAttackPhysiSpeed;
	int m_DWMaxAttackMagicSpeed;
	int m_DKMaxAttackMagicSpeed;
	int m_FEMaxAttackMagicSpeed;
	int m_MGMaxAttackMagicSpeed;
	int m_DLMaxAttackMagicSpeed;
	int m_SUMaxAttackMagicSpeed;
	int m_RFMaxAttackMagicSpeed;
	...
	// Zero-initialized in the constructor - same reasoning and same toolset
	// constraint as the AttackSuccessRatePvP block above.
	...
	int m_CustommixThanhCong;
	int m_CustommixThatBai;

	// MG
	int TiLeLamChamPhayMG;
	int ThoiGianLamChamPhayMG;
	int ChinhDamePhayMG;
	int ChinhDameHoaDiemMG;
	int ChinhDameChemLuaMG;
	int ChinhDameBaoDienMG;
	int ChinhDameQuayRongMG;
	int ChinhDameCotLuaMG;
	int ChinhDameMuaDocToMG;
	int ChinhDameMuaDocMG;
	int ChinhDameMuaSaoBangMG;
	...
	int m_TyLeRaComboRF;
	// DW
	int ChinhDameQuayRongDW;
	int ChinhDameMuaBangTuyetDW;
	int ChinhDameMuaSaoBangDW;
	int ChinhDameCotLuaDW;
	int ChinhDameMuaDocToDW;
	int ChinhDameMuaDocDW;

	//DK
	int ChinhDameXoayKiemDK;
	int ChinhDameDamGioDK;
	int ChinhDameChemBangDK;

	// DL
	int ChinhDameHoaXichLongDL;
	int ChinhDameHonLoanDL;
	int ChinhDameHoaAmDL;

	// ELF
	int ChinhDameNguTienFE;
	int ChinhDameBanBangFE;
	int ChinhDameXuyenTamTienFE;
	int TyLeRaChoangSkillMasterNguTien;
	int ThoiGianChoangSkillMasterNguTien;

	// RF
	int ChinhDameVatNgaRF;
	int ChinhDameAoAnhLienHoanRF;

	// SUM
	int ChinhDameSetLienHoanSUM;
	int ChinhDameQuaCauDienSUM;
	int ChinhDameSocDienSUM;
	int ChinhDameHutMauSUM;
	...
	// 0 = original file-driven XShop, 1 = database-backed catalog,
	// discounts and purchase logging via the DataServer. Rollback is setting it to 0.
	int m_CustomCashShop;
	//Event PvP
	...
	void ReadCustomConfig(char* section, char* path);

	// Reads an ini value by its English key name, falling back to the original
	// Vietnamese key name when the English one is absent. This lets renamed ini
	// files and old untouched ini files (Sub 1, backups, FullSrcTakumi12) both
	// keep working. GetDivisorEx additionally forces a minimum of 1, because the
	// value is used as a divisor and 0 would crash the server.
	int GetProfileIntEx(char* section, const char* key, const char* legacyKey, int defaultValue, char* path);
	int GetDivisorEx(char* section, const char* key, const char* legacyKey, int defaultValue, char* path);

	// Thoat Game
	int m_ThoiGianThoatGame;
	int ThoiGianDameDocDW;
	int TyLeRaChoangSkillMasterDamGio;
	int ThoiGianChoangSkillMasterDamGio;
	int m_TatQuayDo;
	int m_CungGuildPK;
	int m_PKLienMinh;
	...
	int m_TimeTop5Hit;
	int m_TinhDameBoss;
	int m_CommboEffectEnable;
	int m_DarkHorsePushBackChance;
	// ItemDropLog (CustomConfig.ini): 1 enables the [ItemLog] ground-item
	// lifecycle trace (DROP/PICKUP/EXPIRE) in Map.cpp. Off by default - it is
	// one console line per drop, which is a lot on a busy server.
	int m_ItemDropLog;
	int m_CustomComboDistance;
	int m_CustomDarkHorsePushDistance;
	// ItemPickupNotice (CustomConfig.ini): master switch for the server-wide
	// "picked up [item] at map [x]" broadcast (CustomNoticeDrop::NoticeItem).
	// Independent of the Enable column in Custom\CustomNoticeItem.txt - either
	// one being off suppresses the notice.
	int m_ItemPickupNotice;

	// PvPDamageRamp* (CustomConfig.ini): a duel that drags on hits harder the
	// longer it stays on the same opponent. Every m_PvPDamageRampSeconds on one
	// target adds m_PvPDamageRampPercent to that attacker's PvP damage against
	// THAT target, up to m_PvPDamageRampMaxPercent.
	//
	// One clock per pairing (see PVP_RAMP_SLOT in User.h), so switching targets
	// suspends a clock rather than clearing it. A clock restarts on either side
	// dying, or after m_PvPDamageRampResetSeconds with no blow landed between
	// those two. Nothing ramps while either player is on a safe-zone tile.
	//
	// m_PvPDamageRampPercent = 0 turns the whole thing off.
	int m_PvPDamageRampSeconds;
	int m_PvPDamageRampPercent;
	int m_PvPDamageRampMaxPercent;
	int m_PvPDamageRampResetSeconds;

	// PvPReflect* (CustomConfig.ini): the mirror of the ramp above. Every
	// m_PvPReflectDecaySeconds that a pairing has been fighting, the reflect
	// damage flowing back in that fight drops by m_PvPReflectDecayPercent of
	// its full value, down to a floor of m_PvPReflectMinPercent.
	//
	// It reads the SAME per-pairing clock as the ramp (PVP_RAMP_SLOT in
	// User.h), so both measure the same fight, and it shares the ramp's
	// disengage timeout. m_PvPReflectDecayPercent = 0 turns it off.
	int m_PvPReflectDecaySeconds;
	int m_PvPReflectDecayPercent;
	int m_PvPReflectMinPercent;

	// Its own disengage timeout, independent of the ramp's. The two features
	// read one shared clock but judge staleness separately, so reflect can
	// forgive a longer lull than the damage ramp does, or a shorter one.
	int m_PvPReflectResetSeconds;

	// VoiceChat* (CustomConfig.ini): proximity voice chat. The GameServer only
	// hands out sessions and feeds positions - audio never touches it.
	//
	// Two addresses, deliberately. m_VoiceChatHost is where THIS server reaches
	// the service (127.0.0.1 when they share a box); m_VoiceChatClientHost is
	// what players are told to send audio to, which has to be reachable from
	// the internet. Conflating them is the obvious mistake and it fails in the
	// least obvious way: everything works on the server's own machine.
	int  m_VoiceChatEnable;
	char m_VoiceChatHost[64];
	char m_VoiceChatClientHost[64];
	int  m_VoiceChatPort;

	// MinClientBuild* (CustomConfig.ini): refuse outdated clients at login. 0 on
	// either means "do not check that platform", and both default to 0.
	//
	// Two numbers because the platforms version differently, and the units are
	// NOT interchangeable - putting one platform's number in the other's key
	// blocks everybody:
	//
	//   MinClientBuildAndroid  the APK's versionCode (17, 18, ...)
	//   MinClientBuildPC       YYMMDDHH off the exe's link time (26091919 = a
	//                          Main.exe linked 19 Sep 2026, 19:00-19:59)
	//
	// The client reports its number in packet 0xD3:0x7D before it logs in. One
	// too old to send that packet reports nothing at all, which is its own
	// refusal case - see CGConnectAccountRecv. That is how builds predating the
	// feature are caught, and it is why PC needed a gate of its own rather than
	// relying on the Android key's side effects.
	DWORD m_MinClientBuildAndroid;
	DWORD m_MinClientBuildPC;

	// MaxTalismanOfLuck (CustomConfig.ini): how many Talismans of Luck the
	// Chaos Machine accepts in one mix. Each one adds 1% to the success rate;
	// going over this refuses the mix instead of running it. Honoured by the
	// item-level upgrade and the Regular Combination - the only two mixes
	// that take a talisman.
	int m_MaxTalismanOfLuck;
	// ShowResetLimitOnLogin (CustomConfig.ini): the "Today's reset limit is N
	// times, <name>!" golden notice sent on character login. Defaults to 0 (off)
