...
#if(REDEEMCODE)
// CheckItemInventorySpace above only ever answers "is there room for ONE
// item of this size", by scanning the live, unmutated grid - calling it once
// per bundle row independently gives a false positive whenever two rows'
// checks land on the very same free rect (e.g. two 2x2 items with only one
// free 2x2 area between them both independently report "yes, room" for a
// bundle that cannot actually hold both). Every item of every row - Quantity
// counted individually, since this engine doesn't stack multiple item
// instances into one slot - has to provisionally claim its own rect before
// the next check runs, or this check is worthless for anything but a
// single-item bundle.
//
// Built entirely from the two existing primitives InventoryRectCheck (find a
// free rect) and InventoryItemSet (mark it occupied) - the same pair
// InventoryInsertItem itself uses - plus a snapshot/restore of InventoryMap
// (the same idiom already used by MoveItemToInventoryFromInventory) so this
// is a pure check: whatever it provisionally marks, it always unmarks again
// before returning, success or failure.
bool CItemManager::CheckItemInventorySpaceForBundle(LPOBJ lpObj,const REDEEM_ROOM_CHECK_ITEM* items,int count)
{
	BYTE snapshot[INVENTORY_SIZE];
	memcpy(snapshot,lpObj->InventoryMap,INVENTORY_SIZE);

	bool allFit = true;
	int MaxY = (this->GetInventoryMaxValue(lpObj)-INVENTORY_WEAR_SIZE)/8;

	for(int row=0;row < count && allFit;row++)
	{
		ITEM_INFO info;

		if(this->GetInfo(items[row].ItemIndex,&info) == 0)
		{
			allFit = false;
			break;
		}

		for(int copy=0;copy < items[row].Quantity && allFit;copy++)
		{
			bool placed = false;

			for(int y=0;y < MaxY && !placed;y++)
			{
				for(int x=0;x < 8 && !placed;x++)
				{
					...
					BYTE slot = this->InventoryRectCheck(lpObj->Index,x,y,info.Width,info.Height);

					if(slot != 0xFF)
					{
						// Marked directly rather than via InventoryItemSet:
						// that helper derives the rect size from the item
						// ALREADY sitting in the slot (Inventory[slot].m_Index),
						// which during a pure check is still empty - so it
						// fails its own GetInfo and returns having marked
						// nothing, leaving every later row free to "fit" into
						// this very same rect. Width/Height are already known
						// here from GetInfo above, so mark the cells outright.
						for(int sy=0;sy < info.Height;sy++)
						{
							for(int sx=0;sx < info.Width;sx++)
							{
								lpObj->InventoryMap[(((sy+y)*8)+(sx+x))] = 1;
							}
						}
	...
	memcpy(lpObj->InventoryMap,snapshot,INVENTORY_SIZE); // always restore - this is a check, not a placement
	return allFit;
}
#endif
