...
void CMessage::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			// _TRUNCATE, not strcpy_s: this file is hand-edited by admins, and a
			// line longer than the buffer would otherwise trip the invalid
			// parameter handler and take the whole server down at boot.
			strncpy_s(info.Message,lpMemScript->GetAsString(),_TRUNCATE);
