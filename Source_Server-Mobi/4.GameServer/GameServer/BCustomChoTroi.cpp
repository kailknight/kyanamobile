...
void BCustomChoTroi::Load(char* FilePath)
{
	...
	LogAddCat(LOG_CAT_MARKET, LOG_BLUE, "[Market] Load VAT %d, Load ItemBlock %d", this->m_DataVAT.size(), this->m_DataItemBlock.size());
}
...
void BCustomChoTroi::CGReqItemSell(PMSG_REQ_MARKET_SELL * lpMsg, int aIndex)
{
	...
	// Short summary for the Logs > Player Market Logs view. The full field dump
	// below stays in LOG_TRADE on disk - at ~260 characters it would be cut off
	// around "Level:" in the ~80-character log panel.
	LogAddCat(LOG_CAT_MARKET, LOG_BLACK, "[Market] %s listed %s +%d for %d [type %d]",
		lpUser->Name,
		gItemLevel.GetItemName(lpUser->CH_InfoItem[0].m_Index, lpUser->CH_InfoItem[0].m_Level),
		lpUser->CH_InfoItem[0].m_Level, ItemPrice, ItemPriceType);
		...
		for (int i = 0; i < this->m_ListDataChoTroi[aIndex].size(); i++)
		{
			if (lpMsg->ID == this->m_ListDataChoTroi[aIndex][i].ID)
			{
				...
				if (lpMsg->Result == 1) //MUA
				{
					...
					LogAddCat(LOG_CAT_MARKET, LOG_BLACK, "[Market] %s bought #%d %s +%d for %d [type %d]",
						lpUser->Name, ListID,
						gItemLevel.GetItemName(item.m_Index, item.m_Level),
						item.m_Level, ItemPrice, ItemPriceType);
				...
				else //Thu hoi
				{
					...
					int ItemPrice = this->m_ListDataChoTroi[aIndex][i].Price;
					int ItemPriceType = this->m_ListDataChoTroi[aIndex][i].PriceType;
					LogAddCat(LOG_CAT_MARKET, LOG_BLACK, "[Market] %s withdrew #%d %s +%d",
						lpUser->Name, ListID,
						gItemLevel.GetItemName(item.m_Index, item.m_Level),
						item.m_Level);
...
void BCustomChoTroi::DGAnsItemStatus(CBCUSTOM_LOAD_COUNT * lpMsg)
{
	...
	for (int n = 0; n < lpMsg->count; n++)
	{
		...
		LogAddCat(LOG_CAT_MARKET, LOG_GREEN, "[Market] %s sold %s +%d for %d %s (tax %d%%, net %d)",
			lpUser->Name,
			gItemLevel.GetItemName(item.m_Index, item.m_Level),
			item.m_Level, ItemPrice, TypeCoin[ItemPriceType], RateTax, CoinAfterTax);
