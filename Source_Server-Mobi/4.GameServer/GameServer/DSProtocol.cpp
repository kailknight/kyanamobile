	...
	switch(head)
	{
...
#endif
#if(REDEEMCODE)
			case 0x30: //Redeem code - preview result
				gRedeemCode.OnPeekReply((SDHP_REDEEM_CODE_PEEK_RECV*)lpMsg);
				break;
			case 0x31: //Redeem code - commit result
				gRedeemCode.OnCommitReply((SDHP_REDEEM_CODE_COMMIT_RECV*)lpMsg);
				break;
#endif
			case 0x34: //Slot machine reward box - load result
#if(SLOTMACHINE)
				gSlotMachine.DGSlotClaimRecv((SDHP_SLOTCLAIM_SEND*)lpMsg);
#endif
				break;
		...
		case 0x18:
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
			{
				...
				case 0x08:
					#if(GAMESERVER_UPDATE>=501)
					gCashShop.DGCashShopCatalogBeginRecv((SDHP_CASH_SHOP_CATALOG_BEGIN_RECV*)lpMsg);
					#endif
					break;
				case 0x09:
					#if(GAMESERVER_UPDATE>=501)
					gCashShop.DGCashShopCatalogProductRecv((SDHP_CASH_SHOP_CATALOG_PRODUCT_RECV*)lpMsg);
					#endif
					break;
				case 0x0A:
					#if(GAMESERVER_UPDATE>=501)
					gCashShop.DGCashShopCatalogPackageRecv((SDHP_CASH_SHOP_CATALOG_PACKAGE_RECV*)lpMsg);
					#endif
					break;
				case 0x0C:
					#if(GAMESERVER_UPDATE>=501)
					gCashShop.DGCashShopPriceRecv((SDHP_CASH_SHOP_PRICE_RECV*)lpMsg);
					#endif
					break;
				case 0x0D:
					#if(GAMESERVER_UPDATE>=501)
					gCashShop.DGCashShopNameRecv((SDHP_CASH_SHOP_NAME_RECV*)lpMsg);
					#endif
					break;
				case 0x0E:
					#if(GAMESERVER_UPDATE>=501)
					gCashShop.DGCashShopBannerRecv((SDHP_CASH_SHOP_BANNER_RECV*)lpMsg);
					#endif
					break;
				case 0x10:
					#if(GAMESERVER_UPDATE>=501)
					gCashShop.DGCashShopCatalogLineRecv((SDHP_CASH_SHOP_CATALOG_LINE_RECV*)lpMsg);
					#endif
					break;
			}
			break;
...
{
	// The DataServer link is up. Ask for the custom catalog here rather than in
	// ServerInfo: the .txt loaders run during startup, long before this socket
	// exists, so a request sent there would go nowhere.
	//
	// This also covers a reconnect - if the DataServer restarts, the handshake
	// runs again and the catalog is refreshed without bouncing the GameServer.
	#if(GAMESERVER_UPDATE>=501)
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
 ...
 //DATCheckMap 	Check nếu thoát ra vào lại thì đá về Fate yêu cầu!
	...
	// Spin wheel claim box - load whatever prizes this character is still owed.
	gCustomVongQuay.GDSpinClaimSend(lpObj->Index);

#if(SLOTMACHINE)
	// Slot machine reward box - load whatever winnings this character has not taken.
	gSlotMachine.GDSlotClaimSend(lpObj->Index);
#endif
	...
	// ShowResetLimitOnLogin in CustomConfig.ini, default off. Note the message
	// cannot be suppressed by blanking or deleting entry 3064 in Message.txt -
	// CMessage::GetMessage returns "Could not find message 3064!" for a missing
	// index, which would be displayed instead.
	if(gServerInfo.m_ShowResetLimitOnLogin != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 0, 0, 0, 0, 0, 0, gMessage.GetMessage(3064), gServerInfo.GHRSMax, lpObj->Name);
	}
	...
	// Sent here, as the character enters the game, because the shop window has to
	// be able to label its custom-currency button before any shop is opened.
	gCustomStore.GCCustomStoreItemInfoSend(lpObj->Index);
...
void DGCreateItemRecv(SDHP_CREATE_ITEM_RECV* lpMsg) // OK
{
	...
	else if(lpMsg->Map == 0xEB)
	{
		...
		{
			// The inventory was full and this branch did not exist - the item was
			// dropped on the floor of the function with no log line, no message to
			// the player and no record anywhere, even though the DataServer had
			// already issued it a serial. Deliberately only LOGGED here rather than
			// rerouted: by this point the item is already real, and every 0xEB
			// caller shares this path (GM /make, Boss Guild rewards, cash shop
			// withdrawals, quest rewards...), so silently changing where their items
			// end up would be a behaviour change well beyond the spin wheel.
			//
			// The spin wheel avoids reaching here at all - CCustomVongQuay::MakeItem
			// now checks for space BEFORE granting and parks the prize in its claim
			// box instead, so no serial is ever issued for an item nobody holds.
			LogAdd(LOG_RED,"[DGCreateItemRecv] %s inventory full - item discarded (index %d, serial %08X)",
				lpObj->Name,item.m_Index,item.m_Serial);
		}
	}
...
void GDCharacterInfoSaveSend(int aIndex) // OK
{
	...
	// Spin wheel claim box. Reached on logout (CharacterGameClose), on the
	// 10-minute autosave, and immediately after a prize is parked or claimed.
	gCustomVongQuay.GDSpinClaimSaveSend(aIndex);

#if(SLOTMACHINE)
	// Slot machine reward box. Same three moments: logout, the 10-minute autosave,
	// and immediately after a win is parked or claimed.
	gSlotMachine.GDSlotClaimSaveSend(aIndex);
#endif
