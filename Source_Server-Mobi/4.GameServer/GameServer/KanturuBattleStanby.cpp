...
// Seconds left before the whole standby phase hands over to Battle of Maya,
// not just the current sub-state. m_Value is milliseconds for timed states.
int CKanturuBattleStanby::GetPhaseRemainTime() // OK
{
	int state = this->GetBattleStanbyState();

	if(state == KANTURU_STANBY_END)
	{
		return 0;
	}

	int remain = 0;
	int n = KANTURU_STANBY_START;

	if(state != KANTURU_STANBY_NONE)
	{
		remain = this->m_StateInfo[state].GetRemainTime();
		n = (state+1);
	}

	for(;n < KANTURU_STANBY_END;n++)
	{
