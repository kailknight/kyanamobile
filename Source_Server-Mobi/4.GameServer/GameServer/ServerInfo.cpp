...
CServerInfo::CServerInfo() // OK
{
	...
	// These otherwise sit as uninitialized heap garbage until ReadCharacterInfo()
	// loads them from the ini. ObjectManager.cpp's AttackSuccessRatePvP/
	// DefenseSuccessRatePvP formulas divide by these unconditionally, so a
	// process that started (or last reloaded config) before this ini section
	// existed showed the same garbage number for every class's PvP attack and
	// defense rate. Zeroing them here makes that fail safe - the divisions in
	// ObjectManager.cpp guard against a zero denominator - instead of showing
	// an arbitrary number. (Not a default member initializer at the
	// declaration: Release_EX603CS's older toolset rejects those - C2864 -
	// on a plain non-static int.)
	this->m_DWAttackSuccessRatePvPConstA = 0;
	this->m_DWAttackSuccessRatePvPConstB = 0;
	this->m_DWAttackSuccessRatePvPConstC = 0;
	this->m_DWAttackSuccessRatePvPConstD = 0;
	this->m_DKAttackSuccessRatePvPConstA = 0;
	this->m_DKAttackSuccessRatePvPConstB = 0;
	this->m_DKAttackSuccessRatePvPConstC = 0;
	this->m_DKAttackSuccessRatePvPConstD = 0;
	this->m_FEAttackSuccessRatePvPConstA = 0;
	this->m_FEAttackSuccessRatePvPConstB = 0;
	this->m_FEAttackSuccessRatePvPConstC = 0;
	this->m_FEAttackSuccessRatePvPConstD = 0;
	this->m_MGAttackSuccessRatePvPConstA = 0;
	this->m_MGAttackSuccessRatePvPConstB = 0;
	this->m_MGAttackSuccessRatePvPConstC = 0;
	this->m_MGAttackSuccessRatePvPConstD = 0;
	this->m_DLAttackSuccessRatePvPConstA = 0;
	this->m_DLAttackSuccessRatePvPConstB = 0;
	this->m_DLAttackSuccessRatePvPConstC = 0;
	this->m_DLAttackSuccessRatePvPConstD = 0;
	this->m_SUAttackSuccessRatePvPConstA = 0;
	this->m_SUAttackSuccessRatePvPConstB = 0;
	this->m_SUAttackSuccessRatePvPConstC = 0;
	this->m_SUAttackSuccessRatePvPConstD = 0;
	this->m_RFAttackSuccessRatePvPConstA = 0;
	this->m_RFAttackSuccessRatePvPConstB = 0;
	this->m_RFAttackSuccessRatePvPConstC = 0;
	this->m_RFAttackSuccessRatePvPConstD = 0;
	this->m_DWDefenseSuccessRatePvPConstA = 0;
	this->m_DWDefenseSuccessRatePvPConstB = 0;
	this->m_DWDefenseSuccessRatePvPConstC = 0;
	this->m_DKDefenseSuccessRatePvPConstA = 0;
	this->m_DKDefenseSuccessRatePvPConstB = 0;
	this->m_DKDefenseSuccessRatePvPConstC = 0;
	this->m_FEDefenseSuccessRatePvPConstA = 0;
	this->m_FEDefenseSuccessRatePvPConstB = 0;
	this->m_FEDefenseSuccessRatePvPConstC = 0;
	this->m_MGDefenseSuccessRatePvPConstA = 0;
	this->m_MGDefenseSuccessRatePvPConstB = 0;
	this->m_MGDefenseSuccessRatePvPConstC = 0;
	this->m_DLDefenseSuccessRatePvPConstA = 0;
	this->m_DLDefenseSuccessRatePvPConstB = 0;
	this->m_DLDefenseSuccessRatePvPConstC = 0;
	this->m_SUDefenseSuccessRatePvPConstA = 0;
	this->m_SUDefenseSuccessRatePvPConstB = 0;
	this->m_SUDefenseSuccessRatePvPConstC = 0;
	this->m_RFDefenseSuccessRatePvPConstA = 0;
	this->m_RFDefenseSuccessRatePvPConstB = 0;
	this->m_RFDefenseSuccessRatePvPConstC = 0;
}
...
void CServerInfo::ReadInit() // OK
{
	...
	gServerDisplayer.InitTimeEvent();
...
void CServerInfo::ReadChaosMixInfo() // OK
{
	...
	gChaosBox.LoadMixRateShow(gPath.GetFullPath("Custom\\CustomRateMix.xml"));
...
void CServerInfo::ReloadMapManagerInfo() // OK
{
	gMapManager.Load(gPath.GetFullPath("MapManager.txt"));

	LogAdd(LOG_BLUE,"[ServerInfo] MapManager reloaded successfully");
}
...
{
	gCustomVongQuay.LoadFileXML(gPath.GetFullPath("Custom\\CustomSpinWheel.xml"));
#if(SLOTMACHINE)
	gSlotMachine.Load(gPath.GetFullPath("Custom\\CustomSlotMachine.txt"));
#endif
#if (SAUDOIITEM)
	gSauChangeItem.LoadConfig(gPath.GetFullPath("Custom\\ChangeItem.xml"));
#endif
	gCustomMonterPoint.Load(gPath.GetFullPath("Custom\\CustomMonsterPoint.xml"));
#if (CongHuongV2)
	gCBCongHuong.CH_LoadConfigXML(gPath.GetFullPath("Custom\\CustomResonance.xml"));
#endif
#if(CB_HUYDONGEXC)
	gBHuyDongExc.LoadConfigXML(gPath.GetFullPath("Custom\\CustomRemoveExc.xml"));
#endif
#if(CB_VIP_CHAR)
	gBCustomVIPChar.Load(gPath.GetFullPath("Custom\\CustomVIPChar.xml")); //Custom VIP
#endif	
	// ------------------------------------------------------------
	// ADVANCE STATS
	...
	this->ReadCustomConfig("CustomConfig",".\\Data\\CustomConfig.ini");
...
#if(DANH_HIEU_NEW)
	//	gDanhHieu.Load();
	gDanhHieu.Load1(gPath.GetFullPath("Custom\\Title.txt"));
#endif
#if (CUSTOM_BCHOTROI)
	gBCustomChoTroi.Load(gPath.GetFullPath("Custom\\CustomPlayerMarket.xml"));
#endif
#if(JEWELBANKVER2)
...
#if(MOCNAP)
	gMocNap.LoadFileXML(gPath.GetFullPath("Custom\\CustomTopUpMilestone.xml"));
#endif
...
void CServerInfo::ReadMonsterInfo() // OK
{
	...
	gMonsterSetBase.LoadFolder(gPath.GetFullPath("MonsterSetBase"));
...
#endif
#if(SLOTMACHINE)
	// Here and not earlier: gMonsterSetBase.LoadFolder above has already run, and
	// SetMonsterData (GameMain.cpp) has not - push the NPC in between or it is
	// either wiped by the load or never instantiated.
	gSlotMachine.SetNPC();
#endif
...
// Message.txt and Notice.txt also reload as part of Common and Util
// respectively. These narrower entry points exist so that editing a line of
// text does not force a reload of everything else in those groups.
void CServerInfo::ReadMessageInfo() // OK
{
	gMessage.Load(gPath.GetFullPath("Message.txt"));

	LogAdd(LOG_BLUE,"[ServerInfo] Message loaded successfully");
}

void CServerInfo::ReadNoticeInfo() // OK
{
	gNotice.Load(gPath.GetFullPath("Util\\Notice.txt"));

	LogAdd(LOG_BLUE,"[ServerInfo] Notice loaded successfully");
}
...
void CServerInfo::ReadUtilInfo() // OK
{
	...
	gResetTableVip.Load(gPath.GetFullPath("Util\\ResetTableVIP.xml"));
#else
...
void CServerInfo::ReadChaosMixInfo(char* section,char* path) // OK
{
	...
	this->m_TatBatSoiTCPT = this->GetProfileIntEx(section, "EnableFenrirAttackDefense", "TatBatSoiTCPT", 1, path);
	...
	//Sói vàng
	this->m_TatBatSoiHK = this->GetProfileIntEx(section, "EnableFenrirGold", "TatBatSoiHK", 1, path);
	...
	this->m_MonsterWingMixRate[0] = GetPrivateProfileInt(section, "Wing25MixRate_AL0", 0, path);

	this->m_MonsterWingMixRate[1] = GetPrivateProfileInt(section, "Wing25MixRate_AL1", 0, path);

	this->m_MonsterWingMixRate[2] = GetPrivateProfileInt(section, "Wing25MixRate_AL2", 0, path);

	this->m_MonsterWingMixRate[3] = GetPrivateProfileInt(section, "Wing25MixRate_AL3", 0, path);
...
void CServerInfo::ReadCharacterInfo(char* section,char* path) // OK
{
	...
	this->m_TyLeRaComboDW = this->GetProfileIntEx(section, "ComboProcRateDW", "TyLeRaComboDW", 0, path);
	this->m_TyLeRaComboDK = this->GetProfileIntEx(section, "ComboProcRateDK", "TyLeRaComboDK", 0, path);
	this->m_TyLeRaComboEF = this->GetProfileIntEx(section, "ComboProcRateEF", "TyLeRaComboEF", 0, path);
	this->m_TyLeRaComboMG = this->GetProfileIntEx(section, "ComboProcRateMG", "TyLeRaComboMG", 0, path);
	this->m_TyLeRaComboDL = this->GetProfileIntEx(section, "ComboProcRateDL", "TyLeRaComboDL", 0, path);
	this->m_TyLeRaComboSM = this->GetProfileIntEx(section, "ComboProcRateSM", "TyLeRaComboSM", 0, path); 
	this->m_TyLeRaComboRF = this->GetProfileIntEx(section, "ComboProcRateRF", "TyLeRaComboRF", 0, path);


	//---DW
	this->ChinhDameQuayRongDW = this->GetDivisorEx(section, "EvilSpiritDamageDivisorDW", "ChinhDameQuayRongDW", 100, path);

	this->ChinhDameMuaBangTuyetDW = this->GetDivisorEx(section, "IceStormDamageDivisorDW", "ChinhDameMuaBangTuyetDW", 100, path);

	this->ChinhDameMuaSaoBangDW = this->GetDivisorEx(section, "BlastDamageDivisorDW", "ChinhDameMuaSaoBangDW", 100, path);

	this->ChinhDameCotLuaDW = this->GetDivisorEx(section, "FlameDamageDivisorDW", "ChinhDameCotLuaDW", 100, path);

	this->ChinhDameMuaDocToDW = this->GetDivisorEx(section, "DecayDamageDivisorDW", "ChinhDameMuaDocToDW", 100, path);

	this->ChinhDameMuaDocDW = this->GetDivisorEx(section, "PoisonDamageDivisorDW", "ChinhDameMuaDocDW", 100, path);
	//---

	//---MG
	this->ThoiGianLamChamPhayMG = this->GetProfileIntEx(section, "PowerSlashSlowDurationMG", "ThoiGianLamChamPhayMG", 0, path);

	this->TiLeLamChamPhayMG = this->GetProfileIntEx(section, "PowerSlashSlowRateMG", "TiLeLamChamPhayMG", 0, path);

	this->ChinhDamePhayMG = this->GetDivisorEx(section, "PowerSlashDamageDivisorMG", "ChinhDamePhayMG", 0, path);

	this->ChinhDameHoaDiemMG = this->GetDivisorEx(section, "SwordSlashDamageDivisorMG", "ChinhDameHoaDiemMG", 100, path);

	this->ChinhDameChemLuaMG = this->GetDivisorEx(section, "FireSlashDamageDivisorMG", "ChinhDameChemLuaMG", 100, path);

	this->ChinhDameBaoDienMG = this->GetDivisorEx(section, "LightningStormDamageDivisorMG", "ChinhDameBaoDienMG", 100, path);

	this->ChinhDameQuayRongMG = this->GetDivisorEx(section, "EvilSpiritDamageDivisorMG", "ChinhDameQuayRongMG", 100, path);

	this->ChinhDameCotLuaMG = this->GetDivisorEx(section, "FlameDamageDivisorMG", "ChinhDameCotLuaMG", 100, path);

	this->ChinhDameMuaDocToMG = this->GetDivisorEx(section, "DecayDamageDivisorMG", "ChinhDameMuaDocToMG", 100, path);

	this->ChinhDameMuaDocMG = this->GetDivisorEx(section, "PoisonDamageDivisorMG", "ChinhDameMuaDocMG", 100, path);

	this->ChinhDameMuaSaoBangMG = this->GetDivisorEx(section, "BlastDamageDivisorMG", "ChinhDameMuaSaoBangMG", 100, path);

	//---DK
	this->ChinhDameXoayKiemDK = this->GetDivisorEx(section, "TwistingSlashDamageDivisorDK", "ChinhDameXoayKiemDK", 100, path);

	this->ChinhDameDamGioDK = this->GetDivisorEx(section, "DeathStabDamageDivisorDK", "ChinhDameDamGioDK", 100, path);

	this->ChinhDameChemBangDK = this->GetDivisorEx(section, "FrozenStabDamageDivisorDK", "ChinhDameChemBangDK", 100, path);

	//---DL
	this->ChinhDameHoaXichLongDL = this->GetDivisorEx(section, "FireBurstDamageDivisorDL", "ChinhDameHoaXichLongDL", 100, path);

	this->ChinhDameHonLoanDL = this->GetDivisorEx(section, "ChaoticDiseierDamageDivisorDL", "ChinhDameHonLoanDL", 100, path);

	this->ChinhDameHoaAmDL = this->GetDivisorEx(section, "FireScreamDamageDivisorDL", "ChinhDameHoaAmDL", 100, path);

	//---ELF
	this->ChinhDameNguTienFE = this->GetDivisorEx(section, "FiveShotDamageDivisorFE", "ChinhDameNguTienFE", 100, path);

	this->ChinhDameBanBangFE = this->GetDivisorEx(section, "IceArrowDamageDivisorFE", "ChinhDameBanBangFE", 100, path);

	this->ChinhDameXuyenTamTienFE = this->GetDivisorEx(section, "PenetrationDamageDivisorFE", "ChinhDameXuyenTamTienFE", 100, path);

	//---

	//---RF
	this->ChinhDameVatNgaRF = GetPrivateProfileInt(section, "ChinhDameVatNgaRF", 100, path);
	this->ChinhDameAoAnhLienHoanRF = GetPrivateProfileInt(section, "ChinhDameAoAnhLienHoanRF", 100, path);
	//---

	//---SUM
	this->ChinhDameSetLienHoanSUM = this->GetDivisorEx(section, "ChainLightningDamageDivisorSUM", "ChinhDameSetLienHoanSUM", 100, path);

	this->ChinhDameQuaCauDienSUM = this->GetDivisorEx(section, "ElectricSurgeDamageDivisorSUM", "ChinhDameQuaCauDienSUM", 100, path);

	this->ChinhDameSocDienSUM = this->GetDivisorEx(section, "RedStormDamageDivisorSUM", "ChinhDameSocDienSUM", 100, path);

	this->ChinhDameHutMauSUM = this->GetDivisorEx(section, "DrainLifeDamageDivisorSUM", "ChinhDameHutMauSUM", 100, path);

	//---

	this->m_DuelDamageRate = GetPrivateProfileInt(section,"DuelDamageRate",100,path);

	#if(GAMESERVER_UPDATE>=501)

	this->m_GensDamageRate = GetPrivateProfileInt(section,"GensDamageRate",100,path);
	...
	this->m_AttackSpeedAgiStep = GetPrivateProfileInt(section,"AttackSpeedAgiStep",0,path);

	this->m_AttackSpeedPerStep = GetPrivateProfileInt(section,"AttackSpeedPerStep",0,path);
	...
	// Maximum attack speed caps. Default 0 = no cap (keeps old behaviour when
	// the keys are absent from the ini).
	this->m_DWMaxAttackPhysiSpeed = GetPrivateProfileInt(section,"DWMaxAttackPhysiSpeed",0,path);

	this->m_DKMaxAttackPhysiSpeed = GetPrivateProfileInt(section,"DKMaxAttackPhysiSpeed",0,path);

	this->m_FEMaxAttackPhysiSpeed = GetPrivateProfileInt(section,"FEMaxAttackPhysiSpeed",0,path);

	this->m_MGMaxAttackPhysiSpeed = GetPrivateProfileInt(section,"MGMaxAttackPhysiSpeed",0,path);

	this->m_DLMaxAttackPhysiSpeed = GetPrivateProfileInt(section,"DLMaxAttackPhysiSpeed",0,path);

	this->m_SUMaxAttackPhysiSpeed = GetPrivateProfileInt(section,"SUMaxAttackPhysiSpeed",0,path);

	this->m_RFMaxAttackPhysiSpeed = GetPrivateProfileInt(section,"RFMaxAttackPhysiSpeed",0,path);

	this->m_DWMaxAttackMagicSpeed = GetPrivateProfileInt(section,"DWMaxAttackMagicSpeed",0,path);

	this->m_DKMaxAttackMagicSpeed = GetPrivateProfileInt(section,"DKMaxAttackMagicSpeed",0,path);

	this->m_FEMaxAttackMagicSpeed = GetPrivateProfileInt(section,"FEMaxAttackMagicSpeed",0,path);

	this->m_MGMaxAttackMagicSpeed = GetPrivateProfileInt(section,"MGMaxAttackMagicSpeed",0,path);

	this->m_DLMaxAttackMagicSpeed = GetPrivateProfileInt(section,"DLMaxAttackMagicSpeed",0,path);

	this->m_SUMaxAttackMagicSpeed = GetPrivateProfileInt(section,"SUMaxAttackMagicSpeed",0,path);

	this->m_RFMaxAttackMagicSpeed = GetPrivateProfileInt(section,"RFMaxAttackMagicSpeed",0,path);
...
void CServerInfo::ReadCommonInfo(char* section,char* path) // OK
{
	...
	this->m_ItemDropRateVIPBonus[0] = GetPrivateProfileInt(section,"ItemDropRateVIPBonus_AL0",0,path);

	this->m_ItemDropRateVIPBonus[1] = GetPrivateProfileInt(section,"ItemDropRateVIPBonus_AL1",0,path);

	this->m_ItemDropRateVIPBonus[2] = GetPrivateProfileInt(section,"ItemDropRateVIPBonus_AL2",0,path);

	this->m_ItemDropRateVIPBonus[3] = GetPrivateProfileInt(section,"ItemDropRateVIPBonus_AL3",0,path);
	...
	this->m_PickUpSwitch		= GetPrivateProfileInt(section,"PickUpSwitch",0,path);

	GetPrivateProfileString(section, "PK_ColorMessPVP", "FFFFFFFF", this->PK_ColorMessPVP, sizeof(this->PK_ColorMessPVP), path);
	GetPrivateProfileString(section, "Pick_ColorMess", "FFFFFFFF", this->PK_ColorMessPick, sizeof(this->PK_ColorMessPick), path);
...
// Turns the client's displayed build string into the number it reports.
//
// Accepts "1.0.DDMMYY.HHMM", with or without a leading "v." - both spellings
// appear, since the client shows "v.1.0.190926.1943". Anything else, including
// an empty value or a plain "0", returns 0, which every caller reads as
// "not set".
//
// The stamp is ordered DDMMYY.HHMM because that is how a person writes a date;
// the comparable number is YYMMDDHHMM because that is the order in which larger
// means newer. This function is the one place that conversion happens, so the
// two orderings cannot be confused anywhere else.
//
// Returns unsigned: YYMMDDHHMM reaches ~2.6 billion, past the signed maximum.
DWORD ParseClientBuildVersion(const char* szValue) // OK
{
	...
	// Collect the dot-separated pieces, ignoring anything that is not digits -
	// which is how a leading "v" is skipped without special-casing it.
	int  numbers[8] = { 0 };
	int  digits[8] = { 0 };
	int  count = 0;

	for (const char* p = szValue; count < 8; p++)
	{
		if (*p >= '0' && *p <= '9')
		{
			numbers[count] = (numbers[count] * 10) + (*p - '0');
			digits[count]++;
			continue;
		}
	...
	for (int n = 0; n + 1 < count; n++)
	{
		...
		if (dd < 1 || dd > 31 || mm < 1 || mm > 12 || hh > 23 || mi > 59)
		{
			// Looks like the right shape but is not a date. Refusing to guess -
			// returning 0 leaves the gate off, which is the safe direction.
			return 0;
		}

		return (yy * 100000000u) + (mm * 1000000u) + (dd * 10000u) + (hh * 100u) + mi;
	}
...
void CServerInfo::ReadCustomInfo(char* section,char* path) // OK
{
	...
	this->m_CustommixThanhCong = this->GetProfileIntEx(section, "CustomMixSuccess", "CustommixThanhCong", 0, path);
	this->m_CustommixThatBai = this->GetProfileIntEx(section, "CustomMixFail", "CustommixThatBai", 0, path);
	...
	this->GioiHanTyLeX2SatThuong = this->GetProfileIntEx(section, "LimitDoubleDamageRate", "GioiHanTyLeX2SatThuong", 100, path);
	this->GioiHanTyLeX3SatThuong = this->GetProfileIntEx(section, "LimitTripleDamageRate", "GioiHanTyLeX3SatThuong", 100, path);
	this->m_GioiHanSatThuongChiMang = this->GetProfileIntEx(section, "LimitCriticalDamage", "GioiHanSatThuongChiMang", 100, path);
	...
	this->m_DefenseConstant = GetPrivateProfileInt(section, "DefenseConstant", 2000, path);
	this->m_DarkLordAreaSkillPvP = GetPrivateProfileInt(section, "DarkLordAreaSkillPvP", 1, path);
	this->ColorDamex2 = GetPrivateProfileInt(section, "ColorDamex2", 0, path);
	this->ColorDamex3 = GetPrivateProfileInt(section, "ColorDamex3", 0, path);
	...
	this->m_CustomCashShop				=  GetPrivateProfileInt(section,"CustomCashShop",0,path);
	...
	this->m_EnableBuffParty = GetPrivateProfileInt(section, "EnableBuffParty", 1, path);
	this->m_ElfBuffParty = GetPrivateProfileInt(section, "ElfBuffParty", 0, path);
	...
	char WeaponExlOption[128] = { 0 };
	char AccessoryExlOption[128] = { 0 };
	for (int n = 0; n < 6; n++)
	{
		...
		wsprintf(WeaponExlOption, "WeaponExlOption_%02d", n);
		this->NewVKDrop[n] = this->GetProfileIntEx(section, WeaponExlOption, VKExlOption, 0, path);

		wsprintf(PhuKienExlOption, "PhuKienExlOption_%02d", n);
		wsprintf(AccessoryExlOption, "AccessoryExlOption_%02d", n);
		this->NewPhuKienDrop[n] = this->GetProfileIntEx(section, AccessoryExlOption, PhuKienExlOption, 0, path);
	}
...
{
	this->GHRSMax = GetPrivateProfileInt("CONFIG", "GHRSMax", 10, "./GHRSReset.ini");
	//=======================================================================================>
	this->ResetAdd = GetPrivateProfileInt("CONFIG", "ResetAdd", 10, "./GHRSReset.ini");
	this->DayCache = GetPrivateProfileInt("CONFIG", "DayCache", 0, "./GHRSReset.ini");
	this->MonCache = GetPrivateProfileInt("CONFIG", "MonCache", 0, "./GHRSReset.ini");
	this->YearCache = GetPrivateProfileInt("CONFIG", "YearCache", 0, "./GHRSReset.ini");
...
{
	if (!this->LoadGHRSOK) return;

	time_t ltime;
	time(&ltime);
	TimeServer = localtime(&ltime);

	if (TimeServer->tm_mday != this->DayCache || (TimeServer->tm_mon + 1) != this->MonCache || (TimeServer->tm_year + 1900 != this->YearCache)) // nếu 00 giây
	{
		this->GHRSMax += this->ResetAdd; // thì tăng giới hạn thêm 100 nữa
		this->DayCache = TimeServer->tm_mday;
		this->MonCache = (TimeServer->tm_mon + 1);
		this->YearCache = (TimeServer->tm_year + 1900);
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(2000), this->GHRSMax);
		LogAdd(LOG_BLUE, gMessage.GetMessage(2000), this->GHRSMax);
		char string[10];
		WritePrivateProfileStringA("CONFIG", "GHRSMax", itoa(this->GHRSMax, string, 10), "./GHRSReset.ini");
		WritePrivateProfileStringA("CONFIG", "ResetAdd", itoa(this->ResetAdd, string, 10), "./GHRSReset.ini");
		WritePrivateProfileStringA("CONFIG", "DayCache", itoa(this->DayCache, string, 10), "./GHRSReset.ini");
		WritePrivateProfileStringA("CONFIG", "MonCache", itoa(this->MonCache, string, 10), "./GHRSReset.ini");
		WritePrivateProfileStringA("CONFIG", "YearCache", itoa(this->YearCache, string, 10), "./GHRSReset.ini");
		return;
...
int CServerInfo::GetProfileIntEx(char* section, const char* key, const char* legacyKey, int defaultValue, char* path) // OK
{
	char buff[64];

	// Probe for the English key. GetPrivateProfileString returns an empty
	// string when the key is missing, which is how we tell "absent" apart from
	// a legitimately configured value (including negative ones).
	GetPrivateProfileStringA(section, key, "", buff, sizeof(buff), path);
	...
	return GetPrivateProfileInt(section, legacyKey, defaultValue, path);
}

int CServerInfo::GetDivisorEx(char* section, const char* key, const char* legacyKey, int defaultValue, char* path) // OK
{
	int value = this->GetProfileIntEx(section, key, legacyKey, defaultValue, path);

	// These values are used as "damage = (damage * X) / value". A configured 0
	// would be an integer division by zero and take the server down, so clamp.
	if (value <= 0)
	{
		value = 1;
	}
...
void CServerInfo::ReadCustomConfig(char* section, char* path) // OK
{
#if FIXTELE
	gBCustomEdit.Load();
#endif
	this->m_ThoiGianThoatGame = this->GetProfileIntEx(section, "GameExitTime", "ThoiGianThoatGame", 6, path);
	this->ThoiGianDameDocDW = this->GetProfileIntEx(section, "PoisonDamageDurationDW", "ThoiGianDameDocDW", 5, path);
	this->SkillDocDW		= this->GetProfileIntEx(section, "PoisonSkillRateDW", "SkillDocDW", 0, path);
	this->TyLeRaChoangSkillMasterDamGio = this->GetProfileIntEx(section, "DeathStabStunRate", "TyLeRaChoangSkillMasterDamGio", 10, path);
	this->ThoiGianChoangSkillMasterDamGio = this->GetProfileIntEx(section, "DeathStabStunDuration", "ThoiGianChoangSkillMasterDamGio", 10, path);
	this->m_TatQuayDo = this->GetProfileIntEx(section, "DisableItemUpgradeAbove", "TatQuayDo", 15, path);
	this->m_CungGuildPK = this->GetProfileIntEx(section, "AllowSameGuildPK", "CungGuildPK", 0, path);
	this->m_PKLienMinh = this->GetProfileIntEx(section, "AllowAlliancePK", "PKLienMinh", 0, path);
#if(CB_ATTACK_HIDEN_PET)
	this->m_HidenPetIsAttack = GetPrivateProfileInt(section, "HidenPetIsAttack", 0, path);
#endif
#if(CB_GETMIXRATE)
	this->m_UseZenRateReal = GetPrivateProfileInt(section, "UseZenRateReal", 1, path);
#endif
	this->ElfNoBoltArrow = GetPrivateProfileInt(section, "ElfNoBoltArrow", 0, path);
#if(CB_ATTACK_HIDEN_WING)
	this->m_HidenWingIsAttack = GetPrivateProfileInt(section, "HidenWingIsAttack", 0, path);
#endif
	this->TyLeRaChoangSkillMasterNguTien = this->GetProfileIntEx(section, "FiveShotStunRate", "TyLeRaChoangSkillMasterNguTien", 0, path);
	this->ThoiGianChoangSkillMasterNguTien = this->GetProfileIntEx(section, "FiveShotStunDuration", "ThoiGianChoangSkillMasterNguTien", 0, path);
	
    this->m_TimeGiuBoss = this->GetProfileIntEx(section, "TimeKeepDamageBoss", "TimeGiuDmgBoss", 0, path);
	this->m_TimeGiuBossTop = this->GetProfileIntEx(section, "TimeKeepDamageTop", "TimeGiuDmgTop", 0, path);
	this->m_TimeGiuBossParty = this->GetProfileIntEx(section, "TimeKeepDamageParty", "TimeGiuDmgParty", 0, path);
	this->m_TimeTopHitDmg = GetPrivateProfileInt(section, "TimeTopHitDmg", 0, path);
	this->m_TimeTop5Hit = GetPrivateProfileInt(section, "TimeTop5HitDmg", 0, path);
	this->m_TinhDameBoss = this->GetProfileIntEx(section, "UseTotalDamage", "TinhDameTong", 0, path);

	// Combo always push and show effect after kill - see FixSkillGiatLui()
	// (BCustomEdit.cpp) and CSkillManager::ApplyEarthquakeEffect()
	// (SkillManager.cpp), both otherwise gated by a random roll.
	this->m_CommboEffectEnable = GetPrivateProfileInt(section, "CommboEffectEnable", 0, path);

	// DarkHorsePushBackChance (CustomConfig.ini): independent 0-100 chance
	// for the Dark Horse stomp (Earthquake) to physically knock the target
	// back - separate from CommboEffectEnable's stun-effect display roll.
	// 100 = guaranteed push back.
	this->m_DarkHorsePushBackChance = GetPrivateProfileInt(section, "DarkHorsePushBackChance", 0, path);

	// CustomComboDistance / CustomDarkHorsePushDistance (CustomConfig.ini):
	// how many tiles the guaranteed pushes above knock the target back.
	// Defaults match the previous hardcoded values (1 and 3) so leaving
	// these unset keeps the old distance.
	this->m_CustomComboDistance = GetPrivateProfileInt(section, "CustomComboDistance", 1, path);
	this->m_CustomDarkHorsePushDistance = GetPrivateProfileInt(section, "CustomDarkHorsePushDistance", 3, path);

	// ItemDropLog (CustomConfig.ini): 1 turns on the [ItemLog] ground-item
	// lifecycle trace in Map.cpp - a DROP line when an item hits the ground,
	// and a matching PICKUP or EXPIRE when it leaves. Added to chase invisible
	// ground items: every line carries map/slot/serial, so a slot that gets a
	// second DROP with no PICKUP/EXPIRE in between was reused underneath a
	// player's viewport, and a slot with no closing line was never cleaned up.
	// Off by default - it is one console line per drop.
	this->m_ItemDropLog = GetPrivateProfileInt(section, "ItemDropLog", 0, path);

	// ItemPickupNotice (CustomConfig.ini): master switch for the server-wide
	// "picked up [item] at map [x]" broadcast. Custom\CustomNoticeItem.txt
	// already has its own Enable column, but that file also carries the whole
	// item list, so this puts the on/off next to the other gameplay toggles.
	// Defaults to 1 so an ini predating this key keeps the notice it had.
	this->m_ItemPickupNotice = GetPrivateProfileInt(section, "ItemPickupNotice", 1, path);

	// PvPDamageRamp* (CustomConfig.ini): see the header for the mechanic.
	// Percent defaults to 0 - OFF - so dropping this build onto a server whose
	// ini predates these keys changes no combat maths until the keys are added
	// deliberately. A new damage multiplier is not something to switch itself
	// on during a deploy.
	this->m_PvPDamageRampSeconds = GetPrivateProfileInt(section, "PvPDamageRampSeconds", 15, path);
	this->m_PvPDamageRampPercent = GetPrivateProfileInt(section, "PvPDamageRampPercent", 0, path);
	this->m_PvPDamageRampMaxPercent = GetPrivateProfileInt(section, "PvPDamageRampMaxPercent", 200, path);
	this->m_PvPDamageRampResetSeconds = GetPrivateProfileInt(section, "PvPDamageRampResetSeconds", 10, path);

	// A zero or negative interval would divide by zero in the step count, and
	// a negative percent would heal the target. Both mean "off".
	if (this->m_PvPDamageRampSeconds <= 0 || this->m_PvPDamageRampPercent < 0)
	{
		this->m_PvPDamageRampPercent = 0;
	}

	if (this->m_PvPDamageRampMaxPercent < 0)
	{
		this->m_PvPDamageRampMaxPercent = 0;
	}

	if (this->m_PvPDamageRampResetSeconds <= 0)
	{
		this->m_PvPDamageRampResetSeconds = 10;
	}

	// PvPReflect* (CustomConfig.ini): reflect damage fading out over a long
	// fight. Percent defaults to 0 - OFF - for the same reason as the ramp
	// above: deploying a build should not quietly change combat maths.
	this->m_PvPReflectDecaySeconds = GetPrivateProfileInt(section, "PvPReflectDecaySeconds", 15, path);
	this->m_PvPReflectDecayPercent = GetPrivateProfileInt(section, "PvPReflectDecayPercent", 0, path);
	this->m_PvPReflectMinPercent = GetPrivateProfileInt(section, "PvPReflectMinPercent", 10, path);
	this->m_PvPReflectResetSeconds = GetPrivateProfileInt(section, "PvPReflectResetSeconds", 10, path);

	if (this->m_PvPReflectResetSeconds <= 0)
	{
		this->m_PvPReflectResetSeconds = 10;
	}

	// A zero or negative interval would divide by zero in the step count.
	if (this->m_PvPReflectDecaySeconds <= 0 || this->m_PvPReflectDecayPercent < 0)
	{
		this->m_PvPReflectDecayPercent = 0;
	}

	// The floor is a percentage of full reflect, so it has to sit in 0..100 -
	// above 100 it would turn the decay into a permanent buff.
	if (this->m_PvPReflectMinPercent < 0)
	{
		this->m_PvPReflectMinPercent = 0;
	}

	if (this->m_PvPReflectMinPercent > 100)
	{
		this->m_PvPReflectMinPercent = 100;
	}

	// VoiceChat* (CustomConfig.ini): off by default, like every other feature
	// added here - a new UDP talker should not appear because someone deployed
	// a build.
	this->m_VoiceChatEnable = GetPrivateProfileInt(section, "VoiceChatEnable", 0, path);
	this->m_VoiceChatPort = GetPrivateProfileInt(section, "VoiceChatPort", 63004, path);

	GetPrivateProfileString(section, "VoiceChatHost", "127.0.0.1",
		this->m_VoiceChatHost, sizeof(this->m_VoiceChatHost), path);

	// Falls back to whatever the server uses, which is right when the service
	// is on a public address and wrong-but-obvious when it is 127.0.0.1:
	// players simply get no voice, rather than something subtly broken.
	GetPrivateProfileString(section, "VoiceChatClientHost", this->m_VoiceChatHost,
		this->m_VoiceChatClientHost, sizeof(this->m_VoiceChatClientHost), path);

	// MinClientBuild* (CustomConfig.ini): the outdated-client gate.
	//
	// Written the way the client displays it - 1.0.DDMMYY.HHMM, the same string
	// in the PC window title and the Android status line - so what you type is
	// what you read off a client, with no reordering to get wrong.
	//
	// Read as strings, not GetPrivateProfileInt: the comparable form is
	// YYMMDDHHMM, which reaches ~2.6 billion and does not fit a signed int.
	//
	// Both default to empty, which is off, so deploying this build cannot start
	// refusing anybody - the gate has to be switched on deliberately.
	{
		char szMinBuild[64] = { 0 };
	...
	// single Regular Combination. Defaults to MAX_TALISMAN_OF_LUCK, the value
	// that used to be compiled in, so an ini predating this key behaves as
	// before. Clamped to at least 0 - a negative cap would refuse every mix.
	this->m_MaxTalismanOfLuck = GetPrivateProfileInt(section, "MaxTalismanOfLuck", MAX_TALISMAN_OF_LUCK, path);

	if (this->m_MaxTalismanOfLuck < 0)
	{
		this->m_MaxTalismanOfLuck = 0;
	}

	// ShowResetLimitOnLogin (CustomConfig.ini): 1 shows the "Today's reset limit
	// is N times, <name>!" notice on login, 0 hides it. Defaults to 0 because the
