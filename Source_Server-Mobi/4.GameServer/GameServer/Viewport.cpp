...
bool CViewport::AddViewportObjectItem(int aIndex,int bIndex,int type) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		if(lpObj->VpPlayerItem[n].state == VIEWPORT_NONE)
		{
			...
			// Remember which item currently occupies this map slot, so
			// DestroyViewportItem can tell a slot reuse apart from "still
			// the same item" even if it never observes an intervening
			// empty/DIECMD/out-of-range state (see DestroyViewportItem).
			lpObj->VpPlayerItemSerial[n] = gMap[lpObj->Map].m_Item[bIndex].m_Serial;
			lpObj->VPCountItem++;
			return 1;
		}
	}
...
void CViewport::DestroyViewportItem(int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		// The slot is still "live" and in range, but may have been freed and
		// reused by a completely different drop since this player's tracking
		// entry was set (slot reuse happens immediately off the drop/pickup
		// path, while this runs once per second - see MonsterItemDrop/
		// ItemDrop in Map.cpp). None of the checks above catch that case
		// because the slot never passes through empty/DIECMD/out-of-range
		// from this player's point of view. Left undetected, this player's
		// entry never leaves VIEWPORT_SEND/WAIT, so GCViewportItemSend never
		// sends them a create packet for the new occupant - while pickup
		// (CheckItemGive/ItemGive) reads the slot's real current contents
		// with no reference to viewport state, so they can still loot it:
		// pickable and audible, but never rendered.
		if(lpMapItem->m_Serial != lpObj->VpPlayerItemSerial[n])
		{
			lpObj->VpPlayerItem[n].state = VIEWPORT_DESTROY;
			continue;
		}
