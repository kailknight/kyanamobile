...
public:
	...
	// Password-based join, shared by the /join command and the older
	// whisper-the-password route so the two can never drift apart.
	// lpHost is whoever set the password with /setpass and ends up leading.
	void JoinPartyByPassword(LPOBJ lpJoiner,LPOBJ lpHost);
