...
{
	// Watchdog against a corrupt/infinite file - was 1000ms, which is easy
	// to blow through on a larger data file (raised as part of tracking
	// down MapManager.txt only partially loading), especially under
	// per-call overhead from this build's protection layer. A throw here
	// silently truncates whatever the caller was loading (the whole parse
	// loop shares one try/catch), so this budget is generous on purpose.
	if((GetTickCount()-this->m_tick) > 15000)
	{
		this->SetLastError(4);
		throw 1;
	}
