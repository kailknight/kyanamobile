...
// All three variants carry the same name so flipping GAMESERVER_NOMBRE can
// never resurrect a previous owner's branding. GAMESERVER_CLIENT feeds the
// startup title bar (GameServer.cpp), the 2-second title refresh and the big
// 50px banner across the top of the window (both ServerDisplayer.cpp).
#if(GAMESERVER_NOMBRE == 0)
	#define GAMESERVER_CLIENT "KYANa Emulator"
...
// 5x3 slot machine NPC. Off means the feature compiles out entirely, including
// its NPC hook and its two packet cases.
#define SLOTMACHINE					1
