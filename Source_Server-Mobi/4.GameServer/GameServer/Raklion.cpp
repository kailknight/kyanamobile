...
// Feeds the "Selupan" row of the in-game event schedule window (H).
//
// Raklion runs off player progress, not a clock: IDLE ends when every boss egg
// has been destroyed, so there is no "opens at" time to count down to. The one
// interval that IS timed is the window after Selupan dies (NOTIFY4), which runs
// for BossZoneOpenDelay seconds before the eggs respawn and the cycle can start
// over - that is the "when can we do this again" the schedule window is being
// asked for. Every other state reports 0, which the client renders as "Online".
void CRaklion::UpdateEventTimeDisplay() // OK
{
	if(this->m_RaklionState != RAKLION_STATE_NOTIFY4)
	{
		gServerDisplayer.EventSelupan = 0;
		return;
	}

	int elapsed = (int)((GetTickCount()-(DWORD)this->m_BossZoneOpenMSec)/1000);

	int remain = (this->m_BossZoneOpenDelay-elapsed);

	// never report 0 while the boss is not actually up - the client renders 0 as "Online"
	gServerDisplayer.EventSelupan = ((remain<=0)?1:remain);
}
