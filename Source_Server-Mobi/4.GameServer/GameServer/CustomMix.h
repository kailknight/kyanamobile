...
struct PMSG_BCUSTOM_MIX_INFO
{
	...
	// gServerInfo.m_MaxTalismanOfLuck, so the mix window can state the cap
	// instead of the client carrying its own copy that drifts from the ini.
	// Appended at the END of the fixed header on purpose: the trailing
	// ingredient rows are addressed as sizeof(PMSG_BCUSTOM_MIX_INFO) + n *
	// sizeof(BCUSTOM_MIX_NGUYENLIEU) on both sides, so client and server have
	// to be rebuilt together for this packet - they already are.
