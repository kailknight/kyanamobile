...
#if(CB_NEWQUESTFF)
	if (gCB_NewQuest.CheckNpcQuest(lpObj,lpNpc) != 0)
	{
		return 1;
	}
#endif
...
#if(SLOTMACHINE)
	// Before the vanilla switch below, so the machine's NPC class does not also
	// have to be absent from it. Returns 1 to consume the click; returning 0 would
	// fall through and open the generic shop window over our own.
	if (gSlotMachine.Dialog(lpObj,lpNpc) != 0)
	{
		return 1;
	}
#endif
...
void CNpcTalk::CGNpcTalkCloseRecv(int aIndex) // OK
{
	...
	switch(lpObj->Interface.type)
	{
		...
		case INTERFACE_QUEST_WORLD:
			...
			break;
#if(SLOTMACHINE)
		case INTERFACE_SLOTMACHINE:
			// Clears the open flag only. Free spins and held winnings deliberately
			// survive closing the window - both were won, and both persist.
			gSlotMachine.CloseMachine(aIndex);
			break;
#endif
	}
	if (lpObj->Transaction == 1)
