...
enum eCommandNumber
{
	...
	COMMAND_SETSTATS		= 86,
	COMMAND_SETML			= 87,
	// Command.txt is 0-based against this 1-based enum (see CCommandManager::Init,
	// which does Add(gCommand.GetCommand(n), n+1)), so /join lives at INDEX 87
	// in Command.txt. Get that off by one and the label binds to the neighbouring
	// handler with no error at all.
	COMMAND_JOINPARTY		= 88,
};
...
public:
	...
	bool CommandJoinParty(LPOBJ lpObj, char* arg);
	bool CommandSetLevel(LPOBJ lpObj, char* arg);
	bool CommandSetReset(LPOBJ lpObj, char* arg);
	bool CommandSetStats(LPOBJ lpObj, char* arg);
	bool CommandSetMasterLevel(LPOBJ lpObj, char* arg);
private:
