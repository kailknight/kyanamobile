...
// Same as LogAdd but routes the line to one of the log panel's category views
// (eLogCategory in ServerDisplayer.h) instead of the default Common view.
// Screen only - it writes no file, so use it for lines that either have their
// own gLog.Output call already or do not want one.
void LogAddCat(int category,eLogColor color,char* text,...);
...
void PostMessagePK(char* name,char* message,char* text,char* MapName);
void PostMessageUserON(char* name,char* message);
void PostMessageItemNotice(char* name, char* message);
