	...
	switch(head)
	{
		...
		case 0xD2:
			switch(lpMsg[3])
			{
				...
				case 0x23:
					#if(GAMESERVER_UPDATE>=501)
					gCashShop.CGCashShopCatalogReqRecv((PMSG_CASH_SHOP_CATALOG_REQ_RECV*)lpMsg,aIndex);
					#endif
					break;
			}
			break;
		...
		case 0xD3:
			...
			case 0xA0: //Redeem code - "Check Code" (non-binding preview)
			{
				...
				mlpMsg->Code[sizeof(mlpMsg->Code) - 1] = '\0'; // never trust the wire to have terminated it
				gRedeemCode.RequestPreview(aIndex, mlpMsg->Code);
			}
			break;
			case 0xA2: //Redeem code - "Redeem" (confirm)
			{
				PMSG_REDEEM_CODE_SEND* mlpMsg = (PMSG_REDEEM_CODE_SEND*)lpMsg;
				mlpMsg->Code[sizeof(mlpMsg->Code) - 1] = '\0';
				gRedeemCode.RequestRedeem(aIndex, mlpMsg->Code);
			}
			break;
#endif
			// Client build report. Arrives before the login packet, which is
			// what lets CGConnectAccountRecv gate on it - see
			// PMSG_CLIENT_BUILD_RECV.
			case 0x7D:
			{
				CGClientBuildRecv((PMSG_CLIENT_BUILD_RECV*)lpMsg, size, aIndex);
			}
			break;
			...
			case 0x8C: //OpenWIndow
			{
				...
				// Push the claim box along with the wheel data, so opening the
				// window always shows what is actually being held.
				gCustomVongQuay.SendClaimList(lpObj->Index);
			}
			break;

			case 0x8E: //Spin wheel claim box - claim one slot, or 0xFF for all
			{
				gCustomVongQuay.CGClaimRecv(lpObj->Index,lpMsg);
			}
			break;

#if(SLOTMACHINE)
			case 0x90: //Slot machine - spin
			{
				gSlotMachine.CGSpinRecv(lpObj->Index,lpMsg);
			}
			break;

			case 0x92: //Slot machine reward box - claim one row, or 0xFF for all
			{
				gSlotMachine.CGClaimRecv(lpObj->Index,lpMsg);
			}
			break;
#endif
...
void CGChatWhisperRecv(PMSG_CHAT_WHISPER_RECV* lpMsg,int aIndex) // OK
{
	...
	if (strcmp(lpTarget->AutoPartyPassword, lpMsg->message) == 0)
	{
		...
		// Body moved to CParty::JoinPartyByPassword so /join runs the same code.
		gParty.JoinPartyByPassword(lpObj, lpTarget);
	}
}
...
// Records what the client says it is. Deliberately does NOT refuse anything
// here: this packet arrives before the account name is known, so a refusal at
// this point could not be logged against anybody and the client has no screen
// up yet to show it. The gate is applied at login, in CGConnectAccountRecv.
void CGClientBuildRecv(PMSG_CLIENT_BUILD_RECV* lpMsg,int size,int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	// Size-checked because this is the one packet whose absence is meaningful:
	// a truncated or spoofed short one must leave the numbers at 0 - refused -
	// rather than read past the buffer for a value that would let it through.
	if(size < (int)sizeof(PMSG_CLIENT_BUILD_RECV))
	{
		gLog.Output(LOG_HACK,"[SECURITY][GameServer] Short client build packet (Index: %d, Ip: %s, Size: %d)",aIndex,lpObj->IpAddr,size);
		return;
	}

	// First one wins. Otherwise a client could report an acceptable build and
	// then send whatever it liked afterwards, and a replayed packet could
	// overwrite what was already established.
	if(lpObj->m_ClientBuildReported != false)
	{
		return;
	}

	lpObj->m_ClientBuildReported = true;
	lpObj->m_ClientPlatform = lpMsg->Platform;
	lpObj->m_ClientBuild = lpMsg->Build;
}
...
void CGConnectAccountRecv(PMSG_CONNECT_ACCOUNT_RECV* lpMsg,int aIndex) // OK
{
	...
	// Outdated client gate. See ServerInfo.h for MinClientBuildAndroid and why
	// there is no PC equivalent.
	//
	// Result 6 is the existing "wrong version" answer, which the client already
	// presents as a version error telling the player to update - so this needs no
	// new client-side handling, which is what makes it usable against clients
	// that predate the build packet.
	//
	// Two refusals, and the first is the one that catches clients predating this
	// feature - which is most of what "outdated" means in practice.
	if(gServerInfo.m_MinClientBuildAndroid > 0 || gServerInfo.m_MinClientBuildPC > 0)
	{
		// Said nothing at all. Every client new enough to know about this reports
		// something, so silence means one thing: older than the feature. It
		// cannot be waved through - it is exactly what the gate is for - and it
		// cannot be attributed to a platform either, since the report is what
		// would have told us which one it is.
		//
		// This is why setting EITHER minimum refuses non-reporting clients on
		// BOTH platforms. Ship both new clients before switching either on.
		if(lpObj->m_ClientBuildReported == false)
		{
			gLog.Output(LOG_HACK,"[SECURITY][GameServer] Client too old to report a build refused (Index: %d, Ip: %s)",
				aIndex,lpObj->IpAddr);
			GCConnectAccountSend(aIndex,6);
			return;
		}

		// Reported, and behind its platform's minimum. Both sides are YYMMDDHHMM
		// now, so the comparison is the same on either platform - only which key
		// applies differs.
		const DWORD minBuild = (lpObj->m_ClientPlatform == 1)
			? gServerInfo.m_MinClientBuildAndroid
			: gServerInfo.m_MinClientBuildPC;

		if(minBuild > 0 && lpObj->m_ClientBuild < minBuild)
		{
			gLog.Output(LOG_HACK,"[SECURITY][GameServer] Outdated client refused (Index: %d, Ip: %s, Platform: %d, Build: %u, Minimum: %u)",
				aIndex,lpObj->IpAddr,(int)lpObj->m_ClientPlatform,lpObj->m_ClientBuild,minBuild);
			GCConnectAccountSend(aIndex,6);
			return;
		}
	}
...
void CSEND_CHANGEPASS(CSENDGS_DOIMK* lpMsg, int aIndex)
{
	...
	if (lpMsg->TYPE != 0x01)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3118)); //"[Đổi Mật Khẩu] Sai Dữ Liệu !!"
		return;
	}
	if (strlen(lpMsg->PASS_OLD) < 1 || strlen(lpMsg->PASS_NEW) < 1)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3119)); //"[Đổi Mật Khẩu] Hãy Nhập Mật Khẩu Đầy Đủ !!"
		return;
	}
	if (strcmp(lpMsg->PASS_OLD, lpMsg->PASS_NEW) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3120)); //"[Đổi Mật Khẩu] MK Cũ & MK Mới đang giong nhau !"
		return;
	}
...
void DSSENDGS_DOIMK_KQ(DSGS_DOIMK_KQ* lpMsg)
{
	...
	if (lpMsg->result == 1)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3121)); //"[Đổi Mật Khẩu] TK không chính xác !"
		return;
	}
	if (lpMsg->result == 2)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3122)); //"[Đổi Mật Khẩu] MK cũ không đúng !"
		return;
	}
	if (lpMsg->result == 3)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3123)); //"[Đổi Mật Khẩu] Bạn đã đổi MK thành công !!!"
		return;
	}
}
#endif
