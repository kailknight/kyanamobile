...
void gObjEventRunProc() // OK
{
	...
	// outside the guard on purpose - the schedule window needs a Crywolf
	// countdown on every sub-server, not just the one running the event
	gCrywolf.UpdateEventTimeDisplay();
...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	// Spin wheel: neither of these was ever cleared per-connection (the only
	// zeroing was the one-off memset in gObjInit), so a player who disconnected
	// inside the wheel's 5-second deferred-draw window left SauVongQuay armed in
	// that object slot - and the next character to occupy the slot got a free,
	// unpaid draw on the very next tick.
	lpObj->SauVongQuay = 0;
	...
	lpObj->ChaosLastMixID = 0;
	lpObj->ChaosLastMixInfo = 0;
	// Must be reset here, not left to the struct memset: that memset only runs on
	// an object slot's FIRST-EVER allocation - the reuse path re-binds pointers and
	// zeroes nothing. Skipping this means the second character of a session sees an
	// empty claim box that then also never saves, and worse, state can carry across
	// accounts through a recycled slot. Exactly the SauVongQuay bug's mechanism.
	lpObj->LoadSpinClaim = 0;

	// Same reasoning for the slot machine. SlotMachineTick especially: left stale it
	// would already be far in the past, so the first spin of a session would skip
	// its own throttle - and SlotFreeSpins carrying across a recycled slot would
	// hand free spins to a different account.
	lpObj->SlotClaimLoad = 0;
	lpObj->SlotMachineTick = 0;
	lpObj->SlotMachineOpen = 0;
	lpObj->SlotFreeSpins = 0;
	lpObj->SlotFreeStake = -1;
	lpObj->SlotFreeBet = 1;
	...
	lpObj->m_LootRefuseNoticeTick = 0;

	lpObj->m_VoiceToken = 0;

	lpObj->m_ClientBuildReported = false;
	lpObj->m_ClientPlatform = 0;
	lpObj->m_ClientBuild = 0;

	for(int n=0;n < MAX_PVP_RAMP_SLOTS;n++)
	{
		lpObj->m_PvPRamp[n].TargetIndex = -1;
		lpObj->m_PvPRamp[n].TargetLifeId = 0;
		lpObj->m_PvPRamp[n].StartTick = 0;
		lpObj->m_PvPRamp[n].LastHitTick = 0;
	}

	lpObj->m_PvPRampLifeId = 0;
#endif
	lpObj->Option = 3;
...
void gObjClearPlayerOption(LPOBJ lpObj) // OK
{
	...
	// Spin wheel claim box. Clear() per element, NEVER memset: CItem::Clear sets
	// m_Index = -1 and m_ItemExist = 1, whereas a zero-fill would leave 8
	// valid-looking item-index-0 entries that the player could never claim.
...
void gObjCustomLogPlusChaosMix(LPOBJ lpObj,int type,int index)
{
	...
	// The announce that used to live here is gone, for two reasons.
	//
	// It was a DUPLICATE: CChaosBox::PlusItemLevelMix already announces every
	// successful upgrade, on every tier, immediately after calling this - so a
	// +15 produced two lines.
	//
	// And it was BROKEN. AnnounceChaosMixText is "%s upgraded item %s to +%d",
	// three specifiers, but this passed only two arguments. wsprintf then read
	// the third off the stack, which is where the nonsense level in "upgraded
	// item Divine Sword of Archangel to +50" came from. It only showed at +15
	// because of the type check above: type 5 is the top upgrade tier, so this
	// was the only tier that got the second, garbled line.
	//
	// Nothing else here needs keeping - despite the name, the function never
	// logged anything; PlusItemLevelMix does its own gLog.Output.
}
