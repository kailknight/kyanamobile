...
void LogAdd(eLogColor color,char* text,...) // OK
{
	...
	// The LOG_EVENT colour is used almost exclusively by the event subsystems
	// (the GM minigames, King of MU, CTC Mini, Boss Guild, Quiz, Online
	// Lottery), so it doubles as the Event view's selector - that is ~100
	// correctly-categorised call sites for free instead of ~100 edits. The
	// handful of non-event lines that were using the colour cosmetically have
	// been moved to another colour or tagged explicitly with LogAddCat.
	if(color == LOG_EVENT)
	{
		gServerDisplayer.LogAddTextCat(LOG_CAT_EVENT,color,log,strlen(log));

		gLog.Output(LOG_GENERAL,"%s",&log[9]);
...
void LogAddCat(int category,eLogColor color,char* text,...) // OK
{
	tm today;
	time_t ltime;
	time(&ltime);
	...
	// Identical 9-character "HH:MM:SS " prefix that LogAdd builds, so a
	// categorised line lines up with the Common view and any downstream
	// &text[9] indexing stays valid.
	wsprintf(log,"%.8s %s",&time[11],temp);

	gServerDisplayer.LogAddTextCat(category,color,log,strlen(log));
}
