...
void CBsVEvent::ProcState_EMPTY() // OK
{
	if (this->m_RemainTime > 0 && this->m_RemainTime <= (this->m_WarningTime * 60))
	{
		...
		if ((this->AlarmMinSave = (((this->m_RemainTime % 60) == 0) ? ((this->m_RemainTime / 60) - 1) : (this->m_RemainTime / 60))) != this->AlarmMinLeft)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3009), (this->AlarmMinLeft + 1));
	...
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3010));
		this->NoticeSendToAll(1, gMessage.GetMessage(3076), this->m_StandTime); //"[Loạn Chiến] bắt đầu sau %d phút"
...
void CBsVEvent::ProcState_STAND() // OK
{
	...
	{
		this->NoticeSendToAll(0, gMessage.GetMessage(3077)); //"[Loạn Chiến] đã kết thúc!"
		this->SetState(BSV_EVENT_STATE_EMPTY);
		return;
	}
	...
	if (this->m_RemainTime <= 0)
	{
		...
		{
			this->DivisionTeam();
			this->NoticeSendToAll(0, gMessage.GetMessage(3078)); //"[Loạn Chiến] đã bắt đầu!"
...
void CBsVEvent::SetState_CLEAN() // OK
{
	...
	for (int n = 0; n < MAX_BSVEVENT_USER; n++)
	{
		if (OBJECT_RANGE(this->User[n].Index) != 0)
		{
			...
			////gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(3011), lpObj->Name);
...
bool CBsVEvent::Dialog(LPOBJ lpObj, LPOBJ lpNpc)
{
	...
	{
		GCChatTargetSend(lpObj, lpNpc->Index, gMessage.GetMessage(3079)); //"Tham gia Loạn Chiến "
...
bool CBsVEvent::CheckEnterEnabled(LPOBJ lpObj)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3080)); //"Bạn không thể vào Loạn Chiến khi trong nhóm"
		return 0;
	}

	if (lpObj->Guild != 0 && lpObj->Guild->WarState == 1)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0,  gMessage.GetMessage(3081)); //"[Loạn Chiến ] Bạn không thể tham gia!"
		return 0;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3082)); //"[Loạn Chiến] Không mở!"
		return 0;
	}

	if (this->CheckReqItems(lpObj) == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3083)); //"[Loạn Chiến] Bạn không có Item cần thiết để nhập!"
		return 0;
	}
...
void CBsVEvent::UserDieProc(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
	...
	gNotice.GCNoticeSend(lpUserA->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3084), lpUserA->Deaths, this->MaxDie); //"Bạn đã chết [%d/%d lần]"
	gNotice.GCNoticeSend(lpUserB->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(3085), lpUserB->Kills); //"Bạn đã giết được [%d] Kill"
	...
	for (int i = 0; i < MAX_BSVEVENT_USER; i++)
	{
		...
		{
			gNotice.GCNoticeSend(this->User[i].Index, 0, 0, 0, 0, 0, 0, gMessage.GetMessage(3012), lpObj->Name, lpTarget->Name, lpUserB->Kills);
		}
	}
	//item bag
