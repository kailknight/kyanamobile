...
// How long a player goes without being told "This item does not belong to you"
// again, in ms. Long enough to swallow a Muun collector's ~1/second retries on
// a drop it cannot take, short enough that a genuine second attempt a few
// seconds later still explains itself. See CMap::CheckItemGive.
#define LOOT_REFUSE_NOTICE_DELAY 5000
...
#define MAX_MAP 250 //82 original
...
public:
	...
	// notify=false suppresses the "This item does not belong to you" notice for
	// callers that are POLLING rather than acting on a player request - see the
	// loot check in Map.cpp and the auto-pickup callers in CustomPick.cpp.
	bool CheckItemGive(int aIndex,int index,bool notify = true);
