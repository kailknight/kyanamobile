...
// ---- Slot machine reward box ----------------------------------------------
// Declared here rather than in SlotMachine.h because LPOBJ below holds these
// inline, and SlotMachine.h includes THIS file - the other direction would be
// circular.
#define SLOT_CLAIM_SIZE				8

// One held win: what, and how many. See SlotMachine.h for why the box holds a
// COUNT rather than individual items the way the spin wheel's does.
struct SLOT_CLAIM_ROW
{
	int ItemIndex;		// -1 = empty row
	int Level;
	int Count;
};
...
// How many separate opponents one player can hold a PvP damage ramp against
// at once (PvPDamageRamp*, CustomConfig.ini). Beyond this the pairing that has
// gone longest without a blow is evicted, which is the one least likely to
// still be a live fight. Eight covers a normal melee; a player swinging at
// more than eight different people inside the disengage window is in a siege,
// where the oldest pairing being dropped costs nothing.
#define MAX_PVP_RAMP_SLOTS 8

// One pairing's clock: this attacker versus one specific opponent.
struct PVP_RAMP_SLOT
{
	int   TargetIndex;    // opponent, -1 when the slot is free
	DWORD TargetLifeId;   // opponent's m_PvPRampLifeId when this pairing began
	DWORD StartTick;      // first blow of this pairing - what the ramp counts from
	DWORD LastHitTick;    // most recent blow, for the disengage timeout and eviction
};
...
//----------
	...
	char Transaction;
#if(REDEEMCODE)
	// Set for the duration of one preview or commit round trip to DataServer
	// and cleared when its reply arrives - rejects a double-tap of "Check
	// Code"/"Redeem" that fires before the first round trip resolves (that
	// round trip is async, so GameServer's queue thread does not block on it -
	// see RedeemCode.cpp). A dedicated field rather than reusing Transaction
	// itself, to avoid colliding with its other existing uses.
	bool RedeemCodeBusy;
#endif
	CItem* Inventory;
	CItem* Inventory1;
	...
	int ChaosLastMixID;		// last mix the client asked CGChaosMixInfo about - lets a config reload re-push a fresh rate without the client re-asking
	int ChaosLastMixInfo;
	...
	#endif
	// Spin wheel claim box: prizes that could not be placed in the inventory when
	// they were won, held here until the player claims them.
	//
	// Flat and index-addressed on purpose, so no Map array and no 1/2 transaction
	// double-buffer - there is nothing to rect-pack and nothing to roll back.
	// Deliberately NOT inside the #if(GAMESERVER_UPDATE>=802) above, nor in the
	// ALLBOTSSTRUC block further down where SpinWheelIndex/SpinWheelWin live:
	// coupling the declaration to an unrelated build flag would make it vanish
	// while the allocator, reset and save code referencing it stayed.
	//
	// NULL for every object below OBJECT_START_USER - the allocator only hands
	// these out inside its user-range guard - so every accessor needs a
	// Type != OBJECT_USER check first.
	int LoadSpinClaim;
	CItem* SpinClaim;

	// ---- Slot machine ----------------------------------------------------
	// Held INLINE, unlike SpinClaim above. 8 rows x 12 bytes across MAX_OBJECT is
	// about 1 MB, which is cheaper than the NULL check every SpinClaim accessor has
	// to carry - there is no way for this to be unallocated.
	SLOT_CLAIM_ROW SlotClaim[SLOT_CLAIM_SIZE];
	int SlotClaimLoad;

	// A DEDICATED throttle, not ClickClientSend. That one is shared across features
	// under two incompatible conventions and is never reset on login, so it carries
	// a stale value onto a recycled object slot.
	DWORD SlotMachineTick;

	// Set while the machine's window is open. The only thing tying a spin packet to
	// having walked to the NPC.
	BYTE SlotMachineOpen;

	// Free spins survive closing the window and logging out - they were won. The
	// stake is locked to whatever triggered them, so the client's choice is ignored
	// while SlotFreeSpins > 0.
	int SlotFreeSpins;
	int SlotFreeStake;
	int SlotFreeBet;
	int SlotRetriggerCount;

	// Losing-streak pity and beginner luck. All four persist through the reward
	// box's save: a daily limit that reset on relog would be unlimited, and a
	// beginner counter that reset would make beginner luck permanent.
	DWORD SlotLossStreak;
	DWORD SlotPityDate;
	DWORD SlotPityUsedToday;
	DWORD SlotSpinCount;
	...
	// Proximity voice chat session, 0 when none. Issued by CVoiceChat::Proc
	// and cleared by gObjUserInit, so a re-login into this same object slot
	// always gets a fresh one rather than inheriting the last player's.
	unsigned __int64 m_VoiceToken;

	// What the client says it is, from packet 0xD3:0x7D, which it sends after
	// connecting and before it logs in. Used by the MinClientBuild* gate in
	// CGConnectAccountRecv.
	//
	// Both stay 0 for a client too old to send the packet at all, and 0 is
	// below any configured minimum - so outdated builds are refused by the
	// same comparison as merely-behind ones, with no separate case.
	//
	// Cleared by gObjCharZeroSet, which gObjAddSearch calls for every new
	// connection before marking it OBJECT_CONNECTED - so a socket landing on a
	// recycled object slot cannot inherit the previous player's numbers and walk
	// straight past the gate. (It also runs again at character load, which is
	// after the gate has already been applied and therefore harmless.)
	// m_ClientBuildReported is the one that does the work. A PC client reports
	// platform 0 and build 0 on purpose, so a 0 build cannot be read as "did not
	// report" - and "did not report" is precisely what marks a client older than
	// this feature, which is the case the gate exists to catch.
	bool  m_ClientBuildReported;
	BYTE  m_ClientPlatform;
	DWORD m_ClientBuild;

	// Throttle for the "This item does not belong to you" notice. A Muun
	// collector re-asks for the same reserved drop about once a second for as
	// long as it keeps targeting it (PetActionCollecter::eAction_Get in the
	// client), and the pet uses the very same pickup packet a manual click
	// does - so the server cannot tell the two apart and answered every one.
	// See CMap::CheckItemGive.
	DWORD m_LootRefuseNoticeTick;

	// PvP damage ramp (PvPDamageRamp* in CustomConfig.ini). A drawn-out duel
	// hits harder the longer it stays on the same opponent, so two players of
	// similar build stop trading blows for minutes with neither able to finish.
	//
	// One clock PER PAIRING, not one per attacker. Turning to a second
	// opponent suspends the first clock rather than wiping it, and coming back
	// resumes where it paused. Two reasons that matters:
	//
	//   - a player fighting two opponents at once would otherwise zero their
	//     own ramp on every target switch, so the outnumbered side - the one
	//     that most needs a way out of a stalemate - could never ramp at all;
	//   - area skills call CAttack::Attack once per target in a loop
	//     (SkillManager.cpp), so a single Twisting Slash across two players
	//     would flip a single-slot ramp twice and reset it both times,
	//     excluding every AoE build from the mechanic.
	//
	// It still cannot be banked: each pairing carries its own clock, so time
	// spent on one opponent buys nothing against the next.
	PVP_RAMP_SLOT m_PvPRamp[MAX_PVP_RAMP_SLOTS];

	// Bumped on this object's own death. An attacker stores the value it saw
	// when the pairing began, so a target that dies and respawns no longer
	// matches and that pairing's clock restarts - no scan of every object on
	// each death, and no way for a stale index to resurrect an old ramp.
	DWORD m_PvPRampLifeId;
#endif
...
//MC bot
	...
	int RaceTime;
	DWORD	SauVongQuay;
	// Which spin wheel this player paid for. Was a single int on the
	// CCustomVongQuay singleton, written at spin time and read by the deferred
	// draw 5 seconds later - so with more than one wheel configured, two players
	// spinning inside that window drew from each other's prize tables.
	int		SpinWheelIndex;
	// Winning slot, drawn at spin time rather than when the prize is handed out.
	// The client is told this in the spin-start packet so its pointer can
	// decelerate ONTO the real result over the whole 5-second window. Previously
	// the draw happened at grant time, which meant the client learned the winner
	// only after the item, the firework and the global notice had already gone
	// out - so the arrow was still spinning after the player had seen the prize.
	int		SpinWheelWin;
