...
bool CSkillManager::CheckAreaSkillTarget(LPOBJ lpObj,int aIndex,int bIndex,int type) // OK
{
	...
	if(OBJECT_RANGE(aIndex) == 0 || type == OBJECT_NPC)
	{
		return 0;
	}
	...
	return this->CheckAreaSkillPlayerTarget(lpObj,aIndex,bIndex);
}

bool CSkillManager::CheckAreaSkillPlayerTarget(LPOBJ lpObj,int aIndex,int bIndex) // OK
{
	// CheckSkillTarget only lets an area skill reach a player in a duel, Gens,
	// guild war, arena or siege, so outside those a Dark Lord's area skills hit
	// nobody but the exact target. Here they also hit players a normal attack
	// could hit, minus the caster's party, guild and alliance. Bystanders are
	// only hit while the Dark Lord is aiming at a player (bIndex) or the
	// bystander is a murderer, so farming beside other players never PKs them.
	if(gServerInfo.m_DarkLordAreaSkillPvP == 0 || lpObj->Type != OBJECT_USER || lpObj->Class != CLASS_DL)
	{
		return 0;
	}
	...
	if(lpTarget->Type != OBJECT_USER || lpTarget->Index == lpObj->Index)
	{
		return 0;
	}

	if(OBJECT_RANGE(lpObj->PartyNumber) != 0 && gParty.IsMember(lpObj->PartyNumber,lpTarget->Index) != 0)
	{
		return 0;
	}

	if(lpObj->GuildNumber > 0 && lpObj->GuildNumber == lpTarget->GuildNumber)
	{
		return 0;
	}
	...
	bool AimingAtPlayer = (OBJECT_RANGE(bIndex) != 0 && gObj[bIndex].Type == OBJECT_USER && bIndex != lpObj->Index);

	if(AimingAtPlayer == 0 && lpTarget->PKLevel < 5)
	{
		return 0;
	}
...
void CSkillManager::UseAttackSkill(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	bool combo = false;
	if (gCustomCombo.CheckOneSkillCombo(lpSkill->m_skill) != 0) {
		combo = true;
		lpObj->ComboTime = GetTickCount();
		//LogAdd(LOG_ALERT,"%d combo %d",lpSkill->m_skill,combo);

	}
	if (lpSkill->m_skill == 16 //DW
		|| lpSkill->m_skill == 217 // sum
		|| lpSkill->m_skill == 27 //Elf
		|| lpSkill->m_skill == 556 //Elf
		|| lpSkill->m_skill == 569 //Elf
		|| lpSkill->m_skill == 572 //Elf
		|| lpSkill->m_skill == 28) //Elf
	{
		if (aIndex != bIndex && gServerInfo.m_EnableBuffParty == 0)
		{
			return;
		}
	}
	if (gServerInfo.m_ElfBuffParty == 0)
	{
		LPOBJ lpTarget = &gObj[bIndex];
		if (lpSkill->m_skill == 27 || lpSkill->m_skill == 28 || lpSkill->m_skill == 217)
		{
			if (aIndex != bIndex)
			{
				if (gParty.IsParty(lpObj->PartyNumber) != 1 || gParty.IsParty(lpTarget->PartyNumber) != 1)
				{
					return;
				}
...
bool CSkillManager::RunningSkill(int aIndex,int bIndex,CSkill* lpSkill,BYTE x,BYTE y,BYTE angle,bool combo) // OK
{
	...
	SendHidenPetIsAttack(lpObj, lpSkill->m_skill);
#endif
#if(CB_ATTACK_HIDEN_WING)
	SendHidenWingIsAttack(lpObj, lpSkill->m_skill);
#endif
...
bool CSkillManager::MultiSkillAttack(int aIndex,int bIndex,CSkill* lpSkill,bool combo) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		switch(lpSkill->m_skill)
		{
			...
			case SKILL_FIRE_SCREAM:
				this->BasicSkillAttack(aIndex,bIndex,lpSkill,combo);
				this->FireScreamPlayerSplash(aIndex,bIndex,lpSkill,combo);
				break;
		}
	}
	else
...
void CSkillManager::FireScreamPlayerSplash(int aIndex,int bIndex,CSkill* lpSkill,bool combo) // OK
{
	// The client's Fire Scream hit list only carries monsters and the one
	// player it has selected, so other players in the strike never get hit.
	// Monsters still come from that list; this adds the players.
	LPOBJ lpObj = &gObj[aIndex];

	if(gServerInfo.m_DarkLordAreaSkillPvP == 0 || lpObj->Class != CLASS_DL || OBJECT_RANGE(bIndex) == 0)
	{
		return;
	}
	...
	int angle = this->GetSkillAngle(lpObj->X,lpObj->Y,gObj[bIndex].X,gObj[bIndex].Y);
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(index == bIndex || this->CheckAreaSkillPlayerTarget(lpObj,index,bIndex) == 0)
		{
			continue;
		}
...
bool CSkillManager::SkillGreaterLife(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if (OBJECT_RANGE(lpObj->PartyNumber) == 0 || gServerInfo.m_EnableBuffParty == 0)
	{
...
bool CSkillManager::SkillForce(int aIndex,int bIndex,CSkill* lpSkill,bool combo) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		...
		if(this->CheckAreaSkillTarget(lpObj,index,bIndex,lpObj->VpPlayer2[n].type) == 0)
		{
			continue;
		}
...
bool CSkillManager::SkillSwordPowerGetPhysiDamage(int aIndex, __int64* DamageMin, __int64* DamageMax) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
...
bool CSkillManager::SkillSwordPowerGetMagicDamage(int aIndex, __int64* DamageMin, __int64* DamageMax) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
...
bool CSkillManager::SkillSwordPowerGetCurseDamage(int aIndex, __int64* DamageMin, __int64* DamageMax) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
...
void CSkillManager::ApplyDeathStabEffect(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int damage) // OK
{
	...
	// Death Stab (Dam Gio) master skill stun.
	// TyLeRaChoangSkillMasterDamGio is used as a modulus, so a configured value
	// of 0 would be an integer division by zero and crash the server. Treat
	// 0 (or negative) as "stun disabled" instead.
	int DeathStabStunRate = gServerInfo.TyLeRaChoangSkillMasterDamGio;

	if (DeathStabStunRate > 0 && (GetLargeRand() % DeathStabStunRate) < (gMasterSkillTree.GetMasterSkillValue(lpObj, MASTER_SKILL_ADD_DEATH_STAB_MASTERED) - lpTarget->ResistStunRate))
	{
		//gEffectManager.AddEffect(lpTarget,0,EFFECT_STERN,2,0,0,0,0);
		gEffectManager.AddEffect(lpTarget, 0, EFFECT_STERN, gServerInfo.ThoiGianChoangSkillMasterDamGio, 0, 0, 0, 0);
	}
...
void CSkillManager::ApplyEarthquakeEffect(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int damage) // OK
{
	...
	// CommboEffectEnable (CustomConfig.ini): always show the Dark Horse
	// stomp effect instead of only on this random roll.
	if(gServerInfo.m_CommboEffectEnable || (GetLargeRand()%100) < (gMasterSkillTree.GetMasterSkillValue(lpObj,MASTER_SKILL_ADD_EARTHQUAKE_ENHANCED)-lpTarget->ResistStunRate))
	{
		gEffectManager.AddEffect(lpTarget,0,EFFECT_STERN,2,0,0,0,0);
	}

	// DarkHorsePushBackChance (CustomConfig.ini): independent chance to
	// physically knock the target back - decoupled from the stomp effect
	// roll above (it used to be an either/or with it), so 100 means the
	// stomp always both shows the effect AND pushes the target back.
	// Distance comes from CustomDarkHorsePushDistance, in tiles.
	if((GetLargeRand()%100) < gServerInfo.m_DarkHorsePushBackChance)
	{
		// Diagnostic for the still-unconfirmed report that this sometimes
		// visibly doesn't push. gObjBackSpring2 always returns TRUE here
		// (the roll above is the only thing gating whether it's called), so
		// FALSE means the target's class/state hit one of gObjBackSpring2's
		// own early-return gates (bosses/NPCs/siege crown holder/GM flag/
		// certain reborn classes are all excluded from being knocked back at
		// all) - not a bug, but a different, unlogged reason for "no push".
		// Remove once the live report is resolved.
		if(gObjBackSpring2(lpTarget,lpObj,gServerInfo.m_CustomDarkHorsePushDistance) == 0)
		{
			LogAdd(LOG_RED,"[DarkHorse] Push-back roll succeeded but gObjBackSpring2 rejected it - target [%s] class %d",lpTarget->Name,lpTarget->Class);
		}
	}

	#else

	gObjBackSpring2(lpTarget,lpObj,gServerInfo.m_CustomDarkHorsePushDistance);
...
void CSkillManager::ApplyFiveShotEffect(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int damage) // OK
{
	...
	// Five Shot (Ngu Tien) master skill stun. Same divide-by-zero guard as the
	// Death Stab stun above; note this key's built in default is 0, so a
	// missing ini entry would otherwise crash the server outright.
	int FiveShotStunRate = gServerInfo.TyLeRaChoangSkillMasterNguTien;

	if (FiveShotStunRate > 0 && (GetLargeRand() % FiveShotStunRate) < (gMasterSkillTree.GetMasterSkillValue(lpObj, MASTER_SKILL_ADD_FIVE_SHOT_ENHANCED) - lpTarget->ResistStunRate))
	{
