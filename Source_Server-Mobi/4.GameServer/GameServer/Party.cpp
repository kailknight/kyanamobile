...
/*
	Wires up a party from a matched /setpass password. Lifted verbatim out of the
	whisper handler (CGChatWhisperRecv, Protocol.cpp) when /join was added, so
	both entry points run exactly the same code.

	lpHost is the player who set the password and ends up LEADING: the branches
	below deliberately Create() on lpHost, not on lpJoiner. Party leadership is
	Index[0], and it decides who can kick and how experience is shared, so the
	person who opened the party has to be the one holding it. This is why /join
	does not simply call AutoAcceptPartyRequest() - that treats its first
	argument as the requester and creates the party there, which for a
	password join would hand the party to whoever typed /join last.

	Deliberately does NOT consult gObjCheckAutoParty(): that compares the
	joiner's OWN AutoPartyPassword against the host's, which would force every
	member to /setpass the same value first. Here the password arrives in the
	command (or the whisper) itself and has already been matched by the caller.
*/
void CParty::JoinPartyByPassword(LPOBJ lpJoiner,LPOBJ lpHost) // OK
{
	if(lpHost->PartyNumber >= 0)
	{
		if(lpJoiner->PartyNumber == lpHost->PartyNumber)
		{
			gNotice.GCNoticeSend(lpJoiner->Index,1,0,0,0,0,0,gMessage.GetMessage(3117)); //"Ban da trong nhom roi !!"
			return;
		}
	}

	//== Join the joiner's existing party if only they have one ==//
	if(lpJoiner->PartyNumber >= 0 && lpHost->PartyNumber < 0)
	{
		this->AddMember(lpJoiner->PartyNumber,lpHost->Index);
		return;
	}

	if(this->Create(lpHost->Index) == 0)
	{
		if(this->AddMember(lpHost->PartyNumber,lpJoiner->Index) == 0)
		{
			this->GCPartyResultSend(lpJoiner->Index,2);
			this->GCPartyResultSend(lpHost->Index,2);
		}
	}
	else
	{
		if(this->AddMember(lpHost->PartyNumber,lpJoiner->Index) == 0)
		{
			this->GCPartyResultSend(lpJoiner->Index,2);
			this->GCPartyResultSend(lpHost->Index,2);
			this->Destroy(lpHost->PartyNumber);
		}
	}
}
