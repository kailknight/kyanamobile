...
// What build the client is, sent once as soon as it connects and always before
// it logs in - so CGConnectAccountRecv can apply the MinClientBuild* gate
// (ServerInfo.h) while it still has a clean way to refuse.
//
// Its own packet rather than extra fields on the login packet. That one is
// hand-built at exactly 60 bytes and encrypted (AndroidNetwork.cpp's
// ConnectionManager_SendLogin), and lengthening it would have every old client
// misparse rather than get a clean "wrong version" answer.
//
// A client too old to send this at all simply leaves the object's numbers at 0,
// which is below any configured minimum - so it is refused by the same
// comparison, with no separate case to get wrong.
//
// Build is a DWORD and NOT a long: bare long is 4 bytes on the PC client and 8
// on Android, which silently corrupts every field after it (see
// mu-android-long-abi-bug).
struct PMSG_CLIENT_BUILD_RECV
{
	PSBMSG_HEAD header; // C1:D3:7D
	BYTE  Platform;     // 0 = PC, 1 = Android
	DWORD Build;
};
...
// C1:F3:F5 - which items the custom-item personal stores trade in.
//
// The item NAME does not travel: the client already has the whole item table, so
// it derives the name from the index. Sending it too would mean the ini and the
// client could disagree about what the same store takes.
//
// The BUTTON label does travel, because it is not the item name - a currency
// button is too narrow for one, so the ini names a short label instead and there
// is nowhere else the client could learn it.
//
// Enabled is separate from ItemIndex rather than using -1 as the off value,
// because a client that never receives this packet at all (an older server)
// leaves the whole struct at zero and must behave as "off" without having to
// know that 0 is also a legal item index.
// Defined here rather than in CustomStore.h because the packet below needs it and
// CustomStore.h is the one that includes this file, not the other way round.
#define MAX_CUSTOM_STORE_ITEM			3
...
struct PMSG_CUSTOMSTORE_ITEM_INFO_SEND
{
	PSBMSG_HEAD header;
	PMSG_CUSTOMSTORE_ITEM_INFO_SLOT Slot[MAX_CUSTOM_STORE_ITEM];
};
...
void CGClientBuildRecv(PMSG_CLIENT_BUILD_RECV* lpMsg,int size,int aIndex);
...
#if(REDEEMCODE)
//-----------------------------------
// Redemption code system. Client<->GameServer, both directions under the
// shared custom-feature head 0xD3 (see the "==Protocol Custom" switch in
// Protocol.cpp) - 0xA0/0xA1 for the non-binding preview ("Check Code"),
// 0xA2/0xA3 for the actual redeem ("Redeem"). Struct shape modeled on
// PMSG_REGISTER_MAIN_SEND just above.
//-----------------------------------

// C1:D3:A0 (preview) and C1:D3:A2 (redeem) - identical payload, the two
// steps are distinguished only by which subcode the header carries.
...
{
	REDEEM_SUCCESS = 0,      // only ever set on the 0xA3 redeem reply
	REDEEM_OK_TO_REDEEM,     // preview-only: code is valid, nothing consumed yet
	REDEEM_NOT_FOUND,
	REDEEM_INACTIVE,
	REDEEM_EXPIRED,
	REDEEM_CAP_REACHED,
	REDEEM_ALREADY_REDEEMED,
	REDEEM_INVENTORY_FULL,   // redeem-only: room check ran, the bundle didn't fit
	REDEEM_BUSY,             // an earlier preview/redeem for this player is still in flight
	REDEEM_SERVER_ERROR,
};

// Full detail per bundle item - enough to rebuild an ITEM for a real tooltip
// (matches the fields ItemConvert/RenderItemInfo need), not just a compact
// summary. Field names mirror GDCreateItemSend's own parameter breakdown
// (see RedeemCode.h) so granting is a direct 1:1 mapping with no repacking.
struct REDEEM_ITEM_DETAIL
{
	...
	BYTE ItemSkill;      // 0/1
	BYTE ItemLuck;       // 0/1
	BYTE ItemOption;     // the +option value, 0-7
	BYTE ItemExcellent;  // excellent bitmask, 0-63
	BYTE Quantity;
	BYTE SocketOption[5]; // 0xFF = empty slot; literal 5 (not MAX_SOCKET_OPTION, Item.h isn't included here)
	BYTE SocketOptionBonus;
	BYTE Item380;
	BYTE HarmonyOption;
	BYTE HarmonyOptionLevel;
	// Deliberately no duration field here - the preview tooltip doesn't need
	// it, and the actual grant always re-reads it fresh from the DB bundle
	// (see RedeemCode.cpp) rather than trusting anything carried over the wire.
};

#define REDEEM_CODE_BUNDLE_MAX 8

// C1:D3:A1 (preview result) and C1:D3:A3 (redeem result) - identical shape.
struct PMSG_REDEEM_CODE_RECV
{
	PSBMSG_HEAD header;
	BYTE Result; // eRedeemCodeResult
	BYTE ItemCount;
...
// Must stay byte-identical to the client's hand-duplicated copy in
// 5.Main/source/Protocol.h - see the matching assert there for why the STRIDE
// specifically is what matters (a mismatch corrupts Items[1] onward while
// leaving Items[0] looking perfectly fine). Rebuild BOTH sides together.
static_assert(sizeof(REDEEM_ITEM_DETAIL) == 18, "REDEEM_ITEM_DETAIL must stay 18 bytes and byte-identical to the client's copy in 5.Main/source/Protocol.h");
...
struct PMSG_AUTORESET_INFO_RECV //Info Relife
{
	PSBMSG_HEAD header; // C3:F3:03
	DWORD LevelRelife;
	DWORD RelifeCong;
...
void GCAutoResetInfoSend(LPOBJ lpObj);
#endif
...
void SendHidenPetIsAttack(LPOBJ lpObj, int Skill);
#endif
#if(CB_ATTACK_HIDEN_WING)
void SendHidenWingIsAttack(LPOBJ lpObj, int Skill);
#endif
//-----Auto Send move
struct AUTOMOVE_REQ
{
