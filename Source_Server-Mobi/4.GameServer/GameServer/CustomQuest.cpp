...
void CCustomQuest::QuestCommand(LPOBJ lpObj,char* arg,int Npc)
{
	...
	//Log
	// LOG_BLACK, not LOG_EVENT: quest completion is not a scheduled/group event,
	// and the LOG_EVENT colour now selects the Logs > Event Logs view.
	LogAdd(LOG_BLACK,"[CustomQuest][%s][%s] - (Quest %d: Completed)",lpObj->Account,lpObj->Name,lpObj->CustomQuest);
