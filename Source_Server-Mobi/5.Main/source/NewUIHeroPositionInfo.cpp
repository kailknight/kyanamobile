...
void CNewUIHeroPositionInfo::DataViewPortMapLoad(float x, float y) {
  ...
  for (int i = 0; i < 400; ++i) {
    ...
    // NOTE: this was comparing the function's constant input params (x, y)
    // instead of the per-character computed screen position (X, Y), so it
    // was either always-true or always-false for the whole call - every
    // live character in the zone (checked up to 400 of them) got drawn
    // regardless of whether it actually falls within the minimap widget.
...
bool CNewUIHeroPositionInfo::Render() {
  ...
  // instead of every single frame; a position readout doesn't need to be
  // more frequent than that.
  static unicode::t_char s_cachedPosText[255] = {NULL};
  static DWORD s_lastPosTextUpdate = 0;
  const DWORD nowTick = GetTickCount();
  if (s_lastPosTextUpdate == 0 || (nowTick - s_lastPosTextUpdate) >= 300) {
    unicode::_sprintf(s_cachedPosText, "%s (%d/%d)",
                      gMapManager.GetMapName(gMapManager.WorldActive),
                      m_CurHeroPosition.x, m_CurHeroPosition.y);
    s_lastPosTextUpdate = nowTick;
  }
  g_pRenderText->RenderText(m_Pos.x + 10, m_Pos.y + 2, s_cachedPosText, WidenX + 20,
...
#ifdef __ANDROID__
  {
    ...
    const double imgMs = (static_cast<double>(dbgImgTicks) * 1000.0) / freq;
    const double textMs = (static_cast<double>(dbgTextTicks) * 1000.0) / freq;
    const double menuMs = (static_cast<double>(dbgMenuTicks) * 1000.0) / freq;
    const double mapMs = (static_cast<double>(MU_MobilePerfNow() - dbgMapStart) * 1000.0) / freq;
    static int s_dbgHpiCounter = 0;
    if ((++s_dbgHpiCounter % 30) == 0)
    {
      if (FILE* f = fopen("mu_hpi_debug.txt", "a"))
      {
        fprintf(f, "HPI init=%.3fms img=%.3fms text=%.3fms menu=%.3fms drawMiniMap=%.3fms\n", initMs, imgMs, textMs, menuMs, mapMs);
        fclose(f);
      }
    }
  }
