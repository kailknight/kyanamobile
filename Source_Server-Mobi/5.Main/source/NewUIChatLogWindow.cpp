...
bool SEASON3B::CNewUIChatLogWindow::RenderMessages(int Type)
{
	...
	{
		iRenderStartLine = Bm_iCurrentRenderEndLine - MaxShowLines + 1;
	}
	else
	{
		fRenderPosY = fRenderPosY + FONT_LEADING + (SCROLL_MIDDLE_PART_HEIGHT * (MaxShowLines - Bm_iCurrentRenderEndLine - 1));
	}
	...
	for (int i = iRenderStartLine, s = 0; i <= Bm_iCurrentRenderEndLine; i++, s++)
	{
		...
		if (bRenderMessage && !pMsgText->GetText().empty())
		{
			...
			if (!pMsgText->GetID().empty())
			{
				...
				{
					ITEM* pNewItem = pMsgText->GetItemMemory();
					if (pNewItem != NULL)
					{
						...
						if (pNewItem->Type > 0 && pNewItem)
						{
							RenderItemInfo((GetScreenWidth() / 2), (150), pNewItem, 0, 0, false, 1); //DAT FIX POST 
							//RenderItemInfo((MouseX + 30), (MouseY + 70), pNewItem, 0, 0, false, 1); //GOC
...
bool SEASON3B::CNewUIChatLogWindow::Create(CNewUIManager* pNewUIMng, int x, int y, int nShowingLines /* = 6 */)
{
	...
	m_WndPos.x = x; m_WndPos.y = y + DisplayHeightExt;
...
bool SEASON3B::CNewUIChatLogWindow::Render()
{
	...
	//{
	//	//===Tach Chat
	//	static int CacheX = m_WndPos.x;
	//	static int CacheY = m_WndPos.y;
	//	static int CacheLines = m_nShowingLines;
	//	static int CacheType = m_CurrentRenderMsgType;
	//	m_WndPos.y = 210;
	//	m_nShowingLines = 4;
	//	this->ChangeMessage(TYPE_SYSTEM_MESSAGE);
	...
	//	m_WndPos.y = CacheY;
	//	m_nShowingLines = CacheLines;
	//	this->ChangeMessage((MESSAGE_TYPE)CacheType);
	//	//====
	//	if (RenderMessages() == false)
	//	{
	//		return false;
