	...
	if(m_BtnEnter.UpdateMouseEvent() == true)
	{
		if(m_pNpcObject)
		{
			...
			if(	!((pItemWing->Type >= ITEM_WING+0 && pItemWing->Type <= ITEM_WING+6)
				...
				|| gCustomPet2.GetInfoPetType(pItemHelper->Type) ==5 //Custom pet
				|| gCustomPet2.GetInfoPetType(pItemHelper->Type) ==6 //Custom pet
				|| gCustomWing.CheckCustomWingByItem(pItemWing->Type) //Custom Wing
