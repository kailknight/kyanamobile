...
void NewMoveCharacterScene()
{
	...
	if (rInput.IsLBtnDn() || rInput.IsLBtnDbl())
	{
		...
		{
			fprintf(dbgf,
				"TAP mouse=(%d,%d) LBtnDn=%d LBtnDbl=%d onUI=%d selChar=%d selHero=%d connectShow=%d\n",
				MouseX, MouseY,
				rInput.IsLBtnDn() ? 1 : 0,
				rInput.IsLBtnDbl() ? 1 : 0,
				rUIMng.IsCursorOnUI() ? 1 : 0,
				SelectedCharacter,
				SelectedHero,
				rUIMng.m_CharSelMainWin.IsShow() ? 1 : 0);
			fclose(dbgf);
		}
	}
#endif
