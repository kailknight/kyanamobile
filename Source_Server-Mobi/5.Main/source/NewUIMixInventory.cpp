...
bool CNewUIMixInventory::Render()
{
	...
	{
		g_pMixInventory->SetPos(640 - 190 * 3 + DisplayWinCDepthBox, 0);
	}
	else
	{
		g_pMixInventory->SetPos(640 - 190 * 2 + DisplayWinCDepthBox, 0);
	}
	EnableAlphaTest();
	...
	if (GetMixState() == MIX_FINISHED)
	{
		...
		if (gCB_InfoCustomMix)
		{
			if (!gCB_InfoCustomMix->m_DataInfoCustomMix.empty())
			{
				...
				unicode::_sprintf(szText, GlobalText[gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name2]);
				g_pRenderText->RenderText(fPos_x, fPos_y + fLine_y, szText);
				fLine_y += 10;
				if (gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name3 != 0)
				{
					unicode::_sprintf(szText, GlobalText[gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name3]);
					g_pRenderText->RenderText(fPos_x, fPos_y + fLine_y, szText);
				}
				fLine_y += 10;

				goto NextShowNameMix;
			}
		}
		...
		else
		{
			...
			case SEASON3A::MIXTYPE_SEED_SPHERE:
				...
				if (gCB_InfoCustomMix)
				{
					if (!gCB_InfoCustomMix->m_DataInfoCustomMix.empty())
					{
						unicode::_sprintf(szText, GlobalText[584], GlobalText[591], gCB_InfoCustomMix->m_DataInfoCustomMix[0].MixRate);
						goto NextRateShow;
					}
				}
...
NextRateShow:				
	...
	case SEASON3A::MIXTYPE_DETACH_SOCKET:
		{
			...
			if (gCB_InfoCustomMix)
			{
				if (!gCB_InfoCustomMix->m_DataInfoCustomMix.empty())
				{
					ConvertGold(gCB_InfoCustomMix->m_DataInfoCustomMix[0].MixMoney, szGoldText);
					ConvertChaosTaxGold(gCB_InfoCustomMix->m_DataInfoCustomMix[0].MixMoney, szGoldText2);
					unicode::_sprintf(szText, GlobalText[1622], szGoldText2, szGoldText);
				}
			}
#endif
	...
	int iTextPos_y = 0;
#if(CB_CUSTOMMIXINFO)
	if (g_MixRecipeMgr.GetMixInventoryType() == SEASON3A::MIXTYPE_GOBLIN_NORMAL && !gCB_InfoCustomMix->m_DataInfoCustomMix.empty())
	{
		...
		unicode::_sprintf(szText, GlobalText[2334], GlobalText[gCB_InfoCustomMix->m_DataInfoCustomMix[0].Name1]);
		...
		//==Show List Item
		int iResult;
		for (int iLine = 0; iLine < gCB_InfoCustomMix->m_DataInfoCustomMix[0].m_DataItemCheck.size(); ++iLine)
		{
			iResult = gCB_InfoCustomMix->GetSourceName(iLine, szText);
	...
	if (gCB_InfoCustomMix)
	{
		if (!gCB_InfoCustomMix->m_DataInfoCustomMix.empty())
		{
			if (gCB_InfoCustomMix->GetCurRecipeDesc(szText, 1) == TRUE)
				g_pRenderText->RenderText(fPos_x, fPos_y + fLine_y + (++iTextPos_y) * 15, szText);
			if (gCB_InfoCustomMix->GetCurRecipeDesc(szText, 2) == TRUE)
				g_pRenderText->RenderText(fPos_x, fPos_y + fLine_y + (++iTextPos_y) * 15, szText);
			if (gCB_InfoCustomMix->GetCurRecipeDesc(szText, 3) == TRUE)
				g_pRenderText->RenderText(fPos_x, fPos_y + fLine_y + (++iTextPos_y) * 15, szText);

			if (gCB_InfoCustomMix->GetRecipeAdvice(szText, 1) == TRUE)
				g_pRenderText->RenderText(fPos_x, fPos_y + fLine_y + (++iTextPos_y) * 15, szText);
			if (gCB_InfoCustomMix->GetRecipeAdvice(szText, 2) == TRUE)
				g_pRenderText->RenderText(fPos_x, fPos_y + fLine_y + (++iTextPos_y) * 15, szText);
			if (gCB_InfoCustomMix->GetRecipeAdvice(szText, 3) == TRUE)
				g_pRenderText->RenderText(fPos_x, fPos_y + fLine_y + (++iTextPos_y) * 15, szText);
...
bool CNewUIMixInventory::Mix()
{
	...
	if (gCB_InfoCustomMix)
	{
		if (!gCB_InfoCustomMix->m_DataInfoCustomMix.empty())
		{
			nMixZen = gCB_InfoCustomMix->m_DataInfoCustomMix[0].MixMoney;
		}
	}
#endif
	...
	if (gCB_InfoCustomMix)
	{
		if (!gCB_InfoCustomMix->m_DataInfoCustomMix.empty())
		{
			if (gCB_InfoCustomMix->CheckCustomMixOK())
			{
	...
	if(m_pNewInventoryCtrl && pPickedItem)
	{
			...
			pPickedItem->GetOwnerInventory() == g_pMyInventory->GetInventoryCtrl()|| g_pMyInventoryExt->GetOwnerOf(pPickedItem)) //==Inv -> Mix
		{
			...
			if (SEASON3B::IsPress(VK_LBUTTON) || GetClick)
			{
				...
				{
 					if(SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, iSourceIndex, 
...
{ 
	...
	if (gCB_InfoCustomMix)
	{
		gCB_InfoCustomMix->m_DataListItemMixInv.clear();
		gCB_InfoCustomMix->CheckCountMix = 0;
	}
#endif
	ITEM * pItem = NULL;
...
#if(CB_CUSTOMMIXINFO)
		if (gCB_InfoCustomMix) gCB_InfoCustomMix->AddItemToMixItemInventory(pItem);
#endif
	}
	...
	//sprintf(m_szTextBuff, "GetNumberOfItems %d", (int)m_pNewInventoryCtrl->GetNumberOfItems());
