...
bool CheckItemSlotOpt(int IndexItem, int Level, int Exl , int style)
{
	int CountSlot = 0;
	...
	for (int Slot = 0; Slot < 6; Slot++)
	{
		if ((int)GetIndexSlot[Slot] == (int)ITEM((7 + Slot), IndexItem)
			&& ((Level <= GetLevelSlot[Slot] && style == 1) || (Level == GetLevelSlot[Slot] && style == 2)) && Exl <= GetExlSlot[Slot])
		{
			CountSlot++;
		}
	}
...
int nCInfo::nInformationOP(int TextNum, ITEM* ItemSearch)
{
	...
	for (int i = 0; i < this->m_CustomInfoSet.size(); i++)
	{
		...
		if (GetIndexItemSet >= this->m_CustomInfoSet[i].ItemIndexMin && GetIndexItemSet <= this->m_CustomInfoSet[i].ItemIndexMax)
		{
			...
			for (int ib = 0; ib < 15; ib++)
			{
				...
				if (CheckItemSlotOpt(GetIndexItemSet, this->m_CustomInfoSet[i].LevelItem[ib], this->m_CustomInfoSet[i].Exc, this->m_CustomInfoSet[i].TooltipStyle))
				{
					SetColorGray = false;
				}
				else 
			...
			//wsprintf(TextList[TextNum++], "\n");
			wsprintf(TextList[TextNum++], "\n");
			wsprintf(TextList[TextNum], "MuOnline");
			TextListColor[TextNum] = 4;
			TextBold[TextNum++] = 0;
		}
	}
	...
	for (int iT = 0; iT < this->m_CustomInfo.size(); iT++)
	{
		...
		if (ItemSearch->Type >= this->m_CustomInfo[iT].ItemIndexMin && ItemSearch->Type <= this->m_CustomInfo[iT].ItemIndexMax)
		{
			...
			int itemlevel = (int)((ItemSearch->Level >> 3) & 15);
			int itemLevelFix = 0;

			if (this->m_CustomInfo[iT].TooltipStyle == 1)
			{
				itemLevelFix = itemlevel;
			}

			if (this->m_CustomInfo[iT].TooltipStyle == 2)
			{
				for (int ibx = 0; ibx < 15; ibx++)
				{
					int itemlevelConf = this->m_CustomInfo[iT].LevelItem[ibx];

					if (!nGetTextNum(this->m_CustomInfo[iT].TextIndex[ibx])) { continue; }
					if (itemlevel >= itemlevelConf)
					{
						itemLevelFix = itemlevelConf;
					}
				}
			}
			...
			for (int ibx = 0; ibx < 15; ibx++)
			{
				...
				if (( ((itemlevelConf > itemLevelFix && this->m_CustomInfo[iT].TooltipStyle == 1) || (itemlevelConf != itemLevelFix && this->m_CustomInfo[iT].TooltipStyle == 2)) && this->m_CustomInfo[iT].LevelItem[ibx] != -1) || ((int)ItemSearch->Option1 < this->m_CustomInfo[iT].Exc && this->m_CustomInfo[iT].Exc != -1))
				{
					SetColorGray = true;
				}
				else
