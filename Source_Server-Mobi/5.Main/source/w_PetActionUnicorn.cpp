...
bool PetActionUnicorn::Effect( OBJECT* obj, CHARACTER *Owner, int targetKey, DWORD tick, bool bForceRender )
{
	...
	if (rand_fps_check(3)) CreateEffect(BITMAP_PIN_LIGHT, Position, obj->Angle, Light, 4, obj, -1, 0, 0, 0, 0.45f);
	...
	if (rand_fps_check(2)) CreateParticle(BITMAP_SMOKE, Position, obj->Angle, Light, 67, 1.0f);
