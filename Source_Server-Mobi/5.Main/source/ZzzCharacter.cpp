...
namespace
{
    ...
    constexpr float kCharacterNearDistance = 800.0f;
    ...
    inline bool IsAdaptiveCullableCrowdCharacter(const CHARACTER* c)
    {
        return c != nullptr &&
            (c->Object.Kind == KIND_MONSTER || c->Object.Kind == KIND_NPC);
    }
        ...
        const bool isPriority)
    {
        ...
        if (IsAndroidFullCharacterVisibilityMode() ||
            !IsCharacterAdaptiveEnabled() || c == nullptr || isPriority ||
            !IsAdaptiveCullableCrowdCharacter(c))
        {
            return false;
        }

        const float currentFps = static_cast<float>(FPS);
        if (currentFps <= 0.0f || currentFps >= 30.0f)
        {
            return false;
        }

        const bool isMonster = IsAdaptiveMonsterCharacter(c);
        // NPCs don't have their own crowd counter; overall visible-character
        // count is a reasonable proxy and lets a dense NPC cluster (e.g. a
        // town hub) trigger culling without needing 10+ monsters present.
        const int visibleCrowdCount = isMonster
            ? g_characterVisibleMonsterRenderCount
            : g_characterVisibleRenderCount;
        const int crowdGate = isMonster ? 10 : 6;
        if (visibleCrowdCount < crowdGate)
        {
            return false;
        }

        const bool stressedFrame = (currentFps < 24.0f);
        const bool severeFrame = (currentFps < 19.0f);
        const bool extremeFrame = (currentFps < 14.0f);
        ...
        if (bucket == CharacterDistanceBucket::Far)
        {
            ...
            if (visibleCrowdCount >= crowdGate + 4 && stressedFrame)
            {
            ...
            if (visibleCrowdCount >= crowdGate + 6 && severeFrame)
            {
                minImportance = (std::max)(minImportance, 0.0118f);
            }
            if (extremeFrame)
            {
                minImportance = (std::max)(minImportance, 0.0150f);
            }
        }
        else if (bucket == CharacterDistanceBucket::Mid)
        {
            if (stadiumScene &&
                visibleCrowdCount >= crowdGate + 5 &&
                severeFrame &&
                distSq > nearAdaptiveMinDistanceSq * 1.8f)
            {
                minImportance = 0.0044f;
            }
            else if (extremeFrame &&
                visibleCrowdCount >= crowdGate + 2 &&
                distSq > nearAdaptiveMinDistanceSq * 1.3f)
            {
                minImportance = 0.0055f;
            }
        }
        ...
        const bool isPriority)
    {
        ...
        if (nearBucket)
        {
            // Near non-priority players/NPCs always keep their base model
            // and animation, but a dense cluster standing right next to the
            // camera (e.g. a town hub full of NPCs) can still tank frame
            // time on shadow + cosmetic supplemental passes alone. Trim
            // those under severe stress, same as far/mid characters do.
            if (severeFrame && (g_characterVisibleRenderCount >= 10))
            {
                g_characterRenderLodContext.allowShadow = false;
                g_characterRenderLodContext.allowSupplemental = false;
            }
...
void RenderCharactersClient()
{
	...
	for ( int i=0; i<MAX_CHARACTERS_CLIENT; ++i )
	{
		...
		if ( o->Visible )
		{
            ...
            if (adaptiveEnabled && !fullCharacterVisibility && !isPriority)
            {
                // Hard frame budget under extreme stress: same guaranteed
                // bound used for world objects. Non-priority, non-near
                // players/NPCs/monsters stop rendering once a generous
                // number are already on screen this frame.
                if (static_cast<float>(FPS) > 0.0f &&
                    static_cast<float>(FPS) < 15.0f &&
                    bucket != CharacterDistanceBucket::Near &&
                    g_characterPerfSnapshot.renderRendered >= 8)
                {
