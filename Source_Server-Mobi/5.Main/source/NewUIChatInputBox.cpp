...
bool SEASON3B::CNewUIChatInputBox::UpdateKeyEvent()
{
	...
	if (SEASON3B::IsPress(VK_F3) && g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CHATINPUTBOX))
	{
	...
	if (SEASON3B::IsPress(VK_F5) && g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CHATINPUTBOX))
	{
		if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CHATINPUTBOX))
		{
			if (m_pNewUIChatLogWnd->IsShowFrame())
...
bool SEASON3B::CNewUIChatInputBox::Render()
{
	...
	if(gProtect.m_MainInfo.ShowCommandInfo) gInterface.DrawInfoCommand();
