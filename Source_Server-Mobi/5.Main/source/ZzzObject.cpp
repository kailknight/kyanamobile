...
namespace
{
    ...
    constexpr float kObjectNearDistance = 1700.0f;
        ...
        const float currentFps)
    {
        ...
        if (bucket == AdaptiveDistanceBucket::Far)
        {
            ...
            if (crowdPressure > 30 && extremeFrame)
            {
                minImportance = (std::max)(minImportance, 0.0230f);
            }
        }
        else if (bucket == AdaptiveDistanceBucket::Mid && distSq > nearAdaptiveMinDistanceSq)
        {
            if (crowdPressure > 70 && stressedFrame)
            {
                minImportance = 0.0035f;
            }
            if (crowdPressure > 100 && severeFrame)
            {
                minImportance = (std::max)(minImportance, 0.0072f);
            }
            if (crowdPressure > 60 && extremeFrame)
            {
                minImportance = (std::max)(minImportance, 0.0100f);
            }
        }
        ...
        const int crowdPressure)
    {
        ...
        if (nearBucket)
        {
            // Near objects always keep their base geometry, but under severe
...
namespace
{
    struct DeferredStaticObjectDraw
    {
        OBJECT* obj = nullptr;
        bool allowSupplementalPasses = true;
    };

    // Decide-then-draw split for static decoration, so draw submission can
    // be reordered for texture batching without destabilizing the adaptive
    // cull/LOD system. The original single-pass RenderObjectWithAdaptiveBudget
    // ties its decisions to a running "objects processed so far" counter,
    // which only makes sense if render order matches visitation order; once
    // reordered, that counter (and thus the hard cap / importance cull) would
    // cut different objects frame to frame, causing flicker/pop. This version
    // takes a caller-supplied, per-frame-stable crowd-pressure value (the
    // total static candidate count) and is meant to be called in nearest-
    // first order over the whole candidate set, so the hard cap always keeps
    // what's closest regardless of what order they're actually drawn in.
    bool DecideStaticObjectRender(OBJECT* o, float distSq, int stableCrowdPressure, DeferredStaticObjectDraw& outDecision)
    {
        const AdaptiveObjectDistanceThresholds thresholds = GetObjectDistanceThresholds();
        ...
        {
            if (extremeFrame &&
                bucket != AdaptiveDistanceBucket::Near &&
                g_objectPerfSnapshot.renderRendered >= 70)
            {
                ++g_objectPerfSnapshot.renderDistanceCulled;
                return false;
            }

            if (ShouldCullStaticObjectByImportance(o, bucket, distSq, stableCrowdPressure, currentFps))
            {
                ++g_objectPerfSnapshot.renderDistanceCulled;
                return false;
            }
        ...
        ConfigureObjectRenderLodContext(o, bucket, stableCrowdPressure);
        outDecision.obj = o;
        outDecision.allowSupplementalPasses = g_objectRenderLodContext.allowSupplementalPasses;
        ResetObjectRenderLodContext();
    ...
    // Draw pass: applies the decision made above directly (does not
    // recompute anything crowd-pressure-dependent), so this is safe to call
    // in any order, e.g. sorted by Type for draw-call batching.
    void DrawDecidedStaticObject(const DeferredStaticObjectDraw& decision)
    {
        g_objectRenderLodContext.active = true;
        g_objectRenderLodContext.allowSupplementalPasses = decision.allowSupplementalPasses;
        g_objectRenderLodContext.basePassRendered = false;

        const Uint64 baseTicksStart = static_cast<Uint64>(MU_MobilePerfNow());
        RenderObject(decision.obj);
        ResetObjectRenderLodContext();
        ...
        RenderObjectVisual(decision.obj);
...
static bool RenderObjectWithAdaptiveBudget(OBJECT* o)
{
    ...
    if (adaptiveEnabled && !fullObjectVisibility)
    {
        // Hard frame budget under extreme stress: once a generous number of
        // non-near objects have already been drawn this frame, stop drawing
        // more, full stop. The fine-grained heuristics below (importance,
        // cull period) can be neutralized by other conditions (e.g. a wide
        // camera zoom resets cull period back to 1) - this guarantees an
        // actual bound on render cost instead of hoping they all line up.
        if (extremeFrame &&
            bucket != AdaptiveDistanceBucket::Near &&
            g_objectPerfSnapshot.renderRendered >= 70)
        {
        ...
        if (bucket == AdaptiveDistanceBucket::Far)
        {
            ...
            // Extreme stress (<15fps): a still-visible-crowd should not
            // redraw every far object every frame - most only need a
            // fraction of frames to look present.
            if (extremeFrame && crowdPressure > 40)
            {
                cullPeriod = (std::max)(cullPeriod, uint32_t{8u});
            }
        }
        else if (bucket == AdaptiveDistanceBucket::Mid)
        {
            ...
            if (extremeFrame && crowdPressure > 60 && distSq > thresholds.nearSq * 1.15f)
            {
                cullPeriod = (std::max)(cullPeriod, uint32_t{4u});
            }
        }
        ...
        // NOTE: cullPeriod-based periodic skipping (render 1-of-N frames) is
        // intentionally not applied here - it causes visible flicker on the
        // objects it targets, which reads as worse than a stable cull. The
        // importance-based cull above and the hard cap below are both
        // stable per-frame (same objects stay culled, not toggling) and are
        // the only skip paths actually used.
        (void)cullPeriod;
    }
					...
					if(o != NULL) 
					{
						if(o->Live)
						{
							...
							if(o->Visible || CameraTopViewEnable)
							{
								...
								{
#if defined(__ANDROID__) || defined(MU_IOS)
									if (IsAdaptiveStaticSceneObject(o))
									{
										staticCandidates.emplace_back(o, GetObjectDistanceSq2D(o));
									}
									else
									{
										RenderObjectWithAdaptiveBudget(o);
...
#if defined(__ANDROID__) || defined(MU_IOS)
    // Decide nearest-first using a stable, per-frame crowd-pressure
    // value (unlike the immediate path, this does NOT depend on
    // visitation order), then draw grouped by Type so consecutive
    // draws sharing a model/texture actually get batched by the GL
    // compat layer instead of flushing on every object.
    std::sort(staticCandidates.begin(), staticCandidates.end(),
        [](const std::pair<OBJECT*, float>& a, const std::pair<OBJECT*, float>& b) { return a.second < b.second; });

    const int stableCrowdPressure = static_cast<int>(staticCandidates.size());
    std::vector<DeferredStaticObjectDraw> staticToDraw;
    staticToDraw.reserve(staticCandidates.size());
    for (const std::pair<OBJECT*, float>& candidate : staticCandidates)
    {
        DeferredStaticObjectDraw decision;
        if (DecideStaticObjectRender(candidate.first, candidate.second, stableCrowdPressure, decision))
        {
            staticToDraw.push_back(decision);
        }
    }

    std::stable_sort(staticToDraw.begin(), staticToDraw.end(),
        [](const DeferredStaticObjectDraw& a, const DeferredStaticObjectDraw& b) { return a.obj->Type < b.obj->Type; });
    for (const DeferredStaticObjectDraw& decision : staticToDraw)
    {
