...
void CB_NewQuest::DrawWindowMini()
{
	...
	g_pBCustomMenuInfo->DrawWindowCustomMini(&StartX, &StartY, WindowW, WindowH, eWindowNewQuestMini, "Nhi鐎瑰嫬瀚?V鐎?%d", this->Status);
	...
	if (this->m_DataNQuest[0].MonterClass[0] != -1) TextDraw((HFONT)g_hFont, InfoX + 10, InfoY + (HText * CountText++), ColorText, 0x0, WindowW - 30, 0, 1, "Di鐎瑰嫬瀚?[%s] (%d/%d)", getMonsterName(this->m_DataNQuest[0].MonterClass[0]), this->MonterKill[0], this->m_DataNQuest[0].NQuest_C[0]);
	if (this->m_DataNQuest[0].MonterClass[1] != -1) TextDraw((HFONT)g_hFont, InfoX + 10, InfoY + (HText * CountText++), ColorText, 0x0, WindowW - 30, 0, 1, "Di鐎瑰嫬瀚?[%s] (%d/%d)", getMonsterName(this->m_DataNQuest[0].MonterClass[1]), this->MonterKill[1], this->m_DataNQuest[0].NQuest_C[1]);
	if (this->m_DataNQuest[0].MonterClass[2] != -1) TextDraw((HFONT)g_hFont, InfoX + 10, InfoY + (HText * CountText++), ColorText, 0x0, WindowW - 30, 0, 1, "Di鐎瑰嫬瀚?[%s] (%d/%d)", getMonsterName(this->m_DataNQuest[0].MonterClass[2]), this->MonterKill[2], this->m_DataNQuest[0].NQuest_C[2]);
	if (this->m_DataNQuest[0].MonterClass[3] != -1) TextDraw((HFONT)g_hFont, InfoX + 10, InfoY + (HText * CountText++), ColorText, 0x0, WindowW - 30, 0, 1, "Di鐎瑰嫬瀚?[%s] (%d/%d)", getMonsterName(this->m_DataNQuest[0].MonterClass[3]), this->MonterKill[3], this->m_DataNQuest[0].NQuest_C[3]);
	if (this->m_DataNQuest[0].MonterClass[4] != -1) TextDraw((HFONT)g_hFont, InfoX + 10, InfoY + (HText * CountText++), ColorText, 0x0, WindowW - 30, 0, 1, "Di鐎瑰嫬瀚?[%s] (%d/%d)", getMonsterName(this->m_DataNQuest[0].MonterClass[4]), this->MonterKill[4], this->m_DataNQuest[0].NQuest_C[4]);
...
void CB_NewQuest::DrawWindow()
{
	...
	g_pBCustomMenuInfo->gDrawWindowCustom(&StartX, &StartY, WindowW, WindowH, eWindowNewQuest, "He Thong Nhiem Vu");
	//g_pBCustomMenuInfo->DrawWindowCustomMini(&StartX, &StartY, WindowW, WindowH, eWindowNewQuest, "He Thong Nhiem Vu");
	...
	int CountText = 0;
	//0x62FF00FF = Xanh La
	TextDraw((HFONT)g_hFontBold, InfoX, InfoY+(HText* CountText++), 0xFFC421FF, 0x0, WindowW - 30, 0, 3, "Thong Tin Co Ban");
	CountText += 1;
	TextDraw((HFONT)g_hFont, InfoX, InfoY+(HText* CountText++), 0xFFFFFFFF, 0x0, WindowW-30, 0, 1, "- Di den cac ngoi lang trong Mu va tieu diet quai vat cung dong doi");
	TextDraw((HFONT)g_hFont, InfoX, InfoY+(HText* CountText++), 0xFFFFFFFF, 0x0, WindowW-30, 0, 1, "- Ban co the nhan mot so vat pham khi thuc hien nhiem vu");
	TextDraw((HFONT)g_hFont, InfoX, InfoY+(HText* CountText++), 0xFFFFFFFF, 0x0, WindowW-30, 0, 1, "- Sau khi hoan thanh ban se nhan duoc mot so phan thuong");

	if(CountText < 8) CountText = 8;
	if (this->Status != eNone)
	{
		TextDraw((HFONT)g_hFontBold, InfoX, InfoY + (HText * CountText++), 0xFFC421FF, 0x0, WindowW - 30, 0, 3, "Thong Tin Nhiem Vu");
	}
	else
	{
		TextDraw((HFONT)g_hFontBold, InfoX, InfoY + (HText * CountText++), 0xFFC421FF, 0x0, WindowW - 30, 0, 3, "Nhiem Vu Moi");
	}
	CountText += 1;
	TextDraw((HFONT)g_hFontBold, InfoX + 80, InfoY + (HText * CountText), 0x62FF00FF, 0x0, WindowW - 30, 0, 1, this->m_DataNQuest[0].NameQuest);
	TextDraw((HFONT)g_hFont, InfoX + 10, InfoY + (HText * CountText++), 0xFFFFFFFF, 0x0, WindowW - 30, 0, 1, "Nhi鐎瑰嫬瀚?V鐎? ");
	...
	TextDraw((HFONT)g_hFont, InfoX + 10, InfoY + (HText * CountText++), 0xFFFFFFFF, 0x0, WindowW - 30, 0, 1, "C鐎光偓婵傚鐝?闁兼儼妫勭涵? ");

	if (this->Status != eNone)
	{
		if(this->m_DataNQuest[0].MonterClass[0] != -1) TextDraw((HFONT)g_hFontBold, InfoX + 80, InfoY + (HText * CountText), 0xFFFFFFFF, 0x0, WindowW - 30, 0, 1, "Ti闂佹寧顒?di鐎瑰嫬瀚?qu閻犲绠?v鐎光偓妞嬪孩宕?[%s] (%d/%d)", getMonsterName(this->m_DataNQuest[0].MonterClass[0]), this->MonterKill[0], this->m_DataNQuest[0].NQuest_C[0]);
		TextDraw((HFONT)g_hFont, InfoX + 10, InfoY + (HText * CountText++), 0xFFFFFFFF, 0x0, WindowW - 30, 0, 1, "Th鐎瑰嫬瀚〒?hi鐎瑰嫬瀚? ");

		if (this->m_DataNQuest[0].MonterClass[1] != -1) TextDraw((HFONT)g_hFontBold, InfoX + 80, InfoY + (HText * CountText++), 0xFFFFFFFF, 0x0, WindowW - 30, 0, 1, "Ti闂佹寧顒?di鐎瑰嫬瀚?qu閻犲绠?v鐎光偓妞嬪孩宕?[%s] (%d/%d)", getMonsterName(this->m_DataNQuest[0].MonterClass[1]), this->MonterKill[1], this->m_DataNQuest[0].NQuest_C[1]);
		if (this->m_DataNQuest[0].MonterClass[2] != -1) TextDraw((HFONT)g_hFontBold, InfoX + 80, InfoY + (HText * CountText++), 0xFFFFFFFF, 0x0, WindowW - 30, 0, 1, "Ti闂佹寧顒?di鐎瑰嫬瀚?qu閻犲绠?v鐎光偓妞嬪孩宕?[%s] (%d/%d)", getMonsterName(this->m_DataNQuest[0].MonterClass[2]), this->MonterKill[2], this->m_DataNQuest[0].NQuest_C[2]);
		if (this->m_DataNQuest[0].MonterClass[3] != -1) TextDraw((HFONT)g_hFontBold, InfoX + 80, InfoY + (HText * CountText++), 0xFFFFFFFF, 0x0, WindowW - 30, 0, 1, "Ti闂佹寧顒?di鐎瑰嫬瀚?qu閻犲绠?v鐎光偓妞嬪孩宕?[%s] (%d/%d)", getMonsterName(this->m_DataNQuest[0].MonterClass[3]), this->MonterKill[3], this->m_DataNQuest[0].NQuest_C[3]);
		if (this->m_DataNQuest[0].MonterClass[4] != -1) TextDraw((HFONT)g_hFontBold, InfoX + 80, InfoY + (HText * CountText++), 0xFFFFFFFF, 0x0, WindowW - 30, 0, 1, "Ti闂佹寧顒?di鐎瑰嫬瀚?qu閻犲绠?v鐎光偓妞嬪孩宕?[%s] (%d/%d)", getMonsterName(this->m_DataNQuest[0].MonterClass[4]), this->MonterKill[4], this->m_DataNQuest[0].NQuest_C[4]);
	}
	else
	{
		TextDraw((HFONT)g_hFontBold, InfoX + 80, InfoY + (HText * CountText), 0xFFFFFFFF, 0x0, WindowW - 30, 0, 1, "C鐎光偓婵傚鐝?闁兼儼妫勭涵? %d", this->m_DataNQuest[0].ReqLevel);
		TextDraw((HFONT)g_hFont, InfoX + 10, InfoY + (HText * CountText++), 0xFFFFFFFF, 0x0, WindowW - 30, 0, 1, "Y闂佹寧顒?c鐎光偓婵傚绨? ");
	...
	TextDraw((HFONT)g_hFontBold, InfoX, InfoY + (HText * CountText++), 0xFFC421FF, 0x0, WindowW - 30, 0, 3, "Phan Thuong");
	CountText += 1;
	...
	static char* TextButton[] = { "Nhan Nhiem Vu", "Dang Thuc Hien", "Nhan Thuong" };
