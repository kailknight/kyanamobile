...
bool CreateBugSub ( int Type, vec3_t Position, OBJECT *Owner, OBJECT * o, int SubType, int LinkBone )
{
	...
	if ( !o->Live )
	{
		...
		if (gCustomPet2.GetInfoPetType(o->Type-1171) == 5)
		{
			o->Scale = 0.9f;
		}
     	return FALSE;
...
bool MoveBug ( OBJECT* o, bool bForceRender )
{
	if ( o->Live )
	{
		...
		if (o->Owner->CacheTimeAttack > GetTickCount() && (gCustomPet2.GetInfoPetType(o->Type - 1171) == 5 || gCustomPet2.GetInfoPetType(o->Type - 1171) == 6
			|| o->Type == MODEL_FENRIR_BLACK
			|| o->Type == MODEL_FENRIR_BLUE
			|| o->Type == MODEL_FENRIR_RED
			|| o->Type == MODEL_FENRIR_GOLD
			|| o->Type == MODEL_DARK_HORSE)
			//||gCustomPet2.GetInfoPetType(o->Type - 1171) == 7 || gCustomPet2.GetInfoPetType(o->Type - 1171) == 11
			)
		{
			o->Alpha = 0.f;
			return true;
		...
		if (gCustomPet2.GetInfoPetType(o->Type - 1171) == 6)
		{
			goto GetDarHore;
		}
		switch ( o->Type )
...
GetDarHore:
		...
		//==//Custom Pet Move
		if ((gCustomPet2.GetInfoPetType(o->Type - 1171) >=0 && gCustomPet2.GetInfoPetType(o->Type - 1171) <=4) || (gCustomPet2.GetInfoPetType(o->Type - 1171) ==8))
		{
			...
			//gCustomPet2.PetFollowCharacter(o, 150);
		...
		if(o->Type == MODEL_HELPER || o->Type == MODEL_HELPER+1 || gCustomPet2.GetInfoPetType(o->Type - 1171) == 9 || gCustomPet2.GetInfoPetType(o->Type - 1171) == 10)
		{
			vec3_t Range;
			VectorSubtract(TargetPosition,o->Position,Range);
			float Distance = Range[0]*Range[0]+Range[1]*Range[1];
	...
	if(o->Live)
	{
		...
		if(o->Visible)
		{
			...
			if (gCustomPet2.CheckCustomPetByItem(o->Type - 1171) )
			{
				...
				{
					o->Scale = gCustomPet2.GetScaleView(o->Type - 1171);
				}
				else
				{

					o->Scale = gCustomPet2.GetScaleView(o->Type - 1171) + 0.2;
				}
			}
...
void Create_FlyBug(int Type, vec3_t Position, OBJECT* Owner, int SubType, int LinkBone)
{
	if (gMapManager.InChaosCastle() == true) return;
	...
	for (int i = 10; i < MAX_BUTTERFLES; i++)
	{
		OBJECT* o = &Butterfles[i];
		if (CreateBugSub(Type, Position, Owner, o, SubType, LinkBone) == FALSE) return;
	}
}
void CreateEffectEquipping(WORD Type, CHARACTER * pCha, OBJECT * o)
{
	if (Type != 0xFFFF)
	{
		switch (Type)
		{
		case ITEM_HELPER:Create_FlyBug(MODEL_HELPER, o->Position, o); break;
		case ITEM_HELPER + 2:Create_FlyBug(MODEL_UNICON, o->Position, o);
			if (!pCha->SafeZone)
				CreateEffect(BITMAP_MAGIC + 1, o->Position, o->Angle, o->Light, 1, o);
			break;
		case ITEM_HELPER + 3:Create_FlyBug(MODEL_PEGASUS, o->Position, o);
			if (!pCha->SafeZone)
				CreateEffect(BITMAP_MAGIC + 1, o->Position, o->Angle, o->Light, 1, o);
			break;
		case ITEM_HELPER + 4:Create_FlyBug(MODEL_DARK_HORSE, o->Position, o);
			if (!pCha->SafeZone)
				CreateEffect(BITMAP_MAGIC + 1, o->Position, o->Angle, o->Light, 1, o);
			break;
		case ITEM_HELPER + 64:
		...
		{
			{
				ThePetProcess().CreatePet(Type, Type+1171, o->Position, pCha);
			}
		}
		break;
		case ITEM_HELPER + 67:ThePetProcess().CreatePet(Type, Type + 1171, o->Position, pCha); break;
		case ITEM_HELPER + 80:ThePetProcess().CreatePet(Type, Type + 1171, o->Position, pCha); break;
		case ITEM_HELPER + 106:ThePetProcess().CreatePet(Type, Type + 1171, o->Position, pCha); break;
		case ITEM_HELPER + 123:ThePetProcess().CreatePet(Type, Type + 1171, o->Position, pCha); break;
			//	break;
		default:
			Create_FlyBug(Type + 1171, o->Position, o);
			break;
		}
	}
}
