...
#ifdef __ANDROID__
	const char* dbgTopName = "none";
	double dbgTopMs = 0.0;
	double dbgTotalMs = 0.0;
	static int s_dbgUiWinCounter = 0;
#endif
...
#ifdef __ANDROID__
	if ((++s_dbgUiWinCounter % 30) == 0)
	{
		if (FILE* f = fopen("mu_ui_windows_debug.txt", "a"))
		{
			fprintf(f, "UIWIN total=%.3fms top=%s(%.3fms)\n", dbgTotalMs, dbgTopName, dbgTopMs);
			fclose(f);
		}
	}
#endif
