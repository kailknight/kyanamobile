...
void    CSPetDarkSpirit::RenderCmdType ( void )
{
    ...
    if ( (Hero->Helper.Type>=MODEL_HELPER+0 && Hero->Helper.Type<=MODEL_HELPER+4) || Hero->Helper.Type == MODEL_HELPER+37 || gCustomPet2.GetInfoPetType(Hero->Helper.Type - 1171) == 5 ||gCustomPet2.GetInfoPetType(Hero->Helper.Type-1171) == 6)
    {
        PartyWidth += 60.f;
    }
