...
bool SEASON3B::CNewUIItemEnduranceInfo::UpdateMouseEvent()
{
    ...
    for( int i = EQUIPMENT_WEAPON_RIGHT; i<MAX_EQUIPMENT; ++i )
    {
		...
		{
			if( pItem->Type == ITEM_BOW+7 )//CustomArrow
			{
				continue;
			}
		}		
...
#if(HAISLOTRING)
	if (RenderEquipPetLife(m_UIStartPos.x, iNextPosY))
	{
		iNextPosY += (UI_INTERVAL_HEIGHT + PETHP_FRAME_HEIGHT);
	}
#endif
...
void SEASON3B::CNewUIItemEnduranceInfo::RenderHPUI( int iX, int iY, unicode::t_char* pszName, int iLife, int iMaxLife/*=255*/, bool bWarning/*=false*/ )
{
	...
	if( RequireCharisma > iCharisma ) 
		...
		glColor4f(1.f, 1.f, 1.f, 1.f);
	RenderImage(IMAGE_PETHP_FRAME, iX, iY - 368, PETHP_FRAME_WIDTH, PETHP_FRAME_HEIGHT);
	//DAT HPUI_Bar
	float fLife = ((float)iLife / (float)iMaxLife) * (float)PETHP_BAR_WIDTH;
	RenderImage(IMAGE_PETHP_BAR, iX + 4, iY - 368 + PETHP_FRAME_HEIGHT - PETHP_BAR_HEIGHT - 4, fLife, PETHP_BAR_HEIGHT);
	g_pRenderText->RenderText(iX + (PETHP_FRAME_WIDTH / 2), iY + 5 - 371, pszName, 0, 0, RT3_WRITE_CENTER);
...
void SEASON3B::CNewUIItemEnduranceInfo::RenderTooltip( int iX, int iY, const ITEM* pItem, const DWORD& dwTextColor )
{
	...
	if( iX+(iTooltipWidth/2) > (GetScreenWidth() + DisplayWinCDepthBox))
	{
		iX = (GetScreenWidth() + DisplayWinCDepthBox) - (iTooltipWidth/2);
	}
...
#if(HAISLOTRING)
bool SEASON3B::CNewUIItemEnduranceInfo::RenderEquipPetLife(int iX, int iY)
{
	BYTE t = 0;
	int Type = CharacterMachine->EquipmentMuun[12].Type;

	if (Type != 0xFFFF)
	{
		unicode::t_char szText[256] = { NULL, };
		const ITEM_ATTRIBUTE* pItemAttr = &ItemAttribute[Type];
		BYTE Dur = CharacterMachine->EquipmentMuun[12].Durability;

		if (strlen(pItemAttr->Name) > 0)
		{
			unicode::_sprintf(szText, pItemAttr->Name);
			RenderHPUI(iX, iY, szText, Dur, 255, 0); // tro ve day ne
		}
		t = 1;
	}
...
{
	int type = gCustomPet2.GetInfoPetType(Hero->Helper.Type - 1171);
 	if ( Hero->Helper.Type>=MODEL_HELPER && Hero->Helper.Type<=MODEL_HELPER+4
		|| Hero->Helper.Type == MODEL_HELPER+64
		|| Hero->Helper.Type == MODEL_HELPER+65
		|| Hero->Helper.Type == MODEL_HELPER+67
...
bool SEASON3B::CNewUIItemEnduranceInfo::RenderNumArrow( int iX, int iY )
{
	...
	if( m_iCurArrowType == ARROWTYPE_BOW ) //CustomArrow
	{
		iNumArrowSetInInven = g_pMyInventory->GetNumItemByType( ARROWTYPE_BOW );

		if(CharacterMachine->Equipment[EQUIPMENT_WEAPON_RIGHT].Type == ITEM_BOW+15) //CustomArrow
		{
			iNumEquipedArrowDurability = CharacterMachine->Equipment[EQUIPMENT_WEAPON_RIGHT].Durability;
		}
				
		if( (iNumArrowSetInInven == 0) && (CharacterMachine->Equipment[EQUIPMENT_WEAPON_RIGHT].Type != ITEM_BOW+15) ) //CustomArrow
			return false;

		unicode::_sprintf( szText, GlobalText[351], iNumEquipedArrowDurability, iNumArrowSetInInven );
	}
	else if( m_iCurArrowType == ARROWTYPE_CROSSBOW ) //CustomArrow
	{
		iNumArrowSetInInven = g_pMyInventory->GetNumItemByType( ARROWTYPE_CROSSBOW );

		if(CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Type == ITEM_BOW+7) //CustomArrow
		{
			iNumEquipedArrowDurability = CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Durability;
		}
				
		if( (iNumArrowSetInInven == 0) && (CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Type != ITEM_BOW+7) ) //CustomArrow
			return false;
	...
	g_pRenderText->RenderText(iX - 35, iY + 45 - 415, szText, 0, 0, RT3_SORT_LEFT);
