	...
	switch(head)
	{
		...
		case 0x18:
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
			{
				...
				case 0x07:
					#if(DATASERVER_UPDATE>=501)
					gCashShop.GDCashShopLogRecv((SDHP_CASH_SHOP_LOG_RECV*)lpMsg,index);
					#endif
					break;
				case 0x08:
					#if(DATASERVER_UPDATE>=501)
					gCashShop.GDCashShopCatalogReqRecv((SDHP_CASH_SHOP_CATALOG_REQ_RECV*)lpMsg,index);
					#endif
					break;
				case 0x0B:
					#if(DATASERVER_UPDATE>=501)
					gCashShop.GDCashShopPriceReqRecv((SDHP_CASH_SHOP_PRICE_REQ_RECV*)lpMsg,index);
					#endif
					break;
				case 0x0F:
					#if(DATASERVER_UPDATE>=501)
					gCashShop.GDCashShopCatalogLineReqRecv((SDHP_CASH_SHOP_CATALOG_LINE_REQ_RECV*)lpMsg,index);
					#endif
					break;
				case 0x30:
					#if(DATASERVER_UPDATE>=501)
...
#endif
#if(REDEEMCODE)
			case 0x30: //==Redeem code - preview request
				gRedeemCode.OnPeekRequest((GSSENDDS_REDEEM_CODE_PEEK*)lpMsg, index);
				break;
			case 0x31: //==Redeem code - commit request
				gRedeemCode.OnCommitRequest((GSSENDDS_REDEEM_CODE_COMMIT*)lpMsg, index);
				break;
#endif
			case 0x32: //==Spin wheel claim box - load request
				gSpinClaim.GDSpinClaimRecv((SDHP_SPINCLAIM_RECV*)lpMsg, index);
				break;
			case 0x33: //==Spin wheel claim box - save
				gSpinClaim.GDSpinClaimSaveRecv((SDHP_SPINCLAIM_SAVE_RECV*)lpMsg);
				break;
			case 0x34: //==Slot machine reward box - load request
				gSlotClaim.GDSlotClaimRecv((SDHP_SLOTCLAIM_RECV*)lpMsg, index);
				break;
