...
// GameServer's DSProtocol.h (this codebase keeps GameServer<->DataServer wire
// structs duplicated rather than shared between the two projects, same as
// GSSENDDS_COUNT_ITEMBANK just above). Keep any field change mirrored on
// both sides.
//-----------------------------------

// Client-facing result codes (GameServer's Protocol.h) - duplicated here too
// since RedeemCode.cpp needs to compute the same values from SQL results.
...
struct REDEEM_BUNDLE_ITEM
{
	...
	BYTE Quantity;
	DWORD ItemDuration;
	BYTE SocketOption[5]; // matches GameServer's own SDHP_CREATE_ITEM_RECV::SocketOption[5] literal, not a named MAX_SOCKET_OPTION here (that macro doesn't exist on this side)
	BYTE SocketOptionBonus;
	BYTE Item380;
	BYTE HarmonyOption;
...
// Per-code currency rewards, read out of CustomRedemptionCodes itself rather
// than the item table - purely a carrier between RunRedeemQuery and the reply
// packets, never sent on the wire in this shape.
struct REDEEM_CURRENCY_REWARD
{
	DWORD WCoinC;
	DWORD WCoinP;
	DWORD GoblinPoint;
...
// Must stay byte-identical to GameServer's hand-duplicated copy in its own
// DSProtocol.h - see the matching assert there. A stride mismatch corrupts
// Items[1] onward while Items[0] still looks correct, so it is exactly the
// kind of drift that survives casual testing. Rebuild BOTH sides together.
static_assert(sizeof(REDEEM_BUNDLE_ITEM) == 24, "REDEEM_BUNDLE_ITEM must stay 24 bytes and byte-identical to GameServer's copy in DSProtocol.h");
...
void GDMonterPointSaveRecv(SDHP_MONTERPOINT_SAVE_SEND*lpMsg);
